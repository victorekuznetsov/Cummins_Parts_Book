---
aliases:
  - "Опциональный переход на цельный центробежный фильтр Eliminator™"
type: "TSB"
doc: "tsb150164"
title_en: "Optional One-Piece Eliminator™ Centrifugal Oil Filter Upgrade"
title_ru: "Опциональный переход на цельный центробежный фильтр Eliminator™"
released: "2017-05-05"
modified: "2017-05-05"
group: "07 - Lubricating Oil Systems"
engines:
  - "33210083"
  - "33219033"
  - "33224343"
  - "33224404"
  - "33239746"
  - "33239899"
  - "41340468"
families:
  - "K38/K50 · QSK38, QSK50, QSK60"
  - "QSK50"
  - "QSK60"
parts:
  - "3393587"
  - "3393591"
  - "3631862"
  - "3631863"
  - "3631864"
  - "3631865"
figures: 1
lang: "ru+en"
translation: "машинный черновик"
source: "https://quickserve.cummins.com/qs3/pubsys2/xml/en/tsb/2015/tsb150164.html"
pdf: "https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/main/bulletins/tsb/tsb150164.pdf"
tags:
  - "документ/tsb"
  - "двигатель/K38/K50"
  - "двигатель/QSK50"
  - "двигатель/QSK60"
  - "год/2017"
  - "перевод/машинный"
  - "тема/lubricating-oil-systems"
---

# Optional One-Piece Eliminator™ Centrifugal Oil Filter Upgrade
**Опциональный переход на цельный центробежный фильтр Eliminator™**

> [!abstract] TSB · `tsb150164`
> **Раздел Cummins:** 07 - Lubricating Oil Systems
> **Двигатели:** [[33210083 — QSK60 CM500 CPL 2699|33210083]], [[33219033 — QSK60 CM500 CPL 2848|33219033]], [[33224343 — QSK60 CM500 CPL 2849|33224343]], [[33224404 — QSK50 CM2150 MCRS CPL 3391|33224404]], [[33239746 — QSK60 CM2150 MCRS CPL 3451|33239746]], [[33239899 — QSK50 CM2150 MCRS CPL 3379|33239899]], [[41340468 — QSK50 CM2150 MCRS CPL 3728|41340468]]
> **Семейство:** K38/K50 · QSK38, QSK50, QSK60, QSK50, QSK60
> **Даты:** выпущен 2017-05-05 · изменён 2017-05-05
> **Источник:** [QuickServe](https://quickserve.cummins.com/qs3/pubsys2/xml/en/tsb/2015/tsb150164.html) · [PDF-оригинал](https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/main/bulletins/tsb/tsb150164.pdf)

> [!info]- Перевод на русский — машинный черновик
> Русский текст получен автоматическим переводом с английского
> с подстановкой отраслевой терминологии Cummins; он не
> проходил редакторскую вычитку.
> **Юридически значим только английский оригинал** — он
> приведён в свёрнутом блоке в конце заметки и в PDF.


## Опциональный переход на цельный центробежный фильтр Eliminator™

### Положение о гарантии

Сведения в этом документе не изменяют действующие гарантийные обязательства и практику ремонта и не дают оснований для работ по программам TRP или Campaign.

### Содержание

**Затронутая продукция**

- K38 (все версии)
- QSK38 (все версии)
- QSK45 (все версии)
- K50 (все версии)
- QSK50 (все версии)
- QSK60 (все версии)
- QSK95 (все версии)

**Описание изменения**

В этом документе анонсирован выпуск нового фильтра Cummins FiltrationTM с однокомпонентным центробежным масляным фильтром EliminatorTM с технологией SpiraTecTM.

**Причина изменения**

Новый фильтр Cummins FiltrationTM, цельный центробежный масляный фильтр EliminatorTM с технологией SpiraTecTM, является полностью одноразовым и герметичным блоком, который содержит фильтрующий элемент и втулки. Это не требует очистки, так как центробежный масляный фильтр просто заменяется. Чтобы предотвратить преждевременное износ втулки на дисковом стеке Alfa LavalTM, Cummins Inc. Рекомендуется использовать новый фильтр Cummins FiltrationTM, цельный центробежный масляный фильтр EliminatorTM с технологией SpiraTecTM во всех системах EliminatorTM.

**Наличие сервисных деталей**

Сервисные детали доступны для заказа. Номера деталей приведены в таблице 1.

| Таблица 1, Части обслуживания |  |
|---|---|
| Наименование | Fleetguard - Номер детали |
| Однопицевый элиминатор TM Центробежный масляный фильтр с технологией SpiraTecTM | CS41018 |

| Таблица 2, Номера частей центробежного масляного фильтра |  |  |  |  |  |
|---|---|---|---|---|---|
| Оригинальные номера фильтрационных частей | Модернизированные номера фильтрационных деталей |  |  |  |  |
| Фильтр для центробежного масла с дисковым стеком Alfa LavalTM | Фильтрация CumminsTM Центробежный масляный фильтр с технологией ConeStaCTM | Однопицевый элиминатор TM Центробежный масляный фильтр с технологией SpiraTecTM |  |  |  |
| Описание (местоположение на рисунке 1) | Номер детали | Наименование | Номер детали | Наименование | Номер детали |
| Угол (2) | [[3393587]] | О-кольцо | [[3393587]] | Центробежный фильтр для масла | Fleetguard® CS41018 |
| Сборка органов роторов (21) | 3649833 | Сборка органов Ротора | 3649833 |  |  |
| Роторная крышка (22) | 3631861 | Роторное покрытие | 3631861 |  |  |
| Дистрибьютор (23) | [[3631862]] | ConeStaCTM для центробежного фильтра | Fleetguard® CS44000 |  |  |
| Лучший диск (24) | [[3631863]] |  |  |  |  |
| Боул Диск (25) | [[3393591]] |  |  |  |  |
| Распределяющий конус (26) | [[3631864]] |  |  |  |  |
| Кольцо Снап (27) | [[3631865]] |  |  |  |  |
| Сбор ила из бумаги (4) | 3393592 |  |  |  |  |

![[07r00058.png]]

Рисунок 1 - Центробежный фильтр для масла с стеком дисков Alfa Laval

**Совместимость частей**

Новый усовершенствованный цельный центробежный масляный фильтр обратно совместим.

> [!note] Примечание
> Если цельная центрифуга установлена на двигателе, который ранее имел ограничительное отверстие, установленное на вал центрифуги, ограничитель должен быть удален. Его можно снять с вала с помощью шестигранного ключа.

**Идентификация детали**

Новый усовершенствованный цельный центробежный масляный фильтр можно отличить, используя номер детали, указанный на фильтре.

**Часть структуры**

Модернизированный номер детали центробежного масляного фильтра, состоящий из одного элемента, не устарел или заменил существующие номера детали фильтрации. Существующие номера фильтрационных деталей останутся в наличии.

**Публикации затронуты**

| Таблица 3, Затронутые публикации |  |  |  |  |  |
|---|---|---|---|---|---|
| Тип ручного | Двигатель | Номер бюллетеня | Название процедуры | Процедура | Раздел |
| Эксплуатация и техническое обслуживание | K38, K50, QSK38 и QSK50 | [[3810497 — K38, K50, QSK38 and QSK50 Operation and Maintenance Manual\|3810497]] | Центрифуга фильтра Eliminator | 007-066 | 6 |
| QSK38 M2150 | 4332769 | 007-066 |  |  |  |
| QSK45/QSK60 | [[3666260 — QSK45 and QSK60 Operation and Maintenance Manual\|3666260]] | 007-066 |  |  |  |
| QSK50 CM2150 | 4332774 | 007-066 |  |  |  |
| QSK50 CM2350 | 4332824 | 007-066 |  |  |  |
| QSK60 CM2350 | 4367445 | 007-066 |  |  |  |
| QSK95 M2350 | 4358520 | 007-066 |  |  |  |

### История изменений документа

## Детали, упомянутые в документе

| Артикул | Наименование | Русское название |
|---|---|---|
| [[3393587]] | O Ring Seal | Уплотнительное кольцо |
| [[3393591]] | OIL STRAINER |  |
| [[3631862]] | FLUID MANIFOLD |  |
| [[3631863]] | OIL STRAINER |  |
| [[3631864]] | CONNECTION RETAINER |  |
| [[3631865]] | RETAINING RING | Стопорное кольцо |

> [!quote]- Original (English) · английский оригинал
> ## Optional One-Piece Eliminator™ Centrifugal Oil Filter Upgrade
>
> ### Warranty Statement
>
> The information in this document has no effect on present warranty coverage or repair practices, nor does it authorize TRP or Campaign actions.
>
> ### Contents
>
> **Product Affected**
>
> - K38 (All Versions)
> - QSK38 (All Versions)
> - QSK45 (All Versions)
> - K50 (All Versions)
> - QSK50 (All Versions)
> - QSK60 (All Versions)
> - QSK95 (All Versions)
>
> **Description of Change**
>
> This document announces the release of a new Cummins Filtration™ one-piece Eliminator™ centrifugal oil filter with SpiraTec™ technology.
>
> **Reason for Change**
>
> The new Cummins Filtration™ one-piece Eliminator™ centrifugal oil filter with SpiraTec™ technology is a fully disposable and sealed unit that contains the filtering element and bushings. This requires no cleaning as the centrifugal oil filter is just replaced. To prevent premature bushing wear on the Alfa Laval™ disc stack, Cummins Inc. recommends the use of the new Cummins Filtration™ one-piece Eliminator™ centrifugal oil filter with SpiraTec™ technology in all Eliminator™ systems.
>
> **Service Parts Availability**
>
> Service parts are available. See Table 1 for part numbers.
>
> | Table 1, Service Parts |  |
> |---|---|
> | Description | Fleetguard® Part Number |
> | One-Piece Eliminator™ Centrifugal Oil Filter with SpiraTec™ Technology | CS41018 |
>
> | Table 2, Centrifugal Oil Filter Part Numbers |  |  |  |  |  |
> |---|---|---|---|---|---|
> | Original Filtration Part Numbers | Upgraded Filtration Part Numbers |  |  |  |  |
> | Centrifugal Oil Filter with Alfa Laval™ disc stack | Cummins Filtration™ Centrifugal Oil Filter with ConeStaC™ Technology | One-Piece Eliminator™ Centrifugal Oil Filter with SpiraTec™ Technology |  |  |  |
> | Description (Location in Figure 1) | Part Number | Description | Part Number | Description | Part Number |
> | O-ring (2) | [[3393587]] | O-ring | [[3393587]] | Centrifugal Oil Filter | Fleetguard® CS41018 |
> | Rotor Body Assembly (21) | 3649833 | Rotor Body Assembly | 3649833 |  |  |
> | Rotor Cover (22) | 3631861 | Rotor Cover | 3631861 |  |  |
> | Distributor (23) | [[3631862]] | ConeStaC™ for Centrifugal By-Pass Filter | Fleetguard® CS44000 |  |  |
> | Top Disc (24) | [[3631863]] |  |  |  |  |
> | Bowl Disc (25) | [[3393591]] |  |  |  |  |
> | Distributing Cone (26) | [[3631864]] |  |  |  |  |
> | Snap Ring (27) | [[3631865]] |  |  |  |  |
> | Sludge Collecting Paper (4) | 3393592 |  |  |  |  |
>
> Figure 1, Centrifugal Oil Filter with Alfa Laval Disc Stack
>
> **Part Compatibility**
>
> The new upgraded one-piece centrifugal oil filter is backwards compatible.
>
> **Note · Примечание**
> If the one-piece centrifuge is fitted to an engine that has previously had a restrictor orifice fitted to the centrifuge shaft, the restrictor **must** be removed. It can be removed from the shaft with a hex key.
>
> **Part Identification**
>
> The new upgraded one-piece centrifugal oil filter can be distinguished using the part number listed on the filter can.
>
> **Part Structure**
>
> The upgraded one-piece centrifugal oil filter part number does **not** obsolete or supersede the existing filtration part numbers. The existing filtration part numbers will remain available.
>
> **Publications Affected**
>
> | Table 3, Publications Affected |  |  |  |  |  |
> |---|---|---|---|---|---|
> | Manual Type | Engine | Bulletin Number | Procedure Title | Procedure | Section |
> | Operation and Maintenance | K38, K50, QSK38 and QSK50 | [[3810497 — K38, K50, QSK38 and QSK50 Operation and Maintenance Manual\|3810497]] | Eliminator Filter Centrifuge | 007-066 | 6 |
> | QSK38 CM2150 | 4332769 | 007-066 |  |  |  |
> | QSK45/QSK60 | [[3666260 — QSK45 and QSK60 Operation and Maintenance Manual\|3666260]] | 007-066 |  |  |  |
> | QSK50 CM2150 | 4332774 | 007-066 |  |  |  |
> | QSK50 CM2350 | 4332824 | 007-066 |  |  |  |
> | QSK60 CM2350 | 4367445 | 007-066 |  |  |  |
> | QSK95 CM2350 | 4358520 | 007-066 |  |  |  |
>
> ### Document History
