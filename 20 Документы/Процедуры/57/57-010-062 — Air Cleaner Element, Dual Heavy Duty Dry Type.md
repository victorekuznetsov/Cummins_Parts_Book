---
aliases:
  - "Элемент воздухоочистителя, двойной сухого типа"
type: "Процедура"
doc: "57-010-062"
title_en: "Air Cleaner Element, Dual Heavy Duty Dry Type"
title_ru: "Элемент воздухоочистителя, двойной сухого типа"
modified: "2003-01-09"
engines:
  - "37292556"
  - "37295879"
families:
  - "QST30"
manuals:
  - "3666134"
figures: 1
lang: "ru+en"
translation: "машинный черновик"
source: "https://quickserve.cummins.com/qs3/pubsys2/xml/en/procedures/57/57-010-062.html"
pdf: "https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/claude/cummins-parts-knowledge-base-qa0n50/bulletins/procedures/57-010-062.pdf"
tags:
  - "документ/процедура"
  - "двигатель/QST30"
  - "группа/57"
  - "перевод/машинный"
---

# Air Cleaner Element, Dual Heavy Duty Dry Type
**Элемент воздухоочистителя, двойной сухого типа**

> [!abstract] Процедура · `57-010-062`
> **Двигатели:** [[37292556 — QST30 CM552 CPL 1244|37292556]], [[37295879 — QST30 CM552 CPL 2139|37295879]]
> **Семейство:** QST30
> **Входит в руководства:** [[3666134 — QST30 Operation and Maintenance Manual|3666134]]
> **Секции:** Section A - Adjustment, Repair, and Replacement
> **Даты:** изменён 2003-01-09
> **Источник:** [QuickServe](https://quickserve.cummins.com/qs3/pubsys2/xml/en/procedures/57/57-010-062.html) · [PDF-оригинал](https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/claude/cummins-parts-knowledge-base-qa0n50/bulletins/procedures/57-010-062.pdf)

> [!info]- Перевод на русский — машинный черновик
> Русский текст получен автоматическим переводом с английского
> с подстановкой отраслевой терминологии Cummins; он не
> проходил редакторскую вычитку.
> **Юридически значим только английский оригинал** — он
> приведён в свёрнутом блоке в конце заметки и в PDF.


### Служба технического обслуживания

Очистители воздуха сочетают центробежную очистку с фильтрацией элементов перед тем, как воздух попадает в двигатели.

Перед разборкой протереть грязь с крышки и верхней части воздухоочистителя.

Ослабьте крыло (3), удалите зажим (1), закрепив пылесос (2).

Освободите крыло (3). Удалите пылевой щит (4) из пылесоса (2). Очистите пылесборник и щит.

Удалите крыло (5), которое обеспечивает защиту воздуха от первичного элемента (6) в корпусе воздухоочистителя. Осмотрите резиновую уплотнительную стиральную машину на крыле.

Удалите грязный первичный чистящий элемент (6). Если внутренний элемент (8) безопасности заменяется на основе высокого ограничения потребления, удалите гайку (7) крыла и замените внутренний элемент безопасности.

Установите внутренний предохранительный элемент (8) и закрепите его гайкой (7). Проверьте уплотнения.

Установите план пыли (2) и зажим полосы (1). Управляйте двигателем с номинальной скоростью и мощностью и записывайте ограничение на впуск.

![[ac4etgc.png]]


> [!quote]- Original (English) · английский оригинал
> ### Maintenance Service
>
> Heavy duty air cleaners combine centrifugal cleaning with element filtering before air enters the engines.
>
> Before disassembly, wipe dirt from the cover and the upper portion of the air cleaner.
>
> Loosen the wing nut (3), remove the band clamp (1) securing the dust pan (2).
>
> Loosen the wing nut (3). Remove the dust shield (4) from the dust pan (2). Clean the dust pan and shield.
>
> Remove the wing nut (5) that secures the air cleaner primary element (6) in the air cleaner housing. Inspect the rubber sealing washer on the wing nut.
>
> Remove the dirty primary cleaner element (6). If the inner safety element (8) is being replaced based on high intake restriction, remove the wing nut (7) and replace the inner safety element.
>
> Install the inner safety element (8) and secure with the wing nut (7). Check the seals.
>
> Install the dust plan (2) and band clamp (1). Operate the engine at rated speed and power and record the intake restriction.
