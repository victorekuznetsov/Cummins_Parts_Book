---
aliases:
  - "Регулировка клапанного механизма"
type: "Процедура"
doc: "57-003-004"
title_en: "Overhead Set"
title_ru: "Регулировка клапанного механизма"
modified: "2005-09-09"
engines:
  - "37292556"
  - "37295879"
families:
  - "QST30"
manuals:
  - "3666134"
figures: 24
lang: "ru+en"
translation: "машинный черновик"
source: "https://quickserve.cummins.com/qs3/pubsys2/xml/en/procedures/57/57-003-004.html"
pdf: "https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/claude/cummins-parts-knowledge-base-qa0n50/bulletins/procedures/57-003-004.pdf"
tags:
  - "документ/процедура"
  - "двигатель/QST30"
  - "группа/57"
  - "перевод/машинный"
---

# Overhead Set
**Регулировка клапанного механизма**

> [!abstract] Процедура · `57-003-004`
> **Двигатели:** [[37292556 — QST30 CM552 CPL 1244|37292556]], [[37295879 — QST30 CM552 CPL 2139|37295879]]
> **Семейство:** QST30
> **Входит в руководства:** [[3666134 — QST30 Operation and Maintenance Manual|3666134]]
> **Секции:** Section 5 - Maintenance Procedures at 2000 Hours or 1 Year
> **Даты:** изменён 2005-09-09
> **Источник:** [QuickServe](https://quickserve.cummins.com/qs3/pubsys2/xml/en/procedures/57/57-003-004.html) · [PDF-оригинал](https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/claude/cummins-parts-knowledge-base-qa0n50/bulletins/procedures/57-003-004.pdf)

> [!info]- Перевод на русский — машинный черновик
> Русский текст получен автоматическим переводом с английского
> с подстановкой отраслевой терминологии Cummins; он не
> проходил редакторскую вычитку.
> **Юридически значим только английский оригинал** — он
> приведён в свёрнутом блоке в конце заметки и в PDF.


### Общие сведения

Клапаны ** должны быть правильно отрегулированы для эффективного функционирования двигателя. Корректировка клапана ** должна** выполняться с использованием значений, перечисленных в этой процедуре.

Приказ на стрельбу двигателя: 1R-1L-5R-5L-3R-6R-6L-2R-2L-4R-4L

Цилиндры пронумерованы от передней части крышки передних передач двигателя.

Чтобы определить правый и левый берега на двигателе, встаньте сзади двигателя и лицом к переднему.

Для регулировки всех клапанов требуется два оборота коленчатого вала.

![[02600054.png]]

Каждый цилиндр имеет два рычага коромысла. На левом берегу ближайшим к задней части двигателя рычагом является впускной рычаг. На правом берегу выхлопной клапан находится ближе всего к задней части.

Одна пара клапанов регулируется на каждом индексном знаке перед поворотом двигателя на следующий индексный знак.

![[03a00031.png]]

> [!danger] ОПАСНО
> При работе с пароочистителем надевайте защитные очки или щиток и защитную одежду. Горячий пар может привести к тяжёлой травме.

Если вы ранее не очищали двигатель, очистите паром двигатель, чтобы предотвратить попадание грязи в двигатель, когда клапанные крышки удаляются. См. раздел 5.

![[00a00009.png]]

### Снятие

Удалите крышки клапанного клапана и все связанные с ними компоненты.

![[03a00001.png]]

Отсоедините форсунка NBF от проводной ремни.

![[06a00031.png]]

Удалите резиновый громмет из крышки рычага качения клапана.

Пропускайте свинцовый разъём NBF через отверстие в крышке рычага клапанного клапана.

![[06a00030.png]]

### Регулировка

Отметки регулировки клапанов находятся на вибрационном демпфере. Знаки ** должны быть выровнены с указателем.

![[03a00004.png]]

Вращайте двигатель ** по часовой стрелке**, как указывает стрелка на шкиве, с помощью заградительного инструмента.

![[00a00018.png]]

Убедитесь, что клапаны правого берега номера 1 готовы к корректировке, увидев, что оба рычага коромысла на правом берегу номера 1 ровные, а оба рычага коромысла на правом берегу номера 6 неравномерны.

Если **не**, поверните двигатель на 180 градусов.

![[03a00004.png]]

| QST30 клапан Adjustment Chart (схема настройки клапанов) |  |  |
|---|---|---|
| Вибрационная дампа Mark | Закрытые или нажатые протрузии даже на цилиндре | Настройка клапанов на число цилиндров |
| R1-6 | 1RB | 1RB |
| L1-6 | 1LB | 1LB |
| R2-5 | 5RB | 5RB |
| L2-5 | 5LB | 5LB |
| R3-4-4 | 3RB | 3RB |
| L3-4-4 | 3LB | 3LB |
| R1-6 | 6RB | 6RB |
| L1-6 | 6LB | 6LB |
| R2-5 | 2RB | 2RB |
| L2-5 | 2LB | 2LB |
| R3-4-4 | 4RB | 4RB |
| L3-4-4 | 4LB | 4LB |

Рычаги качения будут одинаковой высоты на цилиндре, готовом к регулировке клапана.

Клапанные мосты и клапаны готовы к регулировке на цилиндре, который имеет все клапаны закрыты.

Проверьте два цилиндра, показанные знаком на вибрационном демпфере.

Если клапанные клапанные рамы были удалены, используйте этот шаг для определения цилиндра для установки.

Все регулирующие винты ** должны быть свободны на всех цилиндрах, а толкатели ** должны оставаться в выравнивании.

Держите оба рычага качения против клапанных мостиков. Поверните регулирующие винты, пока они не коснутся толкателей. Поверните каштаны, пока они не коснутся рычагов.

![[03a00002.png]]

Цилиндр с регулирующими винтами, которые имеют почти одинаковую высоту, готов к регулировке клапана. Второй цилиндр, который ** не** готов к регулировке, будет иметь регулировочный винт для впускных клапанов более пяти резьб над выхлопным винтом.

Рычаги качелей будут близки к той же высоте над верхней частью корпуса качелей клапанного клапана на цилиндре, готовом к регулировке клапана.

![[03a00006.png]]

Если рычаги коромысла были **не** удалены и установлены снова, поверните рычаги коромысла клапана на двух цилиндрах, о которых идет речь. Клапанные мосты и клапаны на цилиндре, где оба рычага чувствуют себя свободными, готовы к регулировке.

![[03a00003.png]]

> [!warning] ОСТОРОЖНО
> Используйте правильный процедура стрельбы для двигателя. Неспособность сделать это может привести к повреждению двигателя.

Настройка может начинаться на любом отметке клапана. В примере предположим, что отметка R1-6 выровнена, и высота регулирующего винта для клапана на правом берегу цилиндра № 1 одинакова, что указывает на то, что клапан закрыт и готов к регулировке.

После определения того, какой цилиндр готов к регулировке, следуйте порядку запуска двигателя для последующих регулировок.

Заказ на стрельбу двигателя составляет 1R-1L-5R-5L-3R-3L-6R-6L-2R-2L-4R-4L.

![[03a00004.png]]

Регулировка клапанного моста ** должна всегда** производиться перед попыткой регулировки клапанов.

Настройте клапанные мосты на цилиндре, который имеет оба клапана закрыты.

Ослабьте клапанный мост, регулирующий винтовые замки на впускных и выпускных клапанных мостах.

![[02a00001.png]]

Используйте следующую процедуру для регулировки мостов впускного и выпускного клапанов.

Поверните регулировочный винт, по крайней мере, на один оборот.

Держите клапанный мостик вниз против его направляющего.

Включите регулирующий винт, пока он не коснется верхней части стебля клапана, но не поднимает клапанный мост.

![[02a00002.png]]

Регулирующий винт ** не должен** поворачиваться, когда гайка замка затянута.

Затянуть гаечный орех с помощью крутящего момента сервисной оснастки гаечного ключа, Часть номер ST-699, с адаптером (1).

> [!tip] Момент затяжки
> 45 Н·м [35 фунт-фут]

Затяните гаечный орех с помощью инструмента для обслуживания гаечных ключей, часть номер ST-699, без адаптера.

> [!tip] Момент затяжки
> 60 Н·м [45 фунт-фут]

![[02a00003.png]]

Выберите калибр для правильной спецификации ресниц клапана.

Включить калибр (2) между и клапанным клапаном и клапанным мостиком.

| Коррекция впуска и выхлопных клапанов |  |  |
|---|---|---|
| клапан | мм | [в] |
| выхлоп | 0.80 | 0.032 |
| принимать | 0.43 | 0.017 |

![[rh400ue.png]]

Ниже описаны два различных метода установления зазора клапанной ресницы. Два метода - это метод гаечного ключа или метод ощущений; любой метод может быть использован; однако метод гаечного ключа оказался наиболее последовательным.

Используйте служебный инструментальный ключ с крутящим моментом в дюйме, номер детали 3376592.

> [!tip] Момент затяжки
> 0.68 Н·м [6 фунт-дюйм]

Для метода ощущений используйте отвертку, чтобы повернуть регулирующий винт **только **, пока рычаг не коснется измерителя щупальца.

![[rh6toua.png]]

Регулирующий винт ** не должен** поворачиваться, когда гайка замка затянута.

Затянуть гаечный орех с помощью крутящего момента сервисной оснастки гаечного ключа, Часть номер ST-699, с адаптером (1).

> [!tip] Момент затяжки
> 45 Н·м [35 фунт-фут]

Затяните гаечный орех с помощью инструмента для обслуживания гаечных ключей, часть номер ST-699, без адаптера.

> [!tip] Момент затяжки
> 60 Н·м [45 фунт-фут]

![[02a00003.png]]

Измеритель щупальца ** должен ** скользить назад и вперед с ** только ** небольшим сопротивлением.

Попробуйте вставить датчик измерения щупальца, который составляет 0,03 мм \[0,001 в\] толще. Стрелка клапана ** не** правильна, когда более толстый калибр подойдет.

Повторите процесс регулировки до тех пор, пока зазор не будет правильным как на впускном, так и на выпускном клапанах на регулироваемом цилиндре.

![[03a00030.png]]

После того, как накладные расходы были полностью установлены, проверьте регулировочный выступ винтов. Примерно три резьбы ** должны быть выставлены над каштаном.

Если ** не**, то ищите:

- Накладные расходы, установленные ненадлежащим образом
- Прокладка гильзы пропала
- Толкать стержнем **не** в розетке
- Щелчок для крана ** не** сидящий должным образом
- Толщина орехов **не** стандарт
- клапанный мостик от стебля клапана.

![[03a00065.png]]

### Установка

Установите каждую прокладку крышки коромысла, крышку рычага клапанного клапана и болты.

Затяните все болты.

> [!tip] Момент затяжки
> 5 Н·м [44 фунт-дюйм]

> [!note] Примечание
> Для промышленных двигателей крышка коромысла № 1 на правом и левом берегах имеет отверстие для громмета, для форсунка NBF.

![[03a00001.png]]

Установите резиновый громмет из крышки рычага клапанного клапана.

Пропускайте свинцовый разъём NBF через отверстие в крышке рычага клапанного клапана.

![[06a00030.png]]

Подключите форсунка NBF от проводной ремни.

![[06a00031.png]]


> [!quote]- Original (English) · английский оригинал
> ### General Information
>
> Valves **must** be adjusted correctly for the engine to operate efficiently. Valve adjustment **must** be performed using the values listed in this procedure.
>
> Engine firing order: 1R-1L-5R-5L-3R-3L-6R-6L-2R-2L-4R-4L
>
> Cylinders are numbered from the front gear cover end of the engine.
>
> To determine the right and left banks on the engine, stand at the rear of the engine and face the front.
>
> Two crankshaft revolutions are required to adjust all of the valves.
>
> Each cylinder has two rocker levers. On the left bank, the lever nearest to the rear of the engine is the intake lever. On the right bank, the exhaust valve is nearest to the rear.
>
> One pair of valves are adjusted at each index mark before rotating the engine to the next index mark.
>
> **WARNING · Опасно**
> When using a steam cleaner, wear safety glasses or a face shield, as well as protective clothing. Hot steam can cause serious personal injury.
>
> If you have **not** previously cleaned the engine, steam clean the engine to prevent dirt from entering the engine when the valve covers are removed. Refer to Section 5.
>
> ### Remove
>
> Remove the rocker lever covers and all related components.
>
> Disconnect the NBF injector lead from the wiring harness.
>
> Remove the rubber grommet from the rocker lever cover.
>
> Pass the NBF lead connector through the hole in the rocker lever cover.
>
> ### Adjust
>
> Valve adjustment marks are on the vibration damper. The marks **must** be aligned with the pointer.
>
> Rotate the engine **clockwise**, as the arrow on the pulley indicates, with a barring tool.
>
> Check that the number 1 right-bank valves are ready to be adjusted by seeing that both rocker levers on the number 1 right bank are even and both rocker levers on the number 6 right bank are uneven.
>
> If **not**, rotate the engine 180 degrees.
>
> | QST30 Valve Adjustment Chart |  |  |
> |---|---|---|
> | Vibration Damper Mark | Valves Closed or Push Rod Protrusion Even on Cylinder Number | Adjust Valves on Cylinder Number |
> | R1-6 | 1RB | 1RB |
> | L1-6 | 1LB | 1LB |
> | R2-5 | 5RB | 5RB |
> | L2-5 | 5LB | 5LB |
> | R3-4 | 3RB | 3RB |
> | L3-4 | 3LB | 3LB |
> | R1-6 | 6RB | 6RB |
> | L1-6 | 6LB | 6LB |
> | R2-5 | 2RB | 2RB |
> | L2-5 | 2LB | 2LB |
> | R3-4 | 4RB | 4RB |
> | L3-4 | 4LB | 4LB |
>
> The rocker levers will be the same height on the cylinder ready for valve adjustment.
>
> The crossheads and valves are ready to be adjusted on the cylinder that has all the valves closed.
>
> Check the two cylinders shown by a mark on the vibration damper.
>
> If the rocker lever assemblies have been removed, use this step to determine the cylinder to set.
>
> All adjusting screws **must** be loose on all cylinders and the push rods **must** remain in alignment.
>
> Hold both rocker levers against the crossheads. Turn the adjusting screws until they touch the push rods. Turn the locknuts until they touch the levers.
>
> The cylinder with the adjusting screws that are nearly the same height is ready for valve adjustment. The second cylinder that is **not** ready for adjustment will have the adjusting screw for the intake valves more than five threads above the exhaust screw.
>
> The rocker levers will be close to the same height above the top of the rocker lever housing on the cylinder ready for valve adjustment.
>
> If the rocker levers have **not** been removed and installed again, wiggle the valve rocker levers on the two cylinders in question. The crossheads and valves on the cylinder where both levers feel loose are ready to adjust.
>
> **CAUTION · Осторожно**
> Use the correct firing order for the engine. Failure to do so can cause engine damage.
>
> Adjustment can begin on any valve set mark. In the example, assume the R1-6 mark is aligned and the adjusting screw height for the valve on cylinder number 1 right bank is the same, indicating the valve is closed and ready to adjust.
>
> After identifying which cylinder is ready to be adjusted, follow the engine firing order for subsequent adjustments.
>
> The firing order for the engine is 1R-1L-5R-5L-3R-3L-6R-6L-2R-2L-4R-4L.
>
> Crosshead adjustment **must always** be made before attempting to adjust the valves.
>
> Adjust the crossheads on the cylinder that has both valves closed.
>
> Loosen the crosshead adjusting screw locknuts on the intake and exhaust valve crossheads.
>
> Use the following procedure to adjust both the intake and exhaust crossheads.
>
> Turn the adjusting screw out at least one turn.
>
> Hold the crosshead down against its guide.
>
> Turn the adjusting screw in until it touches the top of the valve stem but does **not** raise the crosshead.
>
> The adjusting screw **must not** turn when the lock nut is tightened.
>
> Tighten the locknut using torque wrench service tool, Part Number ST-699, with adapter (1).
>
> **Момент затяжки · Torque Value**
> 45 n•m [35 ft-lb]
>
> Tighten the locknut using torque wrench service tool, Part Number ST-699, without adapter.
>
> **Момент затяжки · Torque Value**
> 60 n•m [45 ft-lb]
>
> Select a feeler gauge for the correct valve lash specification.
>
> Insert the gauge (2) between and rocker lever and the crosshead.
>
> | Intake and Exhaust Valve Adjustment |  |  |
> |---|---|---|
> | Valve | mm | \[in\] |
> | Exhaust | 0.80 | 0.032 |
> | Intake | 0.43 | 0.017 |
>
> Two different methods for establishing valve lash clearance are described below. The two methods are torque wrench method or feel method; either method can be used; however, the torque wrench method has proven to be the most consistent.
>
> Use a service tool inch pound torque wrench, Part Number 3376592.
>
> **Момент затяжки · Torque Value**
> 0.68 n•m [6 in-lb]
>
> For the feel method, use a screwdriver to turn the adjusting screw **only** until the lever touches the feeler gauge.
>
> The adjusting screw **must not** turn when the lock nut is tightened.
>
> Tighten the locknut using torque wrench service tool, Part Number ST-699, with adapter (1).
>
> **Момент затяжки · Torque Value**
> 45 n•m [35 ft-lb]
>
> Tighten the locknut using torque wrench service tool, Part Number ST-699, without adapter.
>
> **Момент затяжки · Torque Value**
> 60 n•m [45 ft-lb]
>
> The feeler gauge **must** slide backward and forward with **only** a slight drag.
>
> Attempt to insert a feeler gauge that is 0.03 mm \[0.001 in\] thicker. The valve lash is **not** correct when the thicker gauge will fit.
>
> Repeat the adjustment process until the clearance is correct on both the intake and the exhaust valves on the cylinder being adjusted.
>
> After the overhead has been completely set, check the adjusting screw protrusion. Approximately three threads **must** be exposed above the locknut.
>
> If **not**, look for:
>
> - Overhead set improperly
> - Rocker housing gasket missing
> - Push rod **not** in socket
> - Cam follower socket **not** seated properly
> - Nut thickness **not** standard
> - Crosshead off valve stem.
>
> ### Install
>
> Install each rocker cover gasket, rocker lever cover, and capscrews.
>
> Tighten all the capscrews.
>
> **Момент затяжки · Torque Value**
> 5 n•m [44 in-lb]
>
> **Note · Примечание**
> For industrial engines, the number 1 rocker cover on both the right and left banks has a hole for a grommet, for the NBF injector lead.
>
> Install the rubber grommet from the rocker lever cover.
>
> Pass the NBF lead connector through the hole in the rocker lever cover.
>
> Connect the NBF injector lead from the wiring harness.
