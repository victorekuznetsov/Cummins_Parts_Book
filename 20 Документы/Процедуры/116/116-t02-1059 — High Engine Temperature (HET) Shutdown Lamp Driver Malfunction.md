...
pdf: "https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/claude/cummins-parts-knowledge-base-qa0n50/bulletins/procedures/116-t02-1059.pdf"
tags:
  - "документ/процедура"
  - "двигатель/QSK19"
  - "группа/116"
...
> **Источник:** [QuickServe](https://quickserve.cummins.com/qs3/pubsys2/xml/en/procedures/116/116-t02-1059.html) · [PDF-оригинал](https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/claude/cummins-parts-knowledge-base-qa0n50/bulletins/procedures/116-t02-1059.pdf)
...
- Лампа выключения с высокой температурой двигателя (HET) освещается, когда условие выключения с высокой температурой двигателя ** не существует.

- Лампа выключения с высокой температурой двигателя (HET) ** не** освещается при наличии предварительного состояния с высокой температурой двигателя.

### Как пользоваться этим деревом

Это дерево симптомов может быть использовано для устранения неисправности водителя лампы с высокой температурой (HET). Начните с шага 1 поиска неисправностей. На шаге 2 система задаст ряд вопросов и по симптому выдаст перечень действий по поиску неисправности.
...
|  | **STEP 1A.** Проверьте высокую температуру двигателя (HET) | Менее 10 Ом? |
|  | **STEP 1B.** Проверьте высокую температуру двигателя (HET) сигнал отключения для провода к проводу короткий | Менее 10 Ом? |
|  | **STEP 1C.** Проверьте сигнальную проводку с высокой температурой двигателя для короткой посадки | Менее 10 Ом? |
| ШАГ 2. | Проверьте электропроводку двигателя на кабель клиентского интерфейса |  |
|  | **STEP 2A.** Проверьте высокую температуру двигателя (HET) | Менее 10 Ом? |
|  | **STEP 2B.** Проверьте высокую температуру двигателя (HET) сигнал отключения для провода к проводу короткий | Менее 10 Ом? |

### ШАГ 1. Проверка проводки интерфейсной коробки заказчика

#### ШАГ 1A. Проверьте высокую температуру двигателя (HET) выключение сигнала провода для открытого
...
| Проверьте сигнальный провод высокой температуры двигателя (HET) для открытого. Поместите один испытательный щуп на контакт сигнала выключения высокой температуры двигателя (HET) в разъеме C2. Поместите другой испытательный щуп на сигнальный терминал с высокой температурой выключения двигателя (HET) на разъем X4. | Менее 10 Ом? *Да** | 1В |
| Менее 10 Ом? **NORepair:** Заменить провод. См. процедуру[[116-015-023 — Customer Interface Box\|015-023]]. | Ремонт завершён |  |

#### ШАГ 1B. Проверьте высокую температуру двигателя (HET) выключение сигнала провода для провода к проводу короткий

| **Условия: ** Откройте окно клиентского интерфейса Отключить окно клиентского интерфейса к проводах двигателя, разъёму кабеля C2 от окна клиентского интерфейса. |  |  |
|---|---|---|
| **Действие** | ** Спецификация/ремонт** | ** Следующий шаг** |
| Проверьте сигнал отключения высокой температуры двигателя (HET) для провода, чтобы провод был коротким. Поместите один испытательный щуп на контакт сигнала выключения высокой температуры двигателя (HET) в разъеме C2. Поместите другой испытательный щуп на каждый из оставшихся терминалов в разъем X4. | Менее 10 Ом? *** Заменить провод. См. процедуру[[116-015-023 — Customer Interface Box\|015-023]]. | Ремонт завершён |
| Менее 10 Ом? ** НЕТ** | 1С |  |

#### ШАГ 1C. Проверьте высокую температуру двигателя (HET) выключающую сигнальную проводку для короткой посадки

| **Условия: ** Откройте окно клиентского интерфейса Отключить окно клиентского интерфейса к проводах двигателя, разъёму кабеля C2 от окна клиентского интерфейса. |  |  |
|---|---|---|
| **Действие** | ** Спецификация/ремонт** | ** Следующий шаг** |
| Проверьте сигнальный провод высокой температуры двигателя (HET) для короткого заземления. Поместите один испытательный щуп на контакт сигнала выключения высокой температуры двигателя (HET) в разъеме C2. Поместите другой испытательный щуп на панельную площадку. | Менее 10 Ом? *** Заменить провод. См. процедуру[[116-015-023 — Customer Interface Box\|015-023]]. | Ремонт завершён |
| Менее 10 Ом? ** НЕТ** | 2А |  |

### ШАГ 2. Проверьте электропроводку двигателя на кабель клиентского интерфейса

#### ШАГ 2A. Проверьте высокую температуру двигателя (HET) выключение сигнала провода для открытого
