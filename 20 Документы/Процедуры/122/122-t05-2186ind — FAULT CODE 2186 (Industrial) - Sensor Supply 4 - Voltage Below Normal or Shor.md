...
|  | **STEP 3I.** Проверьте реакцию цепи. | Код 2186 активен? |
...
|  | **STEP 4J.** Проверьте реакцию цепи. | Код 2186 активен? |
...
|---|---|---|
| **Действие** | ** Спецификация/ремонт** | ** Следующий шаг** |
| Проверьте штифты датчика и разъем для подключения проводов двигателя на предмет повреждения. См. процедуру 019-361 в разделе 19. Используйте инструмент электронного сервиса INSITETM для считывания кодов неисправностей | Код 2186 активен? *Да** | 3II |
| Код 2186 активен? **NORepair:** Заменить датчик давления подачи топлива. См. процедуру 019-398 в Таблице ассоциированных процедур. | 6А |  |

#### ШАГ 3I. Проверьте отклик цепи.

| **Условия:** Выключите замок зажигания. Отсоедините датчик давления масла предварительного фильтра от электропроводки двигателя. Включите зажигание. Подключите инструмент электронного сервиса INSITETM. |  |  |
|---|---|---|
| **Действие** | ** Спецификация/ремонт** | ** Следующий шаг** |
| Проверьте штифты датчика и разъем для подключения проводов двигателя на предмет повреждения. См. процедуру 019-361 в разделе 19. Используйте инструмент электронного сервиса INSITETM для считывания кодов неисправностей | Код 2186 активен? *Да** | 5а |
| Код 2186 активен? **NORepair:** Заменить датчик давления масла перед фильтром. См. процедуру 019-126 в Таблице ассоциированных процедур. | 6А |  |
...
|---|---|---|
| **Действие** | ** Спецификация/ремонт** | ** Следующий шаг** |
| Проверьте штифты датчика и разъем для подключения проводов двигателя на предмет повреждения. См. процедуру 019-361 в разделе 19. Используйте инструмент электронного сервиса INSITETM для считывания кодов неисправностей | Код 2186 активен? *Да** | 4II |
| Код 2186 активен? **NORepair:** Заменить датчик давления подачи топлива. См. процедуру 019-398 в Таблице ассоциированных процедур. | 6А |  |

#### ШАГ 4I. Проверьте отклик цепи.

| **Условия:** Выключите замок зажигания. Отсоедините датчик давления масла предварительного фильтра от электропроводки двигателя. Включите зажигание. Подключите инструмент электронного сервиса INSITETM. |  |  |
|---|---|---|
| **Действие** | ** Спецификация/ремонт** | ** Следующий шаг** |
| Проверьте штифты датчика и разъем для подключения проводов двигателя на предмет повреждения. См. процедуру 019-361 в разделе 19. Используйте инструмент электронного сервиса INSITETM для считывания кодов неисправностей | Код 2186 активен? *Да** | 4J |
| Код 2186 активен? **NORepair:** Заменить датчик давления масла перед фильтром. См. процедуру 019-126 в Таблице ассоциированных процедур. | 6А |  |

#### ШАГ 4J. Проверьте отклик цепи.

| **Условия:** Выключите замок зажигания. Отсоедините датчик давления масла после фильтра от электропроводки двигателя. Включите зажигание. Подключите инструмент электронного сервиса INSITETM. |  |  |
|---|---|---|
| **Действие** | ** Спецификация/ремонт** | ** Следующий шаг** |
| Проверьте штифты датчика и разъем для подключения проводов двигателя на предмет повреждения. См. процедуру 019-361 в разделе 19. Используйте инструмент электронного сервиса INSITETM для считывания кодов неисправностей | Код 2186 активен? *Да** | 5а |
| Код 2186 активен? **NORepair:** Заменить датчик давления масла после фильтрации. См. процедуру 019-447 в Таблице ассоциированных процедур. | 6А |  |

### ШАГ 5. Проверьте ECM и электропроводку двигателя.

#### ШАГ 5A. Проверьте короткое замыкание контакт-контакт в ремне электропроводки двигателя.

| **Условия:** Выключите замок зажигания. Отсоедините жгут электропроводки двигателя от разъема ECM. Отсоедините датчик скорости распредвала от электропроводки двигателя. Отсоедините датчик давления топлива в рельсах от электропроводки двигателя. Отсоедините датчик температуры и давления впускного коллектора от электропроводки двигателя. Отсоедините датчик давления картерного ящика от электропроводки двигателя. Отсоедините датчик давления масла двигателя от электропроводки двигателя. Отсоедините датчик давления окружающего воздуха от электропроводки двигателя. Отсоедините датчик давления охлаждающей жидкости двигателя от электропроводки двигателя. Отсоедините датчик давления масла предварительного фильтра от электропроводки двигателя. Отсоедините датчик давления после масляного фильтра от электропроводки двигателя. |  |  |
|---|---|---|
| **Действие** | ** Спецификация/ремонт** | ** Следующий шаг** |
...
| Больше 100 тысяч ом? **NORepair:** Ремонтировать или заменить проводку OEM. См. процедуру 019-043 в Таблице ассоциированных процедур. | 6А |  |
...
| Электронный блок управления двигателем | См. процедуру 019-031 | QSK38, QSK50 и QSK60 CM2150 | [[4022102 — QSK38, QSK50, and QSK60 CM2150 Electronic Control System Troubleshooting and Repair M\|4022102]] |
...
| Датчик давления масла до фильтра | См. процедуру 019-126 | QSK38, QSK50 и QSK60 CM2150 | [[4022102 — QSK38, QSK50, and QSK60 CM2150 Electronic Control System Troubleshooting and Repair M\|4022102]] |
...
| Датчик давления после фильтрации масла | См. процедуру 019-447 | QSK38, QSK50 и QSK60 CM2150 | [[4022102 — QSK38, QSK50, and QSK60 CM2150 Electronic Control System Troubleshooting and Repair M\|4022102]] |
