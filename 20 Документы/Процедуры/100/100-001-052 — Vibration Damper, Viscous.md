---
aliases:
  - "Вязкостный гаситель крутильных колебаний"
type: "Процедура"
doc: "100-001-052"
title_en: "Vibration Damper, Viscous"
title_ru: "Вязкостный гаситель крутильных колебаний"
modified: "2017-03-23"
engines:
  - "93058669"
families:
  - "C8.3 · 6C8.3"
manuals:
  - "3666003"
figures: 8
lang: "ru+en"
translation: "машинный черновик"
source: "https://quickserve.cummins.com/qs3/pubsys2/xml/en/procedures/100/100-001-052.html"
pdf: "https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/claude/cummins-parts-knowledge-base-qa0n50/bulletins/procedures/100-001-052.pdf"
tags:
  - "документ/процедура"
  - "двигатель/C8.3"
  - "группа/100"
  - "перевод/машинный"
---

# Vibration Damper, Viscous
**Вязкостный гаситель крутильных колебаний**

> [!abstract] Процедура · `100-001-052`
> **Двигатели:** [[93058669 — 6C8.3 CPL 3105|93058669]]
> **Семейство:** C8.3 · 6C8.3
> **Входит в руководства:** [[3666003 — C Troubleshooting and Repair Manual|3666003]]
> **Секции:** Section 1 - Cylinder Block - Group 01
> **Даты:** изменён 2017-03-23
> **Источник:** [QuickServe](https://quickserve.cummins.com/qs3/pubsys2/xml/en/procedures/100/100-001-052.html) · [PDF-оригинал](https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/claude/cummins-parts-knowledge-base-qa0n50/bulletins/procedures/100-001-052.pdf)

> [!info]- Перевод на русский — машинный черновик
> Русский текст получен автоматическим переводом с английского
> с подстановкой отраслевой терминологии Cummins; он не
> проходил редакторскую вычитку.
> **Юридически значим только английский оригинал** — он
> приведён в свёрнутом блоке в конце заметки и в PDF.


### Подготовительные операции

Снимите жгут проводов.[[100-008-002 — Drive Belt, Cooling Fan|См. процедуру 008-002 в разделе 8.]].

![[ck800wa.png]]

### Снятие

> [!note] Примечание
> Кольцо скорости удерживается на месте теми же болтами, что и вибрационный демпфер.

Удалите шесть болтов.

Удалите вибрационный демпфер.

![[01d00206.png]]

### Проверка при повторном использовании

Проверьте монтажную сеть на наличие трещин.

Проверьте кожух на наличие вмятины или поднятых поверхностей.

Заменить демпфер, если повреждение выявлено.

![[da900sd.png]]

> [!note] Примечание
> Вязкий демпфер заполнен силиконовой жидкостью. После многих часов использования силиконовая жидкость может стать толще и расширяться.

Чтобы определить, является ли толщина амортизатора правильной, удалите краску из амортизатора в четырех местах по обе стороны от амортизатора.

Измерить и зафиксировать толщину амортизатора в четырех местах. Измерьте толщину на 80,65 мм \[3,175 дюйма \] снаружи амортизатора.

Заменить демпфер, если его толщина изменяется более чем на 6,35 мм \[0,25 дюйма \].

![[da900tc.png]]

Распылите демпфер с разработчиком проверки спота, типом SKD-S2 или эквивалентом.

![[da800se.png]]

### Установка

> [!note] Примечание
> Выровнять кольцо скорости и вибрационный демпфер с помощью указательного штифта.

Смазать болты чистым моторным маслом.

Установите вибрационный демпфер и скоростное кольцо.

![[01d00206.png]]

Затягивание болтов демпфера вибрации.

Момент затяжки:

![[da9puha.png]]

### Завершающие операции

Установите приводной ремень.[[100-008-002 — Drive Belt, Cooling Fan|См. процедуру 008-002 в разделе 8.]].

** Совет по обслуживанию: ** Если вы испытываете трудности с установкой приводного ремня (ремень кажется слишком коротким), сначала поместите ремень над канавками, а затем, удерживая натяжитель, переведите ремень через шкив водяного насоса.

Управляйте двигателем, чтобы проверить работу вибрационного демпфера и ремня привода.

![[ck800wa.png]]


> [!quote]- Original (English) · английский оригинал
> ### Preparatory Steps
>
> Remove the drive belt. [[100-008-002 — Drive Belt, Cooling Fan|Refer to Procedure 008-002 in Section 8]].
>
> ### Remove
>
> **Note · Примечание**
> The speed ring is held in place by the same capscrews as the vibration damper.
>
> Remove the six capscrews.
>
> Remove the vibration damper.
>
> ### Inspect for Reuse
>
> Check the mounting web for cracks.
>
> Check the housing for dents or raised surfaces.
>
> Replace the damper if damage is identified.
>
> **Note · Примечание**
> The viscous damper is filled with a silicone fluid. After many hours of use, the silicone fluid can become thicker and expand.
>
> To determine if the damper thickness is correct, remove the paint from the damper in four locations on either side of the damper.
>
> Measure and record the thickness of the damper in four places. Measure the thickness at 80.65 mm \[3.175 in\] from the outside of the damper.
>
> Replace the damper if its thickness varies by more than 6.35 mm \[0.25 in\].
>
> Spray the damper with spot check developer, type SKD-S2, or equivalent.
>
> ### Install
>
> **Note · Примечание**
> Align the speed ring and vibration damper with the index pin.
>
> Lubricate the bolts with clean engine oil.
>
> Install the vibration damper and speed ring.
>
> Tighten vibration damper capscrews.
>
> Torque Value:
>
> ### Finishing Steps
>
> Install the drive belt. [[100-008-002 — Drive Belt, Cooling Fan|Refer to Procedure 008-002 in Section 8]].
>
> **Service Tip:** If difficulty is experienced installing the drive belt (the belt seems too short), position the belt over the grooved pulleys first, and then, while holding the tensioner up, slide the belt over the water pump pulley.
>
> Operate the engine to check the vibration damper and drive belt operation.
