---
aliases:
  - "Требования к инструменту"
type: "Процедура"
doc: "18-102-004"
title_en: "Tool Requirements"
title_ru: "Требования к инструменту"
modified: "2006-07-28"
engines:
  - "33210083"
  - "33219033"
  - "33224343"
  - "33239746"
  - "33239899"
  - "37269910"
  - "37280605"
  - "41349633"
  - "41353297"
  - "85017333"
families:
  - "K19"
  - "K38/K50 · QSK38, QSK50, QSK60"
  - "QSK19"
  - "QSK23"
  - "QSK60"
manuals:
  - "3666013"
  - "3666120"
  - "3810497"
  - "4021375"
  - "4021530"
lang: "ru+en"
translation: "машинный черновик"
source: "https://quickserve.cummins.com/qs3/pubsys2/xml/en/procedures/18/18-102-004.html"
pdf: "https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/main/bulletins/procedures/18-102-004.pdf"
tags:
  - "документ/процедура"
  - "двигатель/K19"
  - "двигатель/K38/K50"
  - "двигатель/QSK19"
  - "двигатель/QSK23"
  - "двигатель/QSK60"
  - "группа/18"
  - "перевод/машинный"
---

# Tool Requirements
**Требования к инструменту**

> [!abstract] Процедура · `18-102-004`
> **Двигатели:** [[33210083 — QSK60 CM500 CPL 2699|33210083]], [[33219033 — QSK60 CM500 CPL 2848|33219033]], [[33224343 — QSK60 CM500 CPL 2849|33224343]], [[33239746 — QSK60 CM2150 MCRS CPL 3451|33239746]], [[33239899 — QSK50 CM2150 MCRS CPL 3379|33239899]], [[37269910 — K19 CPL 1253|37269910]], [[37280605 — K19 CPL 447|37280605]], [[41349633 — QSK19 CM2150 MCRS CPL 3666|41349633]], [[41353297 — QSK19 CM2150 MCRS CPL 3666|41353297]], [[85017333 — QSK23 CM500 CPL 2858|85017333]]
> **Семейство:** K19, K38/K50 · QSK38, QSK50, QSK60, QSK19, QSK23, QSK60
> **Входит в руководства:** [[3666013 — K19 Industrial and Marine Operation and Maintenance Manual|3666013]], [[3666120 — QSK19, QSK19 CM850 MCRS, and QSK19 CM2150 MCRS Operation and Maintenance Manual|3666120]], [[3810497 — K38, K50, QSK38 and QSK50 Operation and Maintenance Manual|3810497]], [[4021375 — QSK23 Troubleshooting and Repair Manual|4021375]], [[4021530 — QSK45 and QSK60 Service Manual|4021530]]
> **Секции:** Section 2 - Maintenance Guidelines
> **Даты:** изменён 2006-07-28
> **Источник:** [QuickServe](https://quickserve.cummins.com/qs3/pubsys2/xml/en/procedures/18/18-102-004.html) · [PDF-оригинал](https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/main/bulletins/procedures/18-102-004.pdf)

> [!info]- Перевод на русский — машинный черновик
> Русский текст получен автоматическим переводом с английского
> с подстановкой отраслевой терминологии Cummins; он не
> проходил редакторскую вычитку.
> **Юридически значим только английский оригинал** — он
> приведён в свёрнутом блоке в конце заметки и в PDF.


### Общие сведения

Большинство операций по техническому обслуживанию, описанных в этом руководстве, могут выполняться с помощью обычной ручной оснастки (драншеи, розетки и отвертки).

Ниже приведен список специальных инструментов обслуживания, необходимых для некоторых операций по техническому обслуживанию:

| Инструмент Часть Номер | Наименование |
|---|---|
| 3375049 | Масляной фильтр Wrench |
| 3376592 | Момент-ренч (Inch Pound) |
| 3376807 | Вода/фильтр топлива |
| 3822524 | Измеритель напряжения пояса (Click-Type) |
| 3822525 | Измеритель напряжения пояса (Click-Type) |
| 3822648 | Инструмент настройки Top Stop Tappet (только для двигателей, оборудованных STC) |
| 3823348 | Инструмент настройки Top Stop Tappet (только для двигателей, оборудованных STC) |
| СТ-1293 | Измеритель напряжения пояса (V-пояса) |
| 3823138 | Измеритель напряжения пояса (Poly V-belt) |

Смотрите соответствующие разделы для описания инструментов и как их использовать.

Свяжитесь с ближайшим авторизованным ремонтным центром Cummins для получения необходимых инструментов обслуживания.


> [!quote]- Original (English) · английский оригинал
> ### General Information
>
> Most of the maintenance operations described in this manual can be performed with common hand tools (wrenches, sockets, and screwdrivers).
>
> The following is a list of special service tools required for some maintenance operations:
>
> | Tool Part Number | Description |
> |---|---|
> | 3375049 | Oil Filter Wrench |
> | 3376592 | Torque Wrench (Inch Pound) |
> | 3376807 | Water/Fuel Filter Wrench |
> | 3822524 | Belt Tension Gauge (Click-Type) |
> | 3822525 | Belt Tension Gauge (Click-Type) |
> | 3822648 | Top Stop Tappet Setting Tool (STC equipped engines **only**) |
> | 3823348 | Top Stop Tappet Setting Tool (STC equipped engines **only**) |
> | ST-1293 | Belt Tension Gauge (V-belts) |
> | 3823138 | Belt Tension Gauge (Poly V-belt) |
>
> Refer to the appropriate sections for a description of the tools and how to use them.
>
> Contact the nearest Cummins Authorized Repair Location for the required service tools.
