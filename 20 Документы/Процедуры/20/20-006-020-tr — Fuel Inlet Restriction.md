---
aliases:
  - "Сопротивление на входе топлива"
type: "Процедура"
doc: "20-006-020-tr"
title_en: "Fuel Inlet Restriction"
title_ru: "Сопротивление на входе топлива"
modified: "2014-01-08"
engines:
  - "41349633"
  - "41353297"
families:
  - "QSK19"
manuals:
  - "4021592"
figures: 37
lang: "ru+en"
translation: "машинный черновик"
source: "https://quickserve.cummins.com/qs3/pubsys2/xml/en/procedures/20/20-006-020-tr.html"
pdf: "https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/main/bulletins/procedures/20-006-020-tr.pdf"
tags:
  - "документ/процедура"
  - "двигатель/QSK19"
  - "группа/20"
  - "перевод/машинный"
---

# Fuel Inlet Restriction
**Сопротивление на входе топлива**

> [!abstract] Процедура · `20-006-020-tr`
> **Двигатели:** [[41349633 — QSK19 CM2150 MCRS CPL 3666|41349633]], [[41353297 — QSK19 CM2150 MCRS CPL 3666|41353297]]
> **Семейство:** QSK19
> **Входит в руководства:** [[4021592 — QSK19, QSK19 CM850 MCRS, and QSK19 CM2150 MCRS Service Manual|4021592]]
> **Секции:** Section 6 - Injectors and Fuel Lines - Group 06
> **Даты:** изменён 2014-01-08
> **Источник:** [QuickServe](https://quickserve.cummins.com/qs3/pubsys2/xml/en/procedures/20/20-006-020-tr.html) · [PDF-оригинал](https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/main/bulletins/procedures/20-006-020-tr.pdf)

> [!info]- Перевод на русский — машинный черновик
> Русский текст получен автоматическим переводом с английского
> с подстановкой отраслевой терминологии Cummins; он не
> проходил редакторскую вычитку.
> **Юридически значим только английский оригинал** — он
> приведён в свёрнутом блоке в конце заметки и в PDF.


### Первичная проверка

с форсункой электронного управления

> [!danger] ОПАСНО
> В зависимости от условий топливо огнеопасно. Держите все сигареты, пламя, пилотные огни, дуговое оборудование и выключатели из рабочей зоны и областей, разделяющих вентиляцию, чтобы уменьшить вероятность серьезных травм или смерти при работе на топливной системе.

> [!warning] ОСТОРОЖНО
> При отсоединении и снятии топливных магистралей, замене фильтров и прокачке системы не допускайте пролива и слива топлива в трюм. Не бросайте фильтрующие элементы в трюм. Топливо и топливные фильтры утилизируйте по местным природоохранным требованиям.

#### 1 этап фильтры

- Удалите шланг (1) с воздушным кровоточащим клапаном (2).
- Удалите проверяющий клапан (2) с воздушным кровотечением из блока сливного коллектора (3).

![[06400351.png]]

Установите тестер топливной системы, (1), номер детали 4918612, вместо контрольного клапана с воздушным кровотечением.

Установите шланг с воздушным кровотечением.

> [!tip] Момент затяжки
> Тестер топливной системы 55 Н·м [41 фут-лб]

> [!tip] Момент затяжки
> Air Bleed Hose 45 Н·м [33 фут-лб]

![[06400352.png]]

После закрытия клапана подачи топлива, вынуть пробку M14 STOR из впускного коллектора топлива и установить фитинг CompuchekTM, номер детали 3824844 или эквивалент.

![[06400414.png]]

Подключите вакуумный калибр и адаптер, номер 3164491 или эквивалент, и цифровой мультиметр, номер 3164488 или 3164489, или эквивалент, к фитингу CompuchekTM.

![[06400415.png]]

Откройте клапан подачи топлива, и запустите и управляйте двигателем на высоком холостом ходу.

Запишите ограничение входа топлива.

Максимальное ограничение впуска топлива составляет: 254 мм рт.ст. \[10 in-Hg\].

Если ограничение выше спецификаций, измерьте ограничение топливного фильтра 1-й стадии.

![[14400049.png]]

Установите тестер топливной системы, (1), номер детали 4918612, вместо контрольного клапана с воздушным кровотечением.

Установите шланг с воздушным кровотечением.

> [!tip] Момент затяжки
> Тестер топливной системы 55 Н·м [41 фут-лб]

> [!tip] Момент затяжки
> Air Bleed Hose 45 Н·м [33 фут-лб]

![[06400352.png]]

Удалите вакуумный калибр и адаптер из блока впускного коллектора топлива.

![[06400415.png]]

Удалите установку CompuchekTM и установите резьбовую кольцевую вилку в порт в блоке впускного коллектора топлива.

> [!tip] Момент затяжки
> Проточенный O-кольцевой Plug 29 Н·м [21 фут-лб]

![[06400414.png]]

### Проверка

с форсункой механического управления

> [!danger] ОПАСНО
> В зависимости от условий топливо огнеопасно. Держите все сигареты, пламя, пилотные огни, дуговое оборудование и выключатели из рабочей зоны и областей, разделяющих вентиляцию, чтобы уменьшить вероятность серьезных травм или смерти при работе на топливной системе.

> [!warning] ОСТОРОЖНО
> При отсоединении и снятии топливных магистралей, замене фильтров и прокачке системы не допускайте пролива и слива топлива в трюм. Не бросайте фильтрующие элементы в трюм. Топливо и топливные фильтры утилизируйте по местным природоохранным требованиям.

Топливный насос должен находиться в положении «полное топливо», чтобы точно измерить ограничение топливной линии.

![[06600218.png]]

Установите линию, содержащую 7 мм \[0,25 дюйма \] игольчатого клапана между клапаном отключения топливного насоса и топливным баком.

![[06600219.png]]

Удалите линию подачи воздуха / топлива для управления воздухом из коллектора воздухозаборника. Установите пробку или колпачок в отверстие воздушного коллектора.

![[06600220.png]]

Установите насос под давлением, часть 3375515, или эквивалент.

Если насос давления не доступен, установите регулируемый шланг давления воздуха с запорным клапаном на линию.

Применить давление воздуха 170 кПа[25 psi] к воздухопроводу/проводу подачи воздуха для управления топливом.

![[06600220.png]]

Удалите шланг подачи топлива на передаточный насос или вилку с задней части топливного насоса, установленного на топливный фильтр.

Установите адаптер как можно ближе к топливному насосу. Установите вакуумный калибр, номер детали ST-434, или эквивалент, на адаптер. Минимальная мощность калибра должна быть 760 мм рт.ст. \[30 in-Hg\].

> [!note] Примечание
> Вакуумный калибр, номер детали ST434, содержит калибр, шланг и адаптер.

![[06600222.png]]

При установке клапана, работайте с двигателем на высоком холостом ходу. Медленно открывайте игловый клапан, пока обороты двигателя не упадут на 100 оборотов в минуту. Это позиция «полного топлива».

![[oi8vava.png]]

Держите калибр на том же уровне, что и зубчатый насос.

> [!note] Примечание
> Калибр будет **не** измерять правильный вакуум, если он **не** удерживается на том же уровне, что и передаточный насос.

Работайте с двигателем в положении «Полное топливо».

Наблюдайте за показаниями на измерительной шкале.

Максимальное ограничение впуска топлива заключается в следующем:

| Ограничение фильтра (чистый) |  |  |
|---|---|---|
| хг |  | в хг |
| 100 | Макс | 4 |

| Ограничение фильтрации (грязный) |  |  |
|---|---|---|
| хг |  | в хг |
| 200 | Макс | 8 |

Исправьте ограничение или замените топливный фильтр. См. процедуру 006-015 в разделе 6.

![[fp8gacd.png]]

### Измерение

с форсункой электронного управления

> [!danger] ОПАСНО
> В зависимости от условий топливо огнеопасно. Держите все сигареты, пламя, пилотные огни, дуговое оборудование и выключатели из рабочей зоны и областей, разделяющих вентиляцию, чтобы уменьшить вероятность серьезных травм или смерти при работе на топливной системе.

> [!warning] ОСТОРОЖНО
> При отсоединении и снятии топливных магистралей, замене фильтров и прокачке системы не допускайте пролива и слива топлива в трюм. Не бросайте фильтрующие элементы в трюм. Топливо и топливные фильтры утилизируйте по местным природоохранным требованиям.

#### 1 этап фильтры

- Удалите шланг (1) с воздушным кровоточащим клапаном (2).
- Удалите проверяющий клапан (2) с воздушным кровотечением из блока сливного коллектора (3).

![[06400351.png]]

Установите тестер топливной системы (1), номер детали 4918612, вместо проверочного клапана с воздушным кровотечением.

Установите шланг с воздушным кровотечением.

> [!tip] Момент затяжки
> Тестер топливной системы 55 Н·м [41 фут-лб]

> [!tip] Момент затяжки
> Air Bleed Hose 45 Н·м [33 фут-лб]

![[06400352.png]]

Промышленное применение

Удалите резьбовую головку с шестигранным кольцом (1) во входе и резьбовую головку с шестигранным кольцом (2) в выходе коллекторов головки фильтра Стадии 1 и замените их фитингами CompuchekTM.

![[06400408.png]]

Подключите вакуумный калибр и адаптер к установке CompuchekTM во входном порту.

Запуск и эксплуатация двигателя на высоком холостом ходу.

Запишите ограничение входа на 1-й этап.

Максимальное ограничение впуска топлива составляет 14 кПа[4 in-Hg].

Если ограничение выше спецификаций, проверьте топливные линии. См. руководство по эксплуатации изготовителя оригинального оборудования (OEM), чтобы определить источник высокого ограничения.

![[06400410.png]]

Удалите вакуумный калибр и адаптер из входного порта и установите его на компучеке, установленном в выходном порту.

Запуск и эксплуатация двигателя на высоком холостом ходу.

![[06400409.png]]

Запишите ограничение выхода 1-й стадии.

Вычтите измерение, полученное на входе стадии 1, из измерения, полученного на выходе стадии 1. Это ограничение 1-й стадии.

Пример:

- Ограничение входа 1 стадии составляет 3,4 кПа[1 in-Hg]
- Ограничение выхода 1 стадии составляет 15,2 кПа[4,5 in-Hg]

Ограничение стадии 1 составляет 15,2 кПа \[4,5 in-Hg\] минус 3,4 кПа \[1 in-Hg\] равно 11,8 кПа \[3,5 in-Hg\].

| 1 стадия ограничения фильтра |  |  |
|---|---|---|
| каша |  | в хг |
| 27.1 | Макс | 8 |

Если ограничение выше технических требований, замените топливные фильтры 1-й ступени.[[20-006-015-tr — Fuel Filter (Spin-On Type)|См. процедуру 006-015 в разделе 6.]]

Если ограничение ниже максимальных спецификаций, проверьте топливные линии. См. руководство по обслуживанию OEM для определения источника высокого ограничения.

![[14400049.png]]

Удалите вакуумный калибр и адаптер с установочной головки топливного фильтра 1-й ступени.

![[06400409.png]]

Удалите фитинги CompuchekTM и установите резьбовые шестиглавые кольцевые вилки во входной порт (1) и выход (2) в головке фильтра.

> [!tip] Момент затяжки
> Проточенные шестеренки Hex Head O-ring Plugs 27 Н·м [239 фунт-дюйм]

![[06400408.png]]

Судовое применение

Удалите резьбовую головку с шестигранным кольцом (1) во входе и резьбовую головку с шестигранным кольцом (2) в выходе коллекторов головки фильтра Стадии 1 и замените их фитингами CompuchekTM.

![[06600475.png]]

Подключите вакуумный калибр и адаптер к установке CompuchekTM во входном порту.

Запуск и эксплуатация двигателя на высоком холостом ходу.

Запишите ограничение входа на 1-й этап.

Максимальное ограничение впуска топлива составляет 14 кПа[4 in-Hg].

Если ограничение выше спецификаций, проверьте топливные линии. См. руководство по обслуживанию OEM для определения источника высокого ограничения.

![[06400457.png]]

Удалите вакуумный калибр и адаптер из входного порта и установите его на компучеке, установленном в выходном порту.

Запуск и эксплуатация двигателя на высоком холостом ходу.

![[06600476.png]]

Запишите ограничение выхода 1-й стадии.

Вычтите измерение, полученное на входе стадии 1, из измерения, полученного на выходе стадии 1. Это ограничение 1-й стадии.

Пример:

- Ограничение входа 1 стадии составляет 3,4 кПа[1 in-Hg]
- Ограничение выхода 1 стадии составляет 15,2 кПа[4,5 in-Hg]

Ограничение стадии 1 составляет 15,2 кПа \[4,5 in-Hg\] минус 3,4 кПа \[1 in-Hg\] равно 11,8 кПа \[3,5 in-Hg\].

| 1 стадия ограничения фильтра |  |  |
|---|---|---|
| каша |  | в хг |
| 27.1 | Макс | 8 |

Если ограничение выше технических требований, замените топливные фильтры 1-й ступени.[[20-006-015-tr — Fuel Filter (Spin-On Type)|См. процедуру 006-015 в разделе 6.]]

Если ограничение ниже максимальных спецификаций, проверьте топливные линии. См. руководство по обслуживанию OEM для определения источника высокого ограничения.

![[14400049.png]]

Удалите вакуумный калибр и адаптер с установочной головки топливного фильтра 1-й ступени.

![[06600476.png]]

Удалите фитинги CompuchekTM и установите резьбовые шестиглавые кольцевые вилки во входной порт (1) и выход (2) в головке фильтра.

> [!tip] Момент затяжки
> Проточенные шестеренки Hex Head O-ring Plugs 27 Н·м [239 фунт-дюйм]

![[06600475.png]]

Удалите шланг с воздушным кровотечением и тестер топливной системы (1) из блока коллектора слива топлива.

![[06400352.png]]

Установите оригинальный проверяющий клапан (2) с воздушным кровотечением в блок сливного коллектора (3).

> [!tip] Момент затяжки
> Air Bleed Check клапан 55 Н·м [41 фут-лб]

Установите шланг с воздушным кровотечением на контрольный клапан с воздушным кровотечением.

> [!tip] Момент затяжки
> Air Bleed Hose 45 Н·м [33 фут-лб]

![[06400351.png]]

#### Этап 2

- Удалите датчик давления на входе на 2-й стадии фильтра (1). См. процедуру 019-119 в разделе 19 в руководстве по устранению неполадок и ремонту системы электронного управления QSK19 (CM850 Modular Common Rail System) Bulletin 4021493.
- Удалите датчик температуры головки 2 фильтра 2. См. процедуру 019-398 в руководстве по устранению неполадок и ремонту системы электронного управления QSK19 (CM850 Modular Common Rail System) Bulletin 4021493.
- Установите фитинги CompchekTM в порты датчиков давления и температуры на 2-й стадии фильтра.

![[06400411.png]]

Подключите датчик измерения давления и адаптер к CompuchekTM, установленному на стороне датчика давления головки фильтра.

Запуск и эксплуатация двигателя на высоком холостом ходу.

Зафиксируйте давление на входе 2-й стадии.

![[06400412.png]]

Удалите калибр и адаптер давления со стороны датчика давления и установите калибр и адаптер давления на фитинг на стороне датчика температуры головки фильтра.

Запуск и эксплуатация двигателя на высоком холостом ходу.

![[06400413.png]]

Зафиксируйте давление на выходе 2-й стадии.

Вычтите измерение, полученное на выходе 2-й стадии, из измерения, полученного на входе 2-й стадии. Это ограничение фильтра 2-й стадии.

Пример:

- Давление на входе 2 стадии составляет 731,5 кПа[104,5 psi]
- Давление выхода 2 стадии составляет 714,0 кПа[102,0 psi]

Ограничение 2 стадии составляет 728,0 кПа \[104,5 psi\] минус 714,0 кПа \[102,0 psi\] равно 17,5 кПа \[2,5 psi\].

| 2 стадия ограничения фильтра |  |  |
|---|---|---|
| каша |  | пси |
| 300 | Макс | 43.5 |

Если ограничение выше технических требований, замените топливные фильтры 2-го этапа.[[20-006-015-tr — Fuel Filter (Spin-On Type)|См. процедуру 006-015 в разделе 6.]]

![[14400049.png]]

Удалите калибр и адаптер давления из головки фильтра второй ступени.

![[06400413.png]]

Удалите фитинги CompuchekTM из головки фильтра второй ступени.

Установите датчик (1) давления в головку фильтра. См. процедуру 019-119 в разделе 19 в руководстве по устранению неполадок и ремонту системы электронного управления QSK19 (CM850 Modular Common Rail System) Bulletin 4021493.

Установите датчик (2) температуры в головку фильтра. См. процедуру 019-398 в руководстве по устранению неполадок и ремонту системы электронного управления QSK19 (CM850 Modular Common Rail System) Bulletin 4021493.

![[06400411.png]]


> [!quote]- Original (English) · английский оригинал
> ### Initial Check
>
> with Electronically Actuated Injector
>
> **WARNING · Опасно**
> Depending on the circumstance, fuel is flammable. Keep all cigarettes, flames, pilot lights, arcing equipment, and switches out of the work area and areas sharing ventilation to reduce the possibility of severe personal injury or death when working on the fuel system.
>
> **CAUTION · Осторожно**
> Do not spill or drain fuel into the bilge area when disconnecting or removing fuel lines, replacing filters, and priming the fuel system. Do not drop or throw filter elements into the bilge area. The fuel and fuel filters must be disposed of in accordance with local environmental regulations.
>
> #### Stage 1 Filters
>
> - Remove the air bleed hose (1) from the air bleed check valve (2).
> - Remove the air bleed check valve (2) from the drain manifold block (3).
>
> Install the fuel system tester, (1), Part Number 4918612, in place of the air bleed check valve.
>
> Install the air bleed hose.
>
> **Момент затяжки · Torque Value**
> Fuel System Tester 55 n•m [41 ft-lb]
>
> **Момент затяжки · Torque Value**
> Air Bleed Hose 45 n•m [33 ft-lb]
>
> With the fuel supply valve closed, remove the M14 STOR plug from the fuel inlet manifold and install a Compuchek™ fitting, Part Number 3824844, or equivalent.
>
> Connect the vacuum gauge and adapter, Part Number 3164491, or equivalent, and digital multimeter, Part Number 3164488 or 3164489, or equivalent, to the Compuchek™ fitting.
>
> Open the fuel supply valve, and start and operate the engine at high idle.
>
> Record the fuel inlet restriction.
>
> The fuel inlet restriction maximum is: 254 mm-Hg \[10 in-Hg\].
>
> If the restriction is above specifications, measure the Stage 1 fuel filter restriction.
>
> Install the fuel system tester, (1), Part Number 4918612, in place of the air bleed check valve.
>
> Install the air bleed hose.
>
> **Момент затяжки · Torque Value**
> Fuel System Tester 55 n•m [41 ft-lb]
>
> **Момент затяжки · Torque Value**
> Air Bleed Hose 45 n•m [33 ft-lb]
>
> Remove the vacuum gauge and adapter from the fuel inlet manifold block.
>
> Remove the Compuchek™ fitting and install the threaded o-ring plug into the port in the fuel inlet manifold block.
>
> **Момент затяжки · Torque Value**
> Threaded O-ring Plug 29 n•m [21 ft-lb]
>
> ### Test
>
> with Mechanically Actuated Injector
>
> **WARNING · Опасно**
> Depending on the circumstance, fuel is flammable. Keep all cigarettes, flames, pilot lights, arcing equipment, and switches out of the work area and areas sharing ventilation to reduce the possibility of severe personal injury or death when working on the fuel system.
>
> **CAUTION · Осторожно**
> Do not spill or drain fuel into the bilge area when disconnecting or removing fuel lines, replacing filters, and priming the fuel system. Do not drop or throw filter elements into the bilge area. The fuel and fuel filters must be disposed of in accordance with local environmental regulations.
>
> The fuel pump **must** be in the "Full Fuel" position to accurately measure fuel line restriction.
>
> Install a line containing a 7 mm \[0.25 in\] needle valve between the fuel pump shutoff valve and the fuel tank.
>
> Remove the air/fuel control air supply line from the air intake manifold. Install a plug or a cap in the air manifold hole.
>
> Install the pressure pump, Part Number 3375515, or equivalent.
>
> If the pressure pump is **not** available, install a regulated air pressure hose, with a shutoff valve, to the line.
>
> Apply 170 kPa \[25 psi\] air pressure to the air/fuel control air supply line.
>
> Remove the fuel supply hose to the gear pump or the plug from the rear of the fuel pump mounted to the fuel filter.
>
> Install the adapter as close to the fuel pump as possible. Install a vacuum gauge, Part Number ST-434, or equivalent, to the adapter. The minimum gauge capacity **must** be 760 mm-Hg \[30 in-Hg\].
>
> **Note · Примечание**
> The vacuum gauge, Part Number ST434, contains the gauge, hose, and adapter.
>
> With the valve installed, operate the engine at high idle. Slowly open the needle valve until the engine rpm drops 100 rpm. This is the "Full Fuel" position.
>
> Hold the gauge at the same level as the gear pump.
>
> **Note · Примечание**
> The gauge will **not** measure the correct vacuum if it is **not** held at the same level as the gear pump.
>
> Operate the engine at the "Full Fuel" position.
>
> Observe the reading on the gauge.
>
> The maximum fuel inlet restriction is as follows:
>
> | Filter Restriction (Clean) |  |  |
> |---|---|---|
> | mm-hg |  | in-hg |
> | 100 | MAX | 4 |
>
> | Filter Restriction (Dirty) |  |  |
> |---|---|---|
> | mm-hg |  | in-hg |
> | 200 | MAX | 8 |
>
> Correct the restriction or replace the fuel filter. [[56-006-015-tr — Fuel Filter (Spin-On Type)|Refer to Procedure 006-015 in Section 6.]]
>
> ### Measure
>
> with Electronically Actuated Injector
>
> **WARNING · Опасно**
> Depending on the circumstance, fuel is flammable. Keep all cigarettes, flames, pilot lights, arcing equipment, and switches out of the work area and areas sharing ventilation to reduce the possibility of severe personal injury or death when working on the fuel system.
>
> **CAUTION · Осторожно**
> Do not spill or drain fuel into the bilge area when disconnecting or removing fuel lines, replacing filters, and priming the fuel system. Do not drop or throw filter elements into the bilge area. The fuel and fuel filters must be disposed of in accordance with local environmental regulations.
>
> #### Stage 1 Filters
>
> - Remove the air bleed hose (1) from the air bleed check valve (2).
> - Remove the air bleed check valve (2) from the drain manifold block (3).
>
> Install the fuel system tester (1), Part Number 4918612, in place of the air bleed check valve.
>
> Install the air bleed hose.
>
> **Момент затяжки · Torque Value**
> Fuel System Tester 55 n•m [41 ft-lb]
>
> **Момент затяжки · Torque Value**
> Air Bleed Hose 45 n•m [33 ft-lb]
>
> Industrial Applications
>
> Remove the threaded hex head o-ring plug (1) in the inlet and the threaded hex head o-ring plug (2) in the outlet of Stage 1 filter head manifolds and replace them with Compuchek™ fittings.
>
> Connect a vacuum gauge and adapter to the Compuchek™ fitting in the inlet port.
>
> Start and operate the engine at high idle.
>
> Record the Stage 1 inlet restriction.
>
> The fuel inlet restriction maximum is 14 kPa \[4 in-Hg\].
>
> If the restriction is above specifications, inspect the fuel lines. Refer to the original equipment manufacturer (OEM) service manual to determine the source of the high restriction.
>
> Remove the vacuum gauge and adapter from the inlet port and install it on the Compuchek™ fitting in the outlet port.
>
> Start and operate the engine at high idle.
>
> Record the Stage 1 outlet restriction.
>
> Subtract the measurement obtained at the Stage 1 inlet from the measurement obtained at the Stage 1 outlet. This is the Stage 1 restriction.
>
> Example:
>
> - Stage 1 inlet restriction is 3.4 kPa \[1 in-Hg\]
> - Stage 1 outlet restriction is 15.2 kPa \[4.5 in-Hg\]
>
> Stage 1 restriction is 15.2 kPa \[4.5 in-Hg\] minus 3.4 kPa \[1 in-Hg\] equals 11.8 kPa \[3.5 in-Hg\].
>
> | Stage 1 Filter Restriction |  |  |
> |---|---|---|
> | kpa |  | in-hg |
> | 27.1 | MAX | 8 |
>
> If the restriction is above specifications, replace the Stage 1 fuel filters. [[20-006-015-tr — Fuel Filter (Spin-On Type)|Refer to Procedure 006-015 in Section 6.]]
>
> If the restriction is below the maximum specifications, inspect the fuel lines. Refer to the OEM service manual to determine the source of the high restriction.
>
> Remove the vacuum gauge and adapter from the Stage 1 fuel filter head.
>
> Remove the Compuchek™ fittings and install the threaded hex head o-ring plugs into the inlet port (1) and outlet (2) in the filter head.
>
> **Момент затяжки · Torque Value**
> Threaded Hex Head O-ring Plugs 27 n•m [239 in-lb]
>
> Marine Applications
>
> Remove the threaded hex head o-ring plug (1) in the inlet and the threaded hex head o-ring plug (2) in the outlet of Stage 1 filter head manifolds and replace them with Compuchek™ fittings.
>
> Connect a vacuum gauge and adapter to the Compuchek™ fitting in the inlet port.
>
> Start and operate the engine at high idle.
>
> Record the Stage 1 inlet restriction.
>
> The fuel inlet restriction maximum is 14 kPa \[4 in-Hg\].
>
> If the restriction is above specifications, inspect the fuel lines. Refer to the OEM service manual to determine the source of the high restriction.
>
> Remove the vacuum gauge and adapter from the inlet port and install it on the Compuchek™ fitting in the outlet port.
>
> Start and operate the engine at high idle.
>
> Record the Stage 1 outlet restriction.
>
> Subtract the measurement obtained at the Stage 1 inlet from the measurement obtained at the Stage 1 outlet. This is the Stage 1 restriction.
>
> Example:
>
> - Stage 1 inlet restriction is 3.4 kPa \[1 in-Hg\]
> - Stage 1 outlet restriction is 15.2 kPa \[4.5 in-Hg\]
>
> Stage 1 restriction is 15.2 kPa \[4.5 in-Hg\] minus 3.4 kPa \[1 in-Hg\] equals 11.8 kPa \[3.5 in-Hg\].
>
> | Stage 1 Filter Restriction |  |  |
> |---|---|---|
> | kpa |  | in-hg |
> | 27.1 | MAX | 8 |
>
> If the restriction is above specifications, replace the Stage 1 fuel filters. [[20-006-015-tr — Fuel Filter (Spin-On Type)|Refer to Procedure 006-015 in Section 6.]]
>
> If the restriction is below the maximum specifications, inspect the fuel lines. Refer to the OEM service manual to determine the source of the high restriction.
>
> Remove the vacuum gauge and adapter from the Stage 1 fuel filter head.
>
> Remove the Compuchek™ fittings and install the threaded hex head o-ring plugs into the inlet port (1) and outlet (2) in the filter head.
>
> **Момент затяжки · Torque Value**
> Threaded Hex Head O-ring Plugs 27 n•m [239 in-lb]
>
> Remove the air bleed hose and fuel system tester (1) from the fuel drain manifold block.
>
> Install the original air bleed check valve (2) into the fuel drain manifold block (3).
>
> **Момент затяжки · Torque Value**
> Air Bleed Check Valve 55 n•m [41 ft-lb]
>
> Install the air bleed hose to the air bleed check valve.
>
> **Момент затяжки · Torque Value**
> Air Bleed Hose 45 n•m [33 ft-lb]
>
> #### Stage 2
>
> - Remove the Stage 2 filter head inlet pressure sensor (1). Refer to Procedure 019-119 in Section 19 in the QSK19 (CM850 Modular Common Rail System) Electronic Control System Troubleshooting and Repair Manual, Bulletin 4021493.
> - Remove the Stage 2 filter head temperature sensor (2). Refer to Procedure 019-398 in the QSK19 (CM850 Modular Common Rail System) Electronic Control System Troubleshooting and Repair Manual, Bulletin 4021493.
> - Install Compchek™ fittings into the Stage 2 filter head pressure and temperature sensor ports.
>
> Connect a pressure gauge and adapter to the Compuchek™ fitting in the pressure sensor side of the filter head.
>
> Start and operate the engine at high idle.
>
> Record the Stage 2 inlet pressure.
>
> Remove the pressure gauge and adapter from the pressure sensor side fitting and install the pressure gauge and adapter on the fitting on the temperature sensor side of the filter head.
>
> Start and operate the engine at high idle.
>
> Record the Stage 2 outlet pressure.
>
> Subtract the measurement obtained at the Stage 2 outlet from the measurement obtained at the Stage 2 inlet. This is the Stage 2 filter restriction.
>
> Example:
>
> - Stage 2 inlet pressure is 731.5 kPa \[104.5 psi\]
> - Stage 2 outlet pressure is 714.0 kPa \[102.0 psi\]
>
> Stage 2 restriction is 728.0 kPa \[104.5 psi\] minus 714.0 kPa \[102.0 psi\] equals 17.5 kPa \[2.5 psi\].
>
> | Stage 2 Filter Restriction Limits |  |  |
> |---|---|---|
> | kpa |  | psi |
> | 300 | MAX | 43.5 |
>
> If the restriction is above specifications, replace the Stage 2 fuel filters. [[20-006-015-tr — Fuel Filter (Spin-On Type)|Refer to Procedure 006-015 in Section 6.]]
>
> Remove the pressure gauge and adapter from the second stage filter head.
>
> Remove the Compuchek™ fittings from the second stage filter head.
>
> Install the pressure sensor (1) into the filter head. Refer to Procedure 019-119 in Section 19 in the QSK19 (CM850 Modular Common Rail System) Electronic Control System Troubleshooting and Repair Manual, Bulletin 4021493.
>
> Install the temperature sensor (2) into the filter head. Refer to Procedure 019-398 in the QSK19 (CM850 Modular Common Rail System) Electronic Control System Troubleshooting and Repair Manual, Bulletin 4021493.
