...
pdf: "https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/claude/cummins-parts-knowledge-base-qa0n50/bulletins/procedures/56-102-002-om.pdf"
tags:
  - "документ/процедура"
  - "двигатель/QST30"
  - "группа/56"
...
> **Источник:** [QuickServe](https://quickserve.cummins.com/qs3/pubsys2/xml/en/procedures/56/56-102-002-om.html) · [PDF-оригинал](https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/claude/cummins-parts-knowledge-base-qa0n50/bulletins/procedures/56-102-002-om.pdf)
...
- Охлаждающий вентилятор - Check
- Анализ нефти (ссылка на следующие бюллетени):
- Автоматический натяжитель ремня (генератор) — проверить
- Фильтр охлаждения - Изменение 4
- Трубка сапуна картера — проверить
- Добавка с добавлением охлаждающей жидкости (SCA) - Check 4
...
### Нефть и фильтры меняют интервал
...
**Не** Расширяйте интервалы изменения масла и фильтра за пределы указанных конфигураций в графике технического обслуживания, если только метод диаграммы не используется для промышленных применений или Сервисного бюллетеня, Расширенной программы интервала обслуживания смазочных масел для приложений набора генераторов в режиме ожидания, Бюллетень 6475874, для резервных приложений, работающих менее 250 часов. Смотрите графики нефтяных интервалов в этом разделе.
...
Когда масло изменяется на двигателе с удаленным нефтяным баком, в котором масло непрерывно циркулирует (** не** макияжный бак CentinelTM), масло в баке ** должно быть изменено в дополнение к маслу в отстойнике двигателя.
...
| JAMA 3 DH-2 |  |  |  |
| API — Американский институт нефти. ACEA - Ассоциация европейских конструкторов. ACEA - Ассоциация европейских конструкторов. JAMA - Японская ассоциация автопроизводителей. JAMA - Японская ассоциация автопроизводителей. CES 20076 добавляет требование 300-часового теста Cummins M11 к API CH-4. CES 20076 добавляет требование 300-часового теста Cummins M11 к API CH-4. CES 20077 добавляет требование 300-часового теста к ACEA E5. CES 20077 добавляет требование 300-часового теста к ACEA E5. Использование масел только с этими обозначениями создает неоправданный риск повреждения двигателя для двигателей, предназначенных для использования более современных масел, даже при значительно сокращенных интервалах изменения масла. Масла CES 20075 CF-4/SH и E-3 могут использоваться в районах, где не имеется ни одного из рекомендованных масел, но интервал слива масла должен быть уменьшен. Ссылка на соответствующий график технического обслуживания. Использование масел только с этими обозначениями создает неоправданный риск повреждения двигателя для двигателей, предназначенных для использования более современных масел, даже при значительно сокращенных интервалах изменения масла. |  |  |  |

> [!warning] ОСТОРОЖНО
> Смазочные масла должны быть классифицированы по одному из стандартов, перечисленных в таблице 1. Нефть, которая никогда не тестировалась и не получала сертификат соответствия от Cummins Inc. Не рекомендуется для использования в продуктах Cummins®.
...
Для объемов нефтяной системы **не**, показанных на графиках, соответствующий интервал изменения масла может быть рассчитан путем интерполяции между интервалами изменения масла, полученными из графиков объемов системы ниже и выше желаемого объема системы. Метод:

#### Переменные

- Объем системы масла для требуемого интервала изменения = VS \\_\\___
- Объем нефтяной системы для диаграммы ниже желаемого объема = VB \_\___
- Объем нефтяной системы для диаграммы выше желаемого объема = VA \_\____
...
2. Расчет коэффициента отношения объема = VR = (VS - VB)/(VA - VB) (\_\_\_____________)/(\_\________________________________________________________________________________________________________________________________________________________________________________________________________________
3. Вычислить желаемый интервал изменения масла = HRS = HRB + ((HRA - HRB) x VR) \[HRS = \_\________________________________________________________________________________________________________________________________________________________________________________________________________________________
