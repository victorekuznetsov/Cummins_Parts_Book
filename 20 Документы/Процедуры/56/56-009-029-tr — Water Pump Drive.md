---
aliases:
  - "Привод водяного насоса"
type: "Процедура"
doc: "56-009-029-tr"
title_en: "Water Pump Drive"
title_ru: "Привод водяного насоса"
modified: "2016-03-15"
engines:
  - "33210083"
  - "33219033"
  - "33224343"
families:
  - "QSK60"
manuals:
  - "4021530"
figures: 20
lang: "ru+en"
translation: "машинный черновик"
source: "https://quickserve.cummins.com/qs3/pubsys2/xml/en/procedures/56/56-009-029-tr.html"
pdf: "https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/main/bulletins/procedures/56-009-029-tr.pdf"
tags:
  - "документ/процедура"
  - "двигатель/QSK60"
  - "группа/56"
  - "перевод/машинный"
---

# Water Pump Drive
**Привод водяного насоса**

> [!abstract] Процедура · `56-009-029-tr`
> **Двигатели:** [[33210083 — QSK60 CM500 CPL 2699|33210083]], [[33219033 — QSK60 CM500 CPL 2848|33219033]], [[33224343 — QSK60 CM500 CPL 2849|33224343]]
> **Семейство:** QSK60
> **Входит в руководства:** [[4021530 — QSK45 and QSK60 Service Manual|4021530]]
> **Секции:** Section 9 - Drive Units · Section 9 - Drive Units - Group 09
> **Даты:** изменён 2016-03-15
> **Источник:** [QuickServe](https://quickserve.cummins.com/qs3/pubsys2/xml/en/procedures/56/56-009-029-tr.html) · [PDF-оригинал](https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/main/bulletins/procedures/56-009-029-tr.pdf)

> [!info]- Перевод на русский — машинный черновик
> Русский текст получен автоматическим переводом с английского
> с подстановкой отраслевой терминологии Cummins; он не
> проходил редакторскую вычитку.
> **Юридически значим только английский оригинал** — он
> приведён в свёрнутом блоке в конце заметки и в PDF.


### Подготовительные операции

с форсункой механического управления

> [!danger] ОПАСНО
> Не снимайте пробку радиатора с горячего двигателя. Снимайте пробку радиатора только после того, как температура охлаждающей жидкости опустится ниже 50 °C \[120 °F\]. Струя горячей охлаждающей жидкости или пар могут привести к травме.

> [!danger] ОПАСНО
> Охлаждающая жидкость токсична. Хранить в недоступном для детей и животных месте. Если деталь не используется повторно, утилизируйте её по местным природоохранным требованиям.

- Слейте охлаждающую жидкость.[[56-008-018-tr — Cooling System|См. процедуру 008-018 в разделе 8.]]
- Удалите водяной насос.[[56-008-062-tr — Water Pump|См. процедуру 008-062 в разделе 8.]]
- Удалите шкив привода генератора.[[56-009-010-tr — Charging System Alternator Drive Pulley|См. процедуру 009-010 в разделе 9.]]
- Удалите уплотнение привода генератора.[[56-001-001 — Alternator Drive Seal|См. процедуру 001-001 в разделе 1.]]

![[ck800wa.png]]

### Первичная проверка

с форсункой механического управления

Измерьте зазор водяного насоса после установки привода.

| Окончательный зазор водяного насоса - установлен |  |  |
|---|---|---|
| мм |  | в |
| 0.05 | Мин | 0.0020 |
| 0.21 | Макс | 0.0083 |

![[09600038.png]]

### Снятие

с форсункой механического управления

Удалите шесть болтов.

Легко нажмите на конец вала резиновым молотком и удалите сборку привода водяного насоса.

Удалите, проверьте и отбросьте прокладку, если она повреждена.

![[09600034.png]]

### Разборка

с форсункой механического управления

Поддерживайте корпус и нажимайте вал через шестерню, пока шестерня не освободится от вала.

![[dw6gena.png]]

Удалите трубу из корпуса.

Используйте свет для осмотра основного прохода масла. Проход должен быть чистым и открытым.

![[09600069.png]]

Удалить вал.

Проверьте корпус на наличие вилки расширения отверстия.

Проверьте вал на наличие пробки для трубы.

Вилки должны быть удалены для очистки масляных буровых установок, если грязь или обломки обнаружены в основном масляном проходе.

Не удаляйте вилки расширения, если основной проход масла чист.

![[09600070.png]]

Измерить внутренний диаметр корпусных втулок.

| Водяной насос Drive Bushing Diameter |  |  |  |
|---|---|---|---|
|  | мм |  | в |
| Свежие бусинги | 49.95 | Мин | 1.965 |
|  | 50.03 | Макс | 1.9697 |
| Стандартные бусинги | 44.48 | Мин | 1.7512 |
|  | 44.58 | Макс | 1.7551 |

Если ветви **не** в пределах спецификаций, ветви должны быть заменены.

![[dw6bsta.png]]

Проверьте бороздчатую поверхность тяговых граней на предмет повреждений.

Измерить расстояние от подшипников тяги (2).

| Водяной насос Drive Thrust Bearing Face Distance |  |  |
|---|---|---|
| мм |  | в |
| 45.370 | Мин | 1.7862 |
| 45.450 | Макс | 1.7894 |

Если расстояние до поверхности подшипника тяги **не** в пределах спецификаций, втулки должны быть заменены.

![[09600072.png]]

Удалите фланцевый втулку из приводного кожуха с помощью латунного дрейфа (1), позаботившись о **не**, чтобы повредить втулку.

![[09600009.png]]

### Очистка и проверка при повторном использовании

с форсункой механического управления

> [!danger] ОПАСНО
> При очистке растворителями, кислотами и щелочными составами соблюдайте указания их изготовителя. Работайте в защитных очках и защитной одежде, чтобы снизить риск травмы.

Очистите детали растворителем.

![[09600082.png]]

Измерить внутренний диаметр корпуса втулочного цилиндра.

| Водяной насос Драйв корпуса бушинг цилиндра бор |  |  |
|---|---|---|
| мм |  | в |
| 60.000 | Мин | 2.3622 |
| 60.050 | Макс | 2.3642 |

Если корпус втулочного цилиндра имеет **не** в пределах спецификаций, корпус должен быть заменен.

![[dw6bsta.png]]

Проверьте вал в зоне уплотнения на износ.

Проверьте область сплинированного вала на предмет износа или повреждения.

Измерить внешний диаметр вала.

| Водный насос приводной вал снаружи диаметра |  |  |  |
|---|---|---|---|
|  | мм |  | в |
| (1) | 35.084 | Мин | 1.3813 |
|  | 35.096 | Макс | 1.3817 |
| (2) | 43.928 | Мин | 1.7294 |
|  | 43.940 | Макс | 1.299 |
| (3) | 44.370 | Мин | 1.7468 |
|  | 44.630 | Макс | 1.7571 |
| (4) | 45.070 | Мин | 1.7744 |
|  | 45.082 | Макс | 1.7749 |
| (5) | 49.820 | Мин | 1.9614 |
|  | 49.832 | Макс | 1.9619 |
| (6) | 39.994 | Мин | 1.5746 |
|  | 40.006 | Макс | 1.5750 |

Если вал **не** в пределах спецификаций, вал должен быть заменен.

![[09600071.png]]

Проверьте зубья зубчатой передачи на износ или питтинг.

Измерьте внутренний диаметр шестерни.

| Водяной насос Drive Gear внутри диаметра |  |  |
|---|---|---|
| мм |  | в |
| 44.98 | Мин | 1.7709 |
| 45.02 | Макс | 1.7724 |

Если передача **не** в пределах спецификаций, то передача должна быть заменена.

![[dp8geta.png]]

### Сборка

с форсункой механического управления

> [!warning] ОСТОРОЖНО
> Втулки должны быть прижаты в квадрате к скобкам корпуса. Лица тяги двух втулок должны быть параллельными и концентрическими. Несоблюдение может привести к искажению тягового лица.

Поддерживайте корпус (5).

Используя пресс (1), спейсер (3) и вал (2), нажмите на втулку (4) в корпус.

Повторите процесс для оставшегося втулки.

![[09600007.png]]

Смазать втулки и тяговые грани с помощью LubriplateTM 105 или эквивалента.

![[09600011.png]]

Поддерживают корпус и монтаж вала в гидравлическом прессе.

Очистите любой любриплат от вала и нанесите ровный слой Loctite® 609 или эквивалентный внутреннему диаметру шестерни.

Установите вал в кожух.

Нажмите на шестерню на вал, пока шестерня не коснется плеча вала.

![[dw6shea.png]]

Измерьте зазор привода водяного насоса.

| Водонасосный привод End Clearance |  |  |
|---|---|---|
| мм |  | в |
| 0.05 | Мин | 0.0020 |
| 0.21 | Макс | 0.0083 |

Если конечный зазор **не** в пределах спецификаций, убедитесь, что оба подшипника тяги правильно расположены в противолодках корпуса водяного насоса.

Если подшипники расположены правильно и конечный зазор **не** в пределах спецификаций, привод должен быть полностью разобран и проверялись размеры отдельных компонентов.

![[dw6drta.png]]

### Установка

с форсункой механического управления

Смазать втулку в кожухе передней передачи с помощью LubriplateTM 105, Part Number 3163086, 3163087 или эквивалента.

Установите прокладку на привод пилота.

Установите привод насоса, позволяя шестерне поворачиваться, когда она сливается с шестерней бездельника.

Выровняйте отверстия болтов и установите шесть болтов для установки водяного насоса.

Затяните шесть болтов.

> [!tip] Момент затяжки
> 45 Н·м [33 фунт-фут]

![[09600034.png]]

Измерьте зазор водяного насоса после установки привода.

| Окончательный зазор водяного насоса - установлен |  |  |
|---|---|---|
| мм |  | в |
| 0.05 | Мин | 0.0020 |
| 0.21 | Макс | 0.0083 |

![[09600038.png]]

### Завершающие операции

с форсункой механического управления

- Установите уплотнение привода генератора.[[56-001-001 — Alternator Drive Seal|См. процедуру 001-001 в разделе 1.]]
- Установите шкив привода генератора переменного тока.[[56-009-010-tr — Charging System Alternator Drive Pulley|См. процедуру 009-010 в разделе 9.]].
- Установите водяной насос.[[56-008-062-tr — Water Pump|См. процедуру 008-062 в разделе 8.]]
- Заправьте систему охлаждения.[[56-008-018-tr — Cooling System|См. процедуру 008-018 в разделе 8.]]
- Запустите двигатель и проверьте на отсутствие утечек.

![[ck800wa.png]]


> [!quote]- Original (English) · английский оригинал
> ### Preparatory Steps
>
> with Mechanically Actuated Injector
>
> **WARNING · Опасно**
> Do not remove the pressure cap from a hot engine. Wait until the coolant temperature is below 50°C \[120°F\] before removing the pressure cap. Heated coolant spray or steam can cause personal injury.
>
> **WARNING · Опасно**
> Coolant is toxic. Keep away from children and pets. If not reused, dispose of in accordance with local environmental regulations.
>
> - Drain the coolant. [[56-008-018-tr — Cooling System|Refer to Procedure 008-018 in Section 8.]]
> - Remove the water pump. [[56-008-062-tr — Water Pump|Refer to Procedure 008-062 in Section 8.]]
> - Remove the alternator drive pulley. [[56-009-010-tr — Charging System Alternator Drive Pulley|Refer to Procedure 009-010 in Section 9.]]
> - Remove the alternator drive seal. [[56-001-001 — Alternator Drive Seal|Refer to Procedure 001-001 in Section 1.]]
>
> ### Initial Check
>
> with Mechanically Actuated Injector
>
> Measure the water pump drive end clearance after the drive has been installed.
>
> | Water Pump Drive End Clearance - Installed |  |  |
> |---|---|---|
> | mm |  | in |
> | 0.05 | MIN | 0.0020 |
> | 0.21 | MAX | 0.0083 |
>
> ### Remove
>
> with Mechanically Actuated Injector
>
> Remove the six capscrews.
>
> Lightly tap on the end of the shaft with a rubber mallet, and remove the water pump drive assembly.
>
> Remove, inspect, and discard the gasket, if damaged.
>
> ### Disassemble
>
> with Mechanically Actuated Injector
>
> Support the housing and press the shaft through the gear until the gear is free from the shaft.
>
> Remove the pipe plug from the housing.
>
> Use a light to inspect the main oil passage. The passage **must** be clean and open.
>
> Remove the shaft.
>
> Check the housing for an orifice expansion plug.
>
> Check the shaft for an orifice pipe plug.
>
> The plugs **must** be removed to clean the oil drillings if dirt or debris is found in the main oil passage.
>
> Do **not** remove the expansion plugs if the main oil passage is clean.
>
> Measure the inside diameter of the housing bushings.
>
> | Water Pump Drive Bushing Diameter |  |  |  |
> |---|---|---|---|
> |  | mm |  | in |
> | Flanged Bushings | 49.95 | MIN | 1.965 |
> |  | 50.03 | MAX | 1.9697 |
> | Standard Bushings | 44.48 | MIN | 1.7512 |
> |  | 44.58 | MAX | 1.7551 |
>
> If the bushings are **not** within specifications, the bushings **must** be replaced.
>
> Check the grooved surface of the thrust faces for damage.
>
> Measure the distance from the thrust bearing faces (2).
>
> | Water Pump Drive Thrust Bearing Face Distance |  |  |
> |---|---|---|
> | mm |  | in |
> | 45.370 | MIN | 1.7862 |
> | 45.450 | MAX | 1.7894 |
>
> If the thrust bearing face distance is **not** within specifications, the bushings **must** be replaced.
>
> Remove the flanged bushing from the drive housing using a brass drift (1), taking care **not** to damage the bushing.
>
> ### Clean and Inspect for Reuse
>
> with Mechanically Actuated Injector
>
> **WARNING · Опасно**
> When using solvents, acids, or alkaline materials for cleaning, follow the manufacturer's recommendations for use. Wear goggles and protective clothing to reduce the possibility of personal injury.
>
> Clean the parts with solvent.
>
> Measure the inside diameter of the housing bushing bore.
>
> | Water Pump Drive Housing Bushing Bore |  |  |
> |---|---|---|
> | mm |  | in |
> | 60.000 | MIN | 2.3622 |
> | 60.050 | MAX | 2.3642 |
>
> If the housing bushing bore is **not** within specifications, the housing **must** be replaced.
>
> Check the shaft at the seal area for wear.
>
> Check the shaft splined area for wear or damage.
>
> Measure the outside diameter of the shaft.
>
> | Water Pump Drive Shaft Outside Diameter |  |  |  |
> |---|---|---|---|
> |  | mm |  | in |
> | (1) | 35.084 | MIN | 1.3813 |
> |  | 35.096 | MAX | 1.3817 |
> | (2) | 43.928 | MIN | 1.7294 |
> |  | 43.940 | MAX | 1.299 |
> | (3) | 44.370 | MIN | 1.7468 |
> |  | 44.630 | MAX | 1.7571 |
> | (4) | 45.070 | MIN | 1.7744 |
> |  | 45.082 | MAX | 1.7749 |
> | (5) | 49.820 | MIN | 1.9614 |
> |  | 49.832 | MAX | 1.9619 |
> | (6) | 39.994 | MIN | 1.5746 |
> |  | 40.006 | MAX | 1.5750 |
>
> If the shaft is **not** within specifications, the shaft **must** be replaced.
>
> Check the gear teeth for wear or pitting.
>
> Measure the inside diameter of the gear.
>
> | Water Pump Drive Gear Inside Diameter |  |  |
> |---|---|---|
> | mm |  | in |
> | 44.98 | MIN | 1.7709 |
> | 45.02 | MAX | 1.7724 |
>
> If the gear is **not** within specifications, the gear **must** be replaced.
>
> ### Assemble
>
> with Mechanically Actuated Injector
>
> **CAUTION · Осторожно**
> The bushings must be pressed in square to the bores of the housing. The thrust faces of the two bushings must be parallel and concentric. Failure to comply can result in distortion of the thrust face.
>
> Support the housing (5).
>
> Using an arbor press (1), spacer (3), and shaft (2), press the bushing (4) into the housing.
>
> Repeat the process for the remaining bushing.
>
> Lubricate the bushing bores and thrust faces with Lubriplate™ 105, or equivalent.
>
> Support the housing and shaft assembly in a hydraulic press.
>
> Clean any Lubriplate™ from the shaft and apply an even coat of Loctite® 609, or equivalent, to the inside diameter of the gear.
>
> Install the shaft into the housing.
>
> Press the gear onto the shaft until the gear touches the shoulder of the shaft.
>
> Measure the water pump drive end clearance.
>
> | Water Pump Drive End Clearance |  |  |
> |---|---|---|
> | mm |  | in |
> | 0.05 | MIN | 0.0020 |
> | 0.21 | MAX | 0.0083 |
>
> If the end clearance is **not** within specifications, make sure that both thrust bearings are correctly positioned in the water pump drive housing counterbores.
>
> If the bearings are correctly positioned and the end clearance is **not** within specifications, the drive **must** be completely disassembled and individual component dimensions checked.
>
> ### Install
>
> with Mechanically Actuated Injector
>
> Lubricate the bushing in the front gear housing cover with Lubriplate™ 105, Part Number 3163086, 3163087, or equivalent.
>
> Install the gasket on the drive pilot.
>
> Install the pump drive, allowing the gear to turn as it meshes with the idler gear.
>
> Align the capscrew holes, and install the six water pump drive mounting capscrews.
>
> Tighten the six capscrews.
>
> **Момент затяжки · Torque Value**
> 45 n•m [33 ft-lb]
>
> Measure the water pump drive end clearance after the drive has been installed.
>
> | Water Pump Drive End Clearance - Installed |  |  |
> |---|---|---|
> | mm |  | in |
> | 0.05 | MIN | 0.0020 |
> | 0.21 | MAX | 0.0083 |
>
> ### Finishing Steps
>
> with Mechanically Actuated Injector
>
> - Install the alternator drive seal. [[56-001-001 — Alternator Drive Seal|Refer to Procedure 001-001 in Section 1.]]
> - Install the alternator drive pulley. [[56-009-010-tr — Charging System Alternator Drive Pulley|Refer to Procedure 009-010 in Section 9]].
> - Install the water pump. [[56-008-062-tr — Water Pump|Refer to Procedure 008-062 in Section 8.]]
> - Fill the cooling system. [[56-008-018-tr — Cooling System|Refer to Procedure 008-018 in Section 8.]]
> - Operate the engine and check for leaks.
