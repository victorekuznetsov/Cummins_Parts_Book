---
aliases:
  - "Головка воздушного компрессора (модели Holset® ST)"
type: "Процедура"
doc: "35-012-106"
title_en: "Air Compressor Cylinder Head (Holset® ST Models)"
title_ru: "Головка воздушного компрессора (модели Holset® ST)"
modified: "2022-12-14"
engines:
  - "35354607"
  - "35373113"
  - "71156161"
families:
  - "QSM11"
manuals:
  - "3666322"
figures: 75
lang: "ru+en"
translation: "машинный черновик"
source: "https://quickserve.cummins.com/qs3/pubsys2/xml/en/procedures/35/35-012-106.html"
pdf: "https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/main/bulletins/procedures/35-012-106.pdf"
tags:
  - "документ/процедура"
  - "двигатель/QSM11"
  - "группа/35"
  - "перевод/машинный"
---

# Air Compressor Cylinder Head (Holset® ST Models)
**Головка воздушного компрессора (модели Holset® ST)**

> [!abstract] Процедура · `35-012-106`
> **Двигатели:** [[35354607 — QSM11 CM570 CPL 8608|35354607]], [[35373113 — QSM11 CM570 CPL 8471|35373113]], [[71156161 — QSM11 CM570 CPL 8543|71156161]]
> **Семейство:** QSM11
> **Входит в руководства:** [[3666322 — ISM, ISMe, and QSM11 Service Manual|3666322]]
> **Секции:** Section 12 - Compressed Air System - Group 12
> **Даты:** изменён 2022-12-14
> **Источник:** [QuickServe](https://quickserve.cummins.com/qs3/pubsys2/xml/en/procedures/35/35-012-106.html) · [PDF-оригинал](https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/main/bulletins/procedures/35-012-106.pdf)

> [!info]- Перевод на русский — машинный черновик
> Русский текст получен автоматическим переводом с английского
> с подстановкой отраслевой терминологии Cummins; он не
> проходил редакторскую вычитку.
> **Юридически значим только английский оригинал** — он
> приведён в свёрнутом блоке в конце заметки и в PDF.


### Выбор сервисного инструмента

#### Рекомендованный сервисный инструмент Cummins®

- Горячая пластина, номер детали ST-749
- Шаровая трость, часть номер ST-302
- Средство для удаления сидений компрессора, номер детали 3377416
- Пружинный тестер клапана, номер детали 5299714
- Установка сиденья компрессора, номер детали 3377415

#### Дополнительные сервисные позиции

- Пресса
- О-кольцевой выбор

### Разборка

> [!danger] ОПАСНО
> Масса этого узла больше 23 кг \[50 фунтов\]. Чтобы не получить тяжёлую травму, поднимайте этот узел с помощником или подходящим грузоподъёмным оборудованием.

Используйте два болта 5/16 - 18 х 1 1/4 дюйма для установки воздушного компрессора на монтажной пластине, часть номер ST-749, которая используется с шаровой шторой, часть номер ST-302.

![[cp8bdwa.png]]

Удалите центральный разгрузчик корпуса шестиугольника головные болты и замкните шайбу.

![[cp8csmb.png]]

Удалите удерживающий зажим.

![[cp8clma.png]]

> [!warning] ОСТОРОЖНО
> Корпус разгрузочного клапана установлен с пружинным натяжением. Тщательно удаляйте, чтобы избежать травм. Носите защиту лица и глаз.

Удалите корпус разгрузочного клапана.

![[cp8bdmb.png]]

Удалите крышку разгрузочного клапана, если она установлена.

![[cp8cuma.png]]

Удалить и выбросить уплотнение с кольцом.

![[cp8orma.png]]

Используйте колечко для удаления прямоугольного кольцевого уплотнения, если оно установлено.

Отбросьте уплотнение.

![[cp8sema.png]]

Удалите пружину разгрузочного клапана.

![[cp8spma.png]]

Используйте колечко для удаления уплотнения.

Отбросьте уплотнение.

![[cp8ormb.png]]

Разгрузить два разгрузчика шестиугольник головных болтов.

![[cp8csmc.png]]

> [!danger] ОПАСНО
> Носите соответствующую защиту глаз и лица при выполнении этой процедуры. Причалы находятся в напряжении и могут действовать как снаряд, если их выпустить, что приводит к травмам.

Держитесь за тело разгрузчика и медленно удалите два болта разгрузчика и два простых шайбы, чтобы предотвратить разгрузчика тела от выкидывания на свободу и причинение личной травмы.

![[cp8csmd.png]]

Удалите корпус разгрузочного клапана.

![[cp8bdme.png]]

Снимите крышку разгрузчика.

![[cp8cumb.png]]

Удалите и отбросьте верхнюю уплотнение.

![[cp8ormc.png]]

Удалите и отбросьте нижнюю уплотнение.

![[cp8ormd.png]]

Используйте колечко для удаления прямоугольного кольцевого уплотнения.

Отбросьте уплотнение.

![[cp8semb.png]]

Удалите пружину разгрузчика.

Повторите последние восемь шагов, чтобы удалить другой разгрузчик.

![[cp8spmb.png]]

Для ST676 удалите десять оставшихся шестиугольников и два шайбы.

Для ST773 удалите восемь шайбых болтов и два шестиугольных головных болта и шайбы.

![[cp8csme.png]]

Снимите крышку.

![[cp8cvmc.png]]

Удалить и выбросить прокладку крышки.

![[cp8gkma.png]]

Снимите впускное клапанное сиденье.

![[cp8vami.png]]

Удалить впускной клапан.

![[cp8vamj.png]]

Удалите пружину впускного клапана.

Повторите последние три шага, чтобы удалить другой впускной клапан.

![[cp8spmc.png]]

Убери голову.

![[cp8hdmd.png]]

Удалить и выбросить прокладку головы.

![[cp8gkmb.png]]

Используйте пресс и воздушный компрессор для удаления сиденья, номер детали 3377416, чтобы удалить сиденье выхлопного клапана из головы.

![[cp8semc.png]]

Удалите выхлопной клапан.

![[cp8vamk.png]]

Удалить и выбросить уплотнение с кольцом.

![[cp8orme.png]]

Удалите пружину выпускного клапана.

![[cp8spmd.png]]

Снимите износостойкую пластину.

Повторите последние пять шагов, чтобы разобрать другой сборочный узел сиденья выпускного клапана.

![[cp8plma.png]]

### Очистка

> [!danger] ОПАСНО
> При очистке растворителями, кислотами и щелочными составами соблюдайте указания их изготовителя. Работайте в защитных очках и защитной одежде, чтобы снизить риск травмы.

Замочите детали в керосиновой эмульсии на основе чистящего средства, предназначенного для удаления углерода. Уборщик должен иметь рН 9,5 или менее, чтобы избежать черного цвета алюминиевых деталей. С производителем или поставщиком более чистых продуктов необходимо связаться по поводу концентрации раствора, температуры и времени выдержки.

![[cp8bdec.png]]

> [!note] Примечание
> Не используйте скребок для удаления углерода и масштабирования, герметизирующие поверхности могут быть повреждены.

Части могут быть очищены жесткой неметаллической щеткой.

![[cp8bdei.png]]

### Проверка при повторном использовании

Измерить диаметр пресса сиденья выхлопного клапана в головке цилиндра.

Заменить, если износ больше 20,688 мм \[0,8145 в\].

![[cp8brna.png]]

Измерить диаметр пресса сиденья выхлопного клапана.

Заменить, если износ меньше 20,714 мм \[0,8155 в\].

![[cp8sena.png]]

Нанесите смазку на поверхность выхлопных газов, чтобы проверить сиденье.

Если поверхность сиденья **не** на 100% соответствует действительности, откажитесь от сиденья и замените его.

![[12800028.png]]

Измерить высоту сиденья выхлопного клапана.

Если высота меньше 12,32 мм \[0,485 дюйма \], заменить сиденье выхлопного клапана.

![[cp8vata.png]]

Нанести смазку на поверхность впускного клапана.

Если поверхность сиденья **не**, то 100% замените сиденье впускного клапана.

![[12800029.png]]

Измерить высоту сиденья впускного клапана.

Если высота меньше 6,86 мм \[0,270 дюйма\], замените сиденье впускного клапана.

![[cp8vana.png]]

Осмотрите выпускные и впускные клапаны на наличие трещин и повреждений.

Измерьте клапаны для плоскостности. Клапаны должны быть плоскими в пределах 0,03 мм \[0,001 дюйма \].

Замените клапаны, если они сломаны, повреждены или **не** плоские.

> [!note] Примечание
> Holset Engineering Co., Inc. Рекомендуется устанавливать новые клапаны во время реконструкции.

![[cp8vask.png]]

Используйте клапанный пружинный тестер, номер детали 5299714, для проверки пружин.

Заменить в пределах, перечисленных в таблице спецификаций усилия пружины компрессора, показанной ниже.

> [!note] Примечание
> Holset Engineering Co., Inc. Рекомендуется устанавливать новые источники во время реконструкции.

![[cp8spta.png]]

| Загрузка, необходимая для сжатия пружины до длины измерения |  |  |  |  |  |  |  |
|---|---|---|---|---|---|---|---|
| Весенний номер детали | Весеннее использование | Приблизиться. Свободная длина: \[in\] | Количество катушек | Диаметр провода: \[in\] | Длина измерения: \[in\] | Минимум: \[lb\] | Максимум: \[lb\] |
| 128080 | Выпускной клапан | 17.02 \[0.670\] | 3.0 | 2.03 \[0.080\] | 7.11 \[0.280\] | 3.6 \[8.55\] | 4.7 \[10.35\] |
| 190334 | Впускной клапан | 12.70 \[0.500\] | 2.8 | 1.58 \[0.062\] | 7.11 \[0.280\] | 0.35 \[0.65\] | 0.5 \[1.10\] |
| 150631 | Разгрузочный клапан (естественно аспирированный) центральный разгрузочный клапан - двойной | 41.91 \[1.650\] | 11.5 | 2.03 \[0.080\] | 24.89 \[0.980\] | 14.5 \[32.00\] | 17.2 \[38.00\] |
| 3023101 | Разгрузочный клапан (все турбированные двигатели) | 41.91 \[1.650\] | 10.8 | 1.65 \[0.065\] | 24.89 \[0.980\] | 5.9 \[13.00\] | 7.7 \[17.00\] |
| 3049553 | Клапан разгрузчика E-Type | 41.91 \[1.650\] | 11.25 | 1.93 \[0.076\] | 24.89 \[0.980\] | 10.4 \[23.00\] | 12.2 \[27.00\] |
| 800399-XW | разгрузчик | 17.02 \[0.67\] | 6 | 1.04 \[0.041\] | 6.60 \[0.260\] | Не применяется | 2.54 \[5.60 |
|  | 10.03 \[3.95\] | 1.71 \[3.78\] | Не применяется |  |  |  |  |
| 802000-ФЗ | Взятие и выхлоп | 10.16 \[0.40\] | 4.25 | 0.79 \[0.031\] | 4.57 \[0.18\] | Не применяется | 0.36 \[0.80\] |
|  | 5.99 \[0.275\] | 0.20 \[0.45\] | Не применяется |  |  |  |  |
| 3054489 | Выпускной клапан | 21.49 \[0.846\] | 4.5 | 2.54 \[0.100\] | 15.21 \[0.599\] | 3.88 \[8.55\] | 4.74 \[10.45\] |

Осмотрите верхнюю часть разгрузочной крышки, где расположены прямоугольные кольцевые сиденья уплотнения для забивания.

Заменить, если забит.

![[cp8cusc.png]]

### Сборка

Установите износостойкую пластину.

![[cp8plma.png]]

Установите пружину выпускного клапана с танговой торцевой станцией **down**.

> [!note] Примечание
> Holset Engineering Co., Inc. Рекомендуется устанавливать новые источники во время реконструкции.

![[cp8spmd.png]]

Установите новое уплотнение.

![[cp8orme.png]]

Установите выпускной клапан.

> [!note] Примечание
> Holset Engineering Co., Inc. Рекомендуется устанавливать новые выпускные клапаны во время реконструкции.

![[cp8vamk.png]]

Используйте чистое масло 15W-40 для смазки уплотнения.

![[cp8orwa.png]]

> [!note] Примечание
> Не использовать чрезмерное давление на сиденье выпускного клапана, чтобы сделать это может исказить клапан.

Используйте ручной пресс и инструмент установки воздушного компрессора, номер детали 3377415, чтобы нажать на сиденье выхлопного клапана в голову.

Повторите последние шесть шагов для установки другого клапана выхлопной системы.

![[cp8seha.png]]

Установите новую прокладку головки цилиндра.

![[cp8gkmb.png]]

Установите голову.

![[cp8hdmd.png]]

Установите новую прокладку крышки.

![[cp8gkma.png]]

Установите крышку.

![[cp8cvmc.png]]

Для ST676 устанавливают восемь шестиугольных головных болтов. Затягивай палец.

Для ST773 установите восемь шайбых болтов в неволе.

![[cp8cshc.png]]

Используйте опцию o-ring, чтобы правильно установить новое уплотнение o-ring.

![[cp8orhb.png]]

Установите разгрузочную пружину.

> [!note] Примечание
> Holset Engineering Co., Inc. Рекомендуется устанавливать новые источники во время реконструкции.

![[cp8sphb.png]]

> [!warning] ОСТОРОЖНО
> Прямоугольное кольцевое уплотнение должно быть установлено с выгнутой стороной вверх; несоблюдение этого требования приведет к повреждению системы воздушного воздуха и неисправности тормоза.

Используйте колечко для кольцевой заборки, номер детали 3376399, чтобы правильно установить новое прямоугольное кольцевое уплотнение, если корпус разгрузчика имеет полость для этого кольца.

![[cp8sehc.png]]

Установите новое уплотнение.

![[cp8orma.png]]

Используйте противозахватное соединение для смазки наружного диаметра разгрузочного клапана.

![[cp8cuwa.png]]

Установите колпачок в корпус разгрузочного клапана.

![[cp8cuma.png]]

Используйте чистое масло 15W-40 для смазки уплотнения.

![[cp8orwb.png]]

> [!danger] ОПАСНО
> Носите соответствующую защиту глаз и лица при выполнении этой процедуры. Причалы находятся в напряжении и могут действовать как снаряд, если их выпустить, что приводит к травмам.

Установите тело в крышку.

![[cp8bdha.png]]

Установите удерживающий зажим и три шайбы замка и три шестиугольных головных болта.

![[cp8cshd.png]]

Затягивайте болты в проиллюстрированной последовательности.

> [!tip] Момент затяжки
> 41 Н·м [30 фунт-фут]

![[cp8csog.png]]

Установите пружину впускного клапана с танговой торцевой станцией **down**.

> [!note] Примечание
> Holset Engineering Co., Inc. Рекомендуется устанавливать новые источники во время реконструкции.

![[cp8sphc.png]]

Установите впускной клапан.

> [!note] Примечание
> Holset Engineering Co., Inc. Рекомендуется устанавливать новые впускные клапаны во время реконструкции.

![[cp8vahf.png]]

Установите сиденье впускного клапана с фланцевым концом **up**.

![[cp8vahg.png]]

Установите пружину разгрузчика.

> [!note] Примечание
> Holset Engineering Co., Inc. Рекомендуется устанавливать новые источники во время реконструкции.

![[cp8sphd.png]]

Используйте противозахватное соединение для смазки наружного диаметра разгрузочного клапана.

![[cp8cuwb.png]]

Установите крышку в крышку и убедитесь, что три касания находятся в трех слотах сиденья впускного клапана.

![[cp8cuha.png]]

> [!warning] ОСТОРОЖНО
> Прямоугольное кольцевое уплотнение должно быть установлено с выгнутой стороной вверх; несоблюдение этого требования приведет к повреждению системы воздушного воздуха и неисправности тормоза.

Используйте колечко для правильной установки нового прямоугольного кольцевого уплотнения.

![[cp8sehb.png]]

Установите новое нижнее уплотнение.

![[cp8ormd.png]]

Установите новое верхнее уплотнение.

![[cp8ormc.png]]

Используйте чистое масло 15W-40 для смазки уплотнений.

![[cp8bdwc.png]]

Установите разгрузочный корпус в крышку.

![[cp8bdhb.png]]

> [!warning] ОСТОРОЖНО
> Корпус разгрузчика устанавливается с весенним натяжением. Он должен быть установлен так, как описано, чтобы предотвратить травмы.

Держитесь крепко за разгрузочный корпус. Установите болты и простые шайбы, чтобы предотвратить выгрузку тела разгрузчика и причинение личной травмы.

![[cp8cshe.png]]

Затяните болты.

> [!tip] Момент затяжки
> 14 Н·м [124 фунт-дюйм]

Повторите последние тринадцать шагов для установки другой разгрузочной сборки кузова.

![[cp8csoh.png]]


> [!quote]- Original (English) · английский оригинал
> ### Select Service Tools
>
> #### Recommended Cummins® Service Tools
>
> - Mounting plate, Part Number ST-749
> - Ball joint vise, Part Number ST-302
> - Air compressor seat removal tool, Part Number 3377416
> - Valve spring tester, Part Number 5299714
> - Air compressor seat installation tool, Part Number 3377415
>
> #### Additional Service Items
>
> - Press
> - O-ring pick
>
> ### Disassemble
>
> **WARNING · Опасно**
> This component or assembly weighs greater than 23 kg \[50 lb\]. To prevent serious personal injury, be sure to have assistance or use appropriate lifting equipment to lift this component or assembly.
>
> Use two 5/16 - 18 x 1 1/4 inch capscrews to mount the air compressor on the mounting plate, Part Number ST-749, which is used with the ball joint vise, Part Number ST-302.
>
> Remove the center unloader body hexagon head capscrew and lock washer.
>
> Remove the retaining clamp.
>
> **CAUTION · Осторожно**
> The unloader valve body is installed with spring tension. Carefully remove to avoid personal injury. Wear face and eye protection.
>
> Remove the unloading valve body.
>
> Remove the unloader valve cap, if installed.
>
> Remove and discard the o-ring seal.
>
> Use o-ring pick to remove the rectangular ring seal, if installed.
>
> Discard the seal.
>
> Remove the unloader valve cap spring.
>
> Use o-ring pick to remove the o-ring seal.
>
> Discard the seal.
>
> Loosen the two unloader body hexagon head capscrews.
>
> **WARNING · Опасно**
> Wear appropriate eye and face protection when performing this procedure. Springs are under tension and can act as a projectile if released, causing personal injury.
>
> Hold down on the unloader body and slowly remove the two unloader body capscrews and the two plain washers so as to prevent the unloader body from being thrown free and causing personal injury.
>
> Remove the unloader valve body.
>
> Remove the unloader cap.
>
> Remove and discard the top o-ring seal.
>
> Remove and discard the bottom o-ring seal.
>
> Use o-ring pick to remove the rectangular ring seal.
>
> Discard the seal.
>
> Remove the unloader cap spring.
>
> Repeat the last eight steps to remove the other unloader body assembly.
>
> For the ST676, remove the ten remaining hexagon head capscrews and two lock washers.
>
> For the ST773, remove the eight captive washer capscrews and the two hexagon head capscrews and lock washers.
>
> Remove the cover.
>
> Remove and discard the cover gasket.
>
> Remove the intake valve seat.
>
> Remove the intake valve.
>
> Remove the intake valve spring.
>
> Repeat the last three steps to remove the other intake valve assembly.
>
> Remove the head.
>
> Remove and discard the head gasket.
>
> Use a press and air compressor seat removal tool, Part Number 3377416, to remove the exhaust valve seat assembly from the head.
>
> Remove the exhaust valve.
>
> Remove and discard the o-ring seal.
>
> Remove the exhaust valve spring.
>
> Remove the wear plate.
>
> Repeat the last five steps to disassemble the other exhaust valve seat assembly.
>
> ### Clean
>
> **WARNING · Опасно**
> When using solvents, acids, or alkaline materials for cleaning, follow the manufacturer's recommendations for use. Wear goggles and protective clothing to reduce the possibility of personal injury.
>
> Soak the parts in a kerosene emulsion based cleaner designed to remove carbon. The cleaner **must** have a pH of 9.5 or less to avoid turning aluminum parts black. The cleaner manufacturer or supplier **must** be contacted about solution concentration, temperature, and soak time.
>
> **Note · Примечание**
> Do **not** use a scraper to remove carbon and scale, the sealing surfaces can be damaged.
>
> The parts can be scrubbed with a stiff nonmetallic bristle brush.
>
> ### Inspect for Reuse
>
> Measure the exhaust valve seat press fit diameter in the cylinder head.
>
> Replace if worn larger than 20.688 mm \[0.8145 in\].
>
> Measure the exhaust valve seat press fit diameter.
>
> Replace if worn smaller than 20.714 mm \[0.8155 in\].
>
> Apply bluing to the exhaust seating surface to check the seat.
>
> If the seating surface is **not** 100 percent true, discard and replace the valve seat.
>
> Measure the exhaust valve seat height.
>
> If the height is less than 12.32 mm \[0.485 in\], replace the exhaust valve seat.
>
> Apply bluing to the intake valve seating surface.
>
> If the seating surface is **not** 100 percent true replace the intake valve seat.
>
> Measure the intake valve seat height.
>
> If the height is less than 6.86 mm \[0.270 in\], replace the intake valve seat.
>
> Inspect the exhaust and intake valves for cracks and damage.
>
> Measure valves for flatness. The valves **must** be flat within 0.03 mm \[0.001 in\].
>
> Replace the valves if cracked, damaged, or **not** flat.
>
> **Note · Примечание**
> Holset Engineering Co., Inc. recommends new valves be installed during rebuild.
>
> Use valve spring tester, Part Number 5299714, to check the springs.
>
> Replace if within limits listed in the Compressor Spring Force Specifications Table shown below.
>
> **Note · Примечание**
> Holset Engineering Co., Inc. recommends new springs be installed during rebuild.
>
> | Load Required to Compress Spring to Measurement Length |  |  |  |  |  |  |  |
> |---|---|---|---|---|---|---|---|
> | Spring Part Number | Spring Use | Approx. Free Length: mm \[in\] | Number of Coils | Wire Diameter: mm \[in\] | Measurement Length: mm \[in\] | Minimum: kg \[lb\] | Maximum: kg \[lb\] |
> | 128080 | Exhaust valve | 17.02 \[0.670\] | 3.0 | 2.03 \[0.080\] | 7.11 \[0.280\] | 3.6 \[8.55\] | 4.7 \[10.35\] |
> | 190334 | Intake valve | 12.70 \[0.500\] | 2.8 | 1.58 \[0.062\] | 7.11 \[0.280\] | 0.35 \[0.65\] | 0.5 \[1.10\] |
> | 150631 | Unloading valve (naturally aspirated) center unloading valve - twin | 41.91 \[1.650\] | 11.5 | 2.03 \[0.080\] | 24.89 \[0.980\] | 14.5 \[32.00\] | 17.2 \[38.00\] |
> | 3023101 | Unloading valve (all turbocharged engines) | 41.91 \[1.650\] | 10.8 | 1.65 \[0.065\] | 24.89 \[0.980\] | 5.9 \[13.00\] | 7.7 \[17.00\] |
> | 3049553 | E-Type unloader valve | 41.91 \[1.650\] | 11.25 | 1.93 \[0.076\] | 24.89 \[0.980\] | 10.4 \[23.00\] | 12.2 \[27.00\] |
> | 800399-XW | Unloader | 17.02 \[0.67\] | 6 | 1.04 \[0.041\] | 6.60 \[0.260\] | N/A | 2.54 \[5.60 |
> |  | 10.03 \[3.95\] | 1.71 \[3.78\] | N/A |  |  |  |  |
> | 802000-FZ | Intake and exhaust | 10.16 \[0.40\] | 4.25 | 0.79 \[0.031\] | 4.57 \[0.18\] | N/A | 0.36 \[0.80\] |
> |  | 5.99 \[0.275\] | 0.20 \[0.45\] | N/A |  |  |  |  |
> | 3054489 | Exhaust valve | 21.49 \[0.846\] | 4.5 | 2.54 \[0.100\] | 15.21 \[0.599\] | 3.88 \[8.55\] | 4.74 \[10.45\] |
>
> Inspect the upper part of the unloader cap where the rectangular ring seal seats for scoring.
>
> Replace if scored.
>
> ### Assemble
>
> Install the wear plate.
>
> Install the exhaust valve spring with the tang end **down**.
>
> **Note · Примечание**
> Holset Engineering Co., Inc. recommends new springs be installed during rebuild.
>
> Install a new o-ring seal.
>
> Install the exhaust valve.
>
> **Note · Примечание**
> Holset Engineering Co., Inc. recommends new exhaust valves be installed during rebuild.
>
> Use clean 15W-40 oil to lubricate the seal.
>
> **Note · Примечание**
> Do **not** use excessive pressure on the exhaust valve seats, to do so can distort the valve.
>
> Use a hand press and an air compressor seat installation tool, Part Number 3377415, to press the exhaust valve seat into the head.
>
> Repeat the last six steps to install the other exhaust valve assembly.
>
> Install a new cylinder head gasket.
>
> Install the head.
>
> Install a new cover gasket.
>
> Install the cover.
>
> For the ST676, install the eight hexagon head capscrews. Finger tighten.
>
> For the ST773, install the eight captive washer capscrews.
>
> Use o-ring pick to correctly install the new o-ring seal.
>
> Install the unloading spring.
>
> **Note · Примечание**
> Holset Engineering Co., Inc. recommends new springs be installed during rebuild.
>
> **CAUTION · Осторожно**
> The rectangular ring seal must be installed with the grooved side up; failure to do so will result in air system damage and brake malfunction.
>
> Use o-ring pick, Part Number 3376399, to correctly install the new rectangular ring seal, if the unloader body has a cavity for this o-ring.
>
> Install a new o-ring seal.
>
> Use anti-seize compound to lubricate the outside diameter of the unloader valve cap.
>
> Install the cap into the unloading valve body.
>
> Use clean 15W-40 oil to lubricate the seal.
>
> **WARNING · Опасно**
> Wear appropriate eye and face protection when performing this procedure. Springs are under tension and can act as a projectile if released, causing personal injury.
>
> Install the body into the cover.
>
> Install the retaining clamp and the three lock washers and the three hexagon head capscrews.
>
> Tighten the capscrews in the sequence illustrated.
>
> **Момент затяжки · Torque Value**
> 41 n•m [30 ft-lb]
>
> Install the intake valve spring with the tang end **down**.
>
> **Note · Примечание**
> Holset Engineering Co., Inc. recommends new springs be installed during rebuild.
>
> Install the intake valve.
>
> **Note · Примечание**
> Holset Engineering Co., Inc. recommends new intake valves be installed during rebuild.
>
> Install the intake valve seat with the flange end **up**.
>
> Install the unloader cap spring.
>
> **Note · Примечание**
> Holset Engineering Co., Inc. recommends new springs be installed during rebuild.
>
> Use anti-seize compound to lubricate the outside diameter of the unloader valve cap.
>
> Install the cap into the cover and make sure the three tangs are in the three slots of the intake valve seat.
>
> **CAUTION · Осторожно**
> The rectangular ring seal must be installed with the grooved side up; failure to do so will result in air system damage and brake malfunction.
>
> Use o-ring pick to correctly install the new rectangular ring seal.
>
> Install a new bottom o-ring seal.
>
> Install a new top o-ring seal.
>
> Use clean 15W-40 oil to lubricate the seals.
>
> Install the unloading body into the cover.
>
> **CAUTION · Осторожно**
> The unloader body is installed with spring tension. It must be installed as described to prevent personal injury.
>
> Hold down on the unloading body firmly. Install the capscrews and plain washers so as to prevent the unloader body from being thrown free and causing personal injury.
>
> Tighten the capscrews.
>
> **Момент затяжки · Torque Value**
> 14 n•m [124 in-lb]
>
> Repeat the last thirteen steps to install the other unloading body assembly.
