...
from common import (engine_registry, BUILD, CAT_RU, DIRS, FAMILY_OF_CAT, VAULT, load_json,
...
def main():
    ...
    for esn, e in sorted(engines.items()):
        r = reg.get(esn, {})
        m = r.get("machine") or r.get("place") or "—"
        home.append(f"| [[{eng_note[esn]}\\|{esn}]] | {e['model']} | {e['cpl']} | "
                    f"{m} | {len(e['options'])} | {e['parts_total']} |")
    home += [
        "",
        "## Индексы",
        "",
    ...
    home += [
        ...
        "> Тексты документов даны в русском переводе, английский оригинал — "
        "в свёрнутом блоке в конце каждой заметки и в PDF по ссылке. Перевод "
        "**машинный черновой**: выполнен офлайн-моделью с подстановкой "
        "терминологии Cummins, редакторскую вычитку не проходил. Названия "
        "документов, деталей, узлов и вся навигация переведены вручную.",
    ...
    search = [
        ...
        "## Язык документов",
        "",
        "Тексты документов приведены **на русском языке**. Английский оригинал "
        "лежит в той же заметке — в свёрнутом блоке `Original (English)` в самом "
        "конце; нажмите на заголовок блока, чтобы раскрыть. Полностью "
        "оригинальная вёрстка с иллюстрациями — в PDF по ссылке `pdf` в шапке "
        "заметки.",
        "",
        "> [!warning] Перевод машинный",
        "> Русский текст получен автоматическим переводом с подстановкой "
        "терминологии Cummins (форсунка, коромысло, момент затяжки, жгут "
        "проводов и т. д.) и **не проходил редакторскую вычитку**. Значения "
        "моментов, зазоров, номера деталей и артикулы перенесены без изменений, "
        "но формулировки могут быть неточными.",
        "> Перед выполнением работ сверяйтесь с английским оригиналом: "
        "юридически значим только он.",
        "",
        "Полнотекстовый поиск (`Ctrl+Shift+F`) ищет **и по русскому переводу, "
        "и по английскому оригиналу** — оба текста лежат в одной заметке. "
        "Чтобы искать только по оригиналу, добавьте к запросу `\"> \"` — "
        "английский блок оформлен цитатой.",
        "",
        "Заметки с переводом помечены тегом `#перевод/машинный`, в поле "
        "`lang` стоит `ru+en`.",
        "",
    ...
    rows = ["---", 'type: "Индекс"', "---", "", "# Детали с ценами", "",
            f"{len(priced)} артикулов Cummins с ценой из прайса «Горная Евразия».", "",
            "| Артикул | Наименование | Русское название | Текущая | Несогласованная |",
            "|---|---|---|---|---|"]
    for p in sorted(priced, key=lambda x: x.get("no") or ""):
        pr = p["price"]
        rows.append(f"| [[{p['no']}]] | {p.get('name','')} | {p.get('name_ru','')} | "
                    f"{pr.get('cur','') if pr.get('cur') is not None else ''} | "
                    f"{pr.get('new','') if pr.get('new') is not None else ''} |")
    ...
    rows = ["---", 'type: "Справка"', "---", "", "# Карта хранилища", "",
            ...
            f"| `11 Машины/Парк` | парк: машина -> VIN -> ESN -> CPL -> каталог | 7 |",
