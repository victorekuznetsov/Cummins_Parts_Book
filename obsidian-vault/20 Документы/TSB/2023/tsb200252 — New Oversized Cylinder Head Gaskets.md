---
aliases:
  - "Новые увеличенные прокладки ГБЦ"
type: "TSB"
doc: "tsb200252"
title_en: "New Oversized Cylinder Head Gaskets"
title_ru: "Новые увеличенные прокладки ГБЦ"
released: "2023-08-28"
modified: "2023-08-28"
group: "02 - Cylinder Head"
engines:
  - "33224404"
  - "33239899"
  - "37269910"
  - "37280605"
  - "41340468"
  - "41349633"
  - "41353297"
families:
  - "K19"
  - "K38/K50 · QSK38, QSK50"
  - "QSK19"
  - "QSK50"
figures: 4
lang: "ru+en"
translation: "машинный черновик"
source: "https://quickserve.cummins.com/qs3/pubsys2/xml/en/tsb/2020/tsb200252.html"
pdf: "https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/main/bulletins/tsb/tsb200252.pdf"
tags:
  - "документ/tsb"
  - "двигатель/K19"
  - "двигатель/K38/K50"
  - "двигатель/QSK19"
  - "двигатель/QSK50"
  - "год/2023"
  - "перевод/машинный"
  - "тема/cylinder-head"
---

# New Oversized Cylinder Head Gaskets
**Новые увеличенные прокладки ГБЦ**

> [!abstract] TSB · `tsb200252`
> **Раздел Cummins:** 02 - Cylinder Head
> **Двигатели:** [[33224404 — QSK50 CM2150 MCRS CPL 3391|33224404]], [[33239899 — QSK50 CM2150 MCRS CPL 3379|33239899]], [[37269910 — K19 CPL 1253|37269910]], [[37280605 — K19 CPL 447|37280605]], [[41340468 — QSK50 CM2150 MCRS CPL 3728|41340468]], [[41349633 — QSK19 CM2150 MCRS CPL 3666|41349633]], [[41353297 — QSK19 CM2150 MCRS CPL 3666|41353297]]
> **Семейство:** K19, K38/K50 · QSK38, QSK50, QSK19, QSK50
> **Даты:** выпущен 2023-08-28 · изменён 2023-08-28
> **Источник:** [QuickServe](https://quickserve.cummins.com/qs3/pubsys2/xml/en/tsb/2020/tsb200252.html) · [PDF-оригинал](https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/main/bulletins/tsb/tsb200252.pdf)

> [!info]- Перевод на русский — машинный черновик
> Русский текст получен автоматическим переводом с английского
> с подстановкой отраслевой терминологии Cummins; он не
> проходил редакторскую вычитку.
> **Юридически значим только английский оригинал** — он
> приведён в свёрнутом блоке в конце заметки и в PDF.


## Новые увеличенные прокладки ГБЦ

### Положение о гарантии

Сведения в этом документе не изменяют действующие гарантийные обязательства и практику ремонта и не дают оснований для работ по программам TRP или Campaign.

### Содержание

**Затронутая продукция**

- K19 (все версии)
- K38 (все версии)
- K50 (все версии)
- QSK19 (все версии)
- QSK38 (все версии)
- QSK50 (все версии)

**Описание изменения**

В дополнение к уже имеющейся прокладке с головкой цилиндра негабаритного размера 0,5 мм доступны прокладки негабаритного размера 0,25 мм и 0,75 мм. На этих негабаритных прокладках появились новые идентификационные признаки, позволяющие их различать:

- Все прокладки головки цилиндра большого размера имеют одну изолированную выемку (показано в левом нижнем углу рисунка 1 ниже).
- Количество выемок в противоположном углу прокладки головки цилиндра представляет собой размер:

**Идентификация детали**

Для оставшегося запаса существующей конструкции прокладки головки цилиндра для справки был включен рисунок 1 ниже. Обратите внимание, что в правом верхнем углу нет выемок, но выемка в левом нижнем углу все еще присутствует, это прокладка размером 0,5 мм.

![[02r00128.png]]

Рисунок 1, существующий дизайн головки цилиндра с увеличенным размером, показывающий выемку внизу слева (только для дифференцировки).

![[02r00129.png]]

Рисунок 2, Новая габаритная цилиндрическая головка размером 0,25 мм, показывающая одну выемку в правом верхнем углу.

![[02r00130.png]]

Рисунок 3, новая оболочка цилиндра размером 0,5 мм, показывающая две выемки в правом верхнем углу.

![[02r00131.png]]

Рисунок 4, Новая габаритная цилиндрическая головка размером 0,75 мм, показывающая три выемки в правом верхнем углу.

**Наличие сервисных деталей**

Сервисные детали доступны для заказа. Номера деталей приведены в таблице 1.

| Таблица 1, Части обслуживания |  |  |  |  |
|---|---|---|---|---|
| Часть описание | Существующий номер детали | устарелый | Заменённый | Новый номер детали |
| 0,25 мм Гаскет головы | - | - | - | 5628816 |
| Гаскет головы размером 0,5 мм | 4334081 | Нет | Нет | - |
| 0,75 мм Гаскет головы | - | - | - | 5628817 |

**Часть инвентаризации**

Используйте существующий инвентарь номеров деталей перед использованием нового номера деталей.

### История изменений документа


> [!quote]- Original (English) · английский оригинал
> ## New Oversized Cylinder Head Gaskets
>
> ### Warranty Statement
>
> The information in this document has no effect on present warranty coverage or repair practices, nor does it authorize TRP or Campaign actions.
>
> ### Contents
>
> **Product Affected**
>
> - K19 (All Versions)
> - K38 (All Versions)
> - K50 (All Versions)
> - QSK19 (All Versions)
> - QSK38 (All Versions)
> - QSK50 (All Versions)
>
> **Description of Change**
>
> In addition to the already-available 0.5 mm oversized cylinder head gasket, a 0.25 mm and 0.75 mm oversized gaskets are available. There are new identification features on these oversized gaskets in order to differentiate them:
>
> - All oversized cylinder head gaskets have a single isolated notch (shown in bottom left corner of Figure 1 below)
> - The number of notches in the opposite corner of the cylinder head gasket represent the size:
>
> **Part Identification**
>
> For remaining stock of the existing cylinder head gasket design, Figure 1 below has been included for reference. Note there are no notches at the top right corner, but a notch at the bottom left corner is still present, this is a 0.5mm oversized gasket.
>
> Figure 1, Existing Oversized Cylinder Head Gasket Design Showing Notch at Bottom Left (For Differentiation Purposes Only).
>
> Figure 2, New 0.25 mm Oversized Cylinder Head Gasket Showing One Notch at Upper Right.
>
> Figure 3, New 0.5 mm Oversized Cylinder Head Gasket Showing Two Notches at Upper Right.
>
> Figure 4, New 0.75 mm Oversized Cylinder Head Gasket Showing Three Notches at Upper Right.
>
> **Service Parts Availability**
>
> Service parts are available. See Table 1 for part numbers.
>
> | Table 1, Service Parts |  |  |  |  |
> |---|---|---|---|---|
> | Part Description | Existing Part Number | Obsolete | Superseded | New Part Number |
> | 0.25mm Oversized Head Gasket | - | - | - | 5628816 |
> | 0.5 mm Oversized Head Gasket | 4334081 | No | No | - |
> | 0.75mm Oversized Head Gasket | - | - | - | 5628817 |
>
> **Part Inventory Action**
>
> Use existing part number inventory before using the new part number.
>
> ### Document History
