---
aliases:
  - "Новый штуцер маслоподводящей магистрали турбокомпрессора QSK19"
type: "TSB"
doc: "tsb110189"
title_en: "New Turbocharger Oil Supply Line Male Union, QSK19 Engine"
title_ru: "Новый штуцер маслоподводящей магистрали турбокомпрессора QSK19"
released: "2011-07-12"
modified: "2011-07-12"
group: "10 - Intake Air Systems"
engines:
  - "41349633"
  - "41353297"
families:
  - "QSK19"
parts:
  - "3015283"
  - "3037537"
  - "3922794"
  - "4924291"
figures: 3
lang: "ru+en"
translation: "машинный черновик"
source: "https://quickserve.cummins.com/qs3/pubsys2/xml/en/tsb/2011/tsb110189.html"
pdf: "https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/main/bulletins/tsb/tsb110189.pdf"
tags:
  - "документ/tsb"
  - "двигатель/QSK19"
  - "год/2011"
  - "перевод/машинный"
  - "тема/intake-air-systems"
---

# New Turbocharger Oil Supply Line Male Union, QSK19 Engine
**Новый штуцер маслоподводящей магистрали турбокомпрессора QSK19**

> [!abstract] TSB · `tsb110189`
> **Раздел Cummins:** 10 - Intake Air Systems
> **Двигатели:** [[41349633 — QSK19 CM2150 MCRS CPL 3666|41349633]], [[41353297 — QSK19 CM2150 MCRS CPL 3666|41353297]]
> **Семейство:** QSK19
> **Даты:** выпущен 2011-07-12 · изменён 2011-07-12
> **Источник:** [QuickServe](https://quickserve.cummins.com/qs3/pubsys2/xml/en/tsb/2011/tsb110189.html) · [PDF-оригинал](https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/main/bulletins/tsb/tsb110189.pdf)

> [!info]- Перевод на русский — машинный черновик
> Русский текст получен автоматическим переводом с английского
> с подстановкой отраслевой терминологии Cummins; он не
> проходил редакторскую вычитку.
> **Юридически значим только английский оригинал** — он
> приведён в свёрнутом блоке в конце заметки и в PDF.


## Новый штуцер маслоподводящей магистрали турбокомпрессора QSK19

### Положение о гарантии

Сведения в этом документе не изменяют действующие гарантийные обязательства и практику ремонта и не дают оснований для работ по программам TRP или Campaign.

### Содержание

В этом документе представлено новое оборудование для сантехники моторного масла для двигателя QSK19. Новый с наружной резьбой фитинг, номер детали 2888894, был выпущен для решения проблем маршрутизации сантехники с турбонаддувом. В настоящем документе содержится решение вопроса, описанного в [[tsb110039 — Turbocharger Plumbing Routing Issue\|TSB110039]]. В таблице 1 показаны новые номера деталей и кольца, включенные в эту сборку.

| Таблица 1: Новые внешне резьбовые фитинги и O-кольца |  |  |
|---|---|---|
| Номер детали | Наименование | Доступно |
| 2888894 | Союз, штыревой | Да |
| Содержит: |  |  |
| 2888849 | Союз, штыревой | Нет |
| [[3037537]] | Тюлень, кольцо | Да |
| [[3922794]] | Тюлень, кольцо | Да |

Новый внешне резьбовой фитинг выше, чем предыдущие союзы, используемые на входе масла турбокомпрессора. Это позволяет обеспечить больший зазор между шлангом подачи масла турбокомпрессора и другими объектами в этой области, включая подшипниковый кожух и линию вентиляции охлаждающей жидкости. На рисунке 1 показана старая фитинга слева и новая фитинга справа.

![[10d00783.png]]

Рисунок 1. Старый с наружной резьбой фитинг Versus Новый таллер с наружной резьбой фитинг

Серийный номер двигателя (ESN) для этого изменения - 37249951, построенный 25 июня-2011 года.

Процедуры 20-010-041-тр и [[20-010-046 — Turbocharger Oil Supply Line\|20-010-046]] в Руководстве по обслуживанию QSK19, Бюллетень 4021595, были обновлены, чтобы включить уведомление о более высоком союзе и проверки на наличие трения и других повреждений на плетеных линиях.

Процедуры 13-019-390 в руководстве по устранению и ремонту электронных систем управления QSK19 (CM850 Modular Common Rail System) Electronic Control System Troubleshooting and Repair Manual, Bulletin 4021493 и процедуре [[123-019-390 — Turbocharger Speed Sensor\|123-019-390]] в руководстве по устранению и ремонту электронных систем управления QSK19 CM2150, Bulletin [[4022094 — QSK19 CM2150 and CM2670 Electronic Control System Troubleshooting and Repair Manual\|4022094]], были обновлены, чтобы включать удаление скобки и установку клипсов для прикрепления провода датчика скорости к линии подачи масла.

Новый с наружной резьбой фитинг, Part Number 2888894, не заменяет номера деталей, поскольку эти соединения используются на других местах и двигателях. В таблице 2 показаны варианты, на которые повлияло это изменение.

| Таблица 2. Варианты, содержащие новую фитинг |  |
|---|---|
| Номер опции | Наименование |
| TP4703 | Сливной, турбокомпрессор |
| TP4704 | Сливной, турбокомпрессор |
| TP4707 | Сливной, турбокомпрессор |
| TP4708 | Сливной, турбокомпрессор |
| TP4709 | Сливной, турбокомпрессор |
| TP4710 | Сливной, турбокомпрессор |
| TP4718 | Сливной, турбокомпрессор |
| TP4725 | Сливной, турбокомпрессор |
| TP4726 | Сливной, турбокомпрессор |
| TP4728 | Сливной, турбокомпрессор |
| TP4729 | Сливной, турбокомпрессор |

На QSK19 MCRS Tier 3 с датчиком скорости турбокомпрессора эта новая внешне резьбовая фитинга вызывает помехи в скобке, поддерживающей провод датчика скорости. Чтобы решить эту проблему, скобка была устаревшей, а аппаратное обеспечение было удалено из затронутых вариантов. Теперь провод должен быть прикреплен к линии подачи масла, чтобы обеспечить максимальный зазор. Это похоже на маршрутизацию других вариантов турбокомпрессора. В таблице 3 показаны изменения структуры этих вариантов. На рисунке 2 показана устаревающая скобка. На рисунке 3 показана новая маршрутизация проводов датчика скорости турбокомпрессора. В таблице 4 показаны варианты, в которых скобка была удалена.

| Таблица 3: Добавлены и удалены номера частей из затронутых вариантов |  |  |  |  |  |
|---|---|---|---|---|---|
| Добавлено | Удаленный |  |  |  |  |
| Новый номер детали | Котти | Наименование | Прежний номер детали | Котти | Наименование |
| 4924789 | 2 | Зажим, проводной стяжка |  |  |  |
|  |  |  | 4924749 | 1 | Брекет, проводной ретейнер |
|  |  |  | 3093927 | 2 | Винт Hex Flange Head Cap |
|  |  |  | [[4924291]] | 1 | Скоба |
|  |  |  | [[3015283]] | 1 | К черту, кэптивный шайба |

![[10d00784.png]]

![[10d00785.png]]

Рисунок 2. Предыдущая Скоростной датчик Wire Routing с кронштейном

Рисунок 3. Новая маршрутизация Speed Sensor Wire с помощью клипов

| Таблица 4. Варианты с Bracket Removed |  |
|---|---|
| Вариант | Наименование |
| PH4760 | Монтаж, датчик скорости турбокомпрессора |
| PH4761 | Монтаж, датчик скорости турбокомпрессора |
| PH4788 | Монтаж, датчик скорости турбокомпрессора |
| PH4789 | Монтаж, датчик скорости турбокомпрессора |

### История изменений документа

## Детали, упомянутые в документе

| Артикул | Наименование | Русское название |
|---|---|---|
| [[3015283]] | CAPTIVE WASHER CAP SCREW | Болт с неотделяемой шайбой |
| [[3037537]] | O RING SEAL | Уплотнительное кольцо |
| [[3922794]] | O RING SEAL | Уплотнительное кольцо |
| [[4924291]] | CLIP | Скоба |

> [!quote]- Original (English) · английский оригинал
> ## New Turbocharger Oil Supply Line Male Union, QSK19 Engine
>
> ### Warranty Statement
>
> The information in this document has no effect on present warranty coverage or repair practices, nor does it authorize TRP or Campaign actions.
>
> ### Contents
>
> This document introduces new turbocharger lubricating oil plumbing hardware for the QSK19 engine. A new male union, Part Number 2888894, was released to address turbocharger plumbing routing issues. This document provides resolution to the issue described in [[tsb110039 — Turbocharger Plumbing Routing Issue\|TSB110039]]. Table 1 shows the new part number and o-rings included in this assembly.
>
> | Table 1: New Male Union and O-rings |  |  |
> |---|---|---|
> | Part Number | Description | Available |
> | 2888894 | Union, male | Yes |
> | Contains: |  |  |
> | 2888849 | Union, male | No |
> | [[3037537]] | Seal, o-ring | Yes |
> | [[3922794]] | Seal, o-ring | Yes |
>
> The new male union is taller than the previous unions used at the turbocharger oil inlet. This allows for more clearance between the turbocharger oil supply hose and other objects in this area, including the bearing housing and coolant vent line. Figure 1 shows the old fitting on the left and the new fitting on the right.
>
> Figure 1. Old Male Union Versus New Taller Male Union
>
> The engine serial number (ESN) first for this change is 37249951, built on 25-June-2011.
>
> Procedures [[20-010-041-tr — Turbocharger Coolant Hoses\|20-010-041-tr]] and [[20-010-046 — Turbocharger Oil Supply Line\|20-010-046]] in the QSK19 Service Manual, Bulletin 4021595, have been updated to include notification of the taller union and checks for fretting and other damage on the braided lines.
>
> Procedures 13-019-390 in the QSK19 (CM850 Modular Common Rail System) Electronic Control System Troubleshooting and Repair Manual, Bulletin 4021493, and procedure [[123-019-390 — Turbocharger Speed Sensor\|123-019-390]] in the QSK19 CM2150 Electronic Control System Troubleshooting and Repair Manual, Bulletin [[4022094 — QSK19 CM2150 and CM2670 Electronic Control System Troubleshooting and Repair Manual\|4022094]], have been updated to include removal of the bracket and installation of the clips to attach the speed sensor wire to the oil supply line.
>
> The new male union, Part Number 2888894, does **not** supersede any part numbers because these unions are used on other locations and engines. Table 2 shows the options affected by this change.
>
> | Table 2. Options Containing a New Fitting |  |
> |---|---|
> | Option Number | Description |
> | TP4703 | Plumbing, turbocharger |
> | TP4704 | Plumbing, turbocharger |
> | TP4707 | Plumbing, turbocharger |
> | TP4708 | Plumbing, turbocharger |
> | TP4709 | Plumbing, turbocharger |
> | TP4710 | Plumbing, turbocharger |
> | TP4718 | Plumbing, turbocharger |
> | TP4725 | Plumbing, turbocharger |
> | TP4726 | Plumbing, turbocharger |
> | TP4728 | Plumbing, turbocharger |
> | TP4729 | Plumbing, turbocharger |
>
> On QSK19 MCRS Tier 3 options with a turbocharger speed sensor, this new male union causes an interference with the bracket supporting the speed sensor wire. To resolve this, the bracket has been obsoleted and the hardware has been removed from the affected options. Now, the wire should be clipped to the oil supply line to allow for maximum clearance. This is similar routing to other turbocharger options. Table 3 shows the structure changes to these options. Figure 2 shows the bracket that is being obsoleted. Figure 3 shows the new turbocharger speed sensor wire routing. Table 4 shows the options that had the bracket removed.
>
> | Table 3: Added and Removed Part Numbers from Affected Options |  |  |  |  |  |
> |---|---|---|---|---|---|
> | Added | Removed |  |  |  |  |
> | New Part Number | Qty | Description | Old Part Number | Qty | Description |
> | 4924789 | 2 | Clamp, Wire Tie |  |  |  |
> |  |  |  | 4924749 | 1 | Bracket, Wiring Retainer |
> |  |  |  | 3093927 | 2 | Screw, Hex Flange Head Cap |
> |  |  |  | [[4924291]] | 1 | Clip |
> |  |  |  | [[3015283]] | 1 | Screw, Captive Washer Cap |
>
> Figure 2. Previous Speed Sensor Wire Routing with Bracket
>
> Figure 3. New Speed Sensor Wire Routing with Clips
>
> | Table 4. Options With Bracket Removed |  |
> |---|---|
> | Option | Description |
> | PH4760 | Mounting, turbocharger speed sensor |
> | PH4761 | Mounting, turbocharger speed sensor |
> | PH4788 | Mounting, turbocharger speed sensor |
> | PH4789 | Mounting, turbocharger speed sensor |
>
> ### Document History
