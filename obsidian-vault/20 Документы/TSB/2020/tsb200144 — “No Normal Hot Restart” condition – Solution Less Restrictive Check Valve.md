...
> [!note] Примечание
> *Приложения для прокрутки прелюбия и CM850, не затронутые этим изменением

**Проблема**

Симптомы: После нормального режима отключения (> 90 С) возникает состояние перезапуска и
...
Когда двигатель холодный, вязкость масла высока, и давление достаточно высокое, чтобы подняться выше порога 4psi, и стартер двигателя включает маховик, чтобы перевернуть двигатель.

Если двигатель оснащен прелюбовой системой, двигатель не перезапускается, когда жаркое время запуска кажется более длительным, чем обычно, может быть пороговое значение допуска или фактора, препятствующее запуску двигателя быстрее. Перед поиском разрешения убедитесь, что коды неисправностей отсутствуют, и следуйте Tc t044, прежде чем приступить к разрешению ниже.

**Решение**

Если проверка детали и или ситуации завершена, замените текущий контрольный клапан прелюбия соответствующим номером детали на диаграмме ниже. Новый контрольный клапан является улучшением текущей конструкции и не требует гарантийного изменения текущей конструкции, если приложение работает. Если приложение работает неправильно (только QST30 CM2350), обратитесь в службу технической поддержки для надлежащей кампании.

Прелюбный контрольный клапан изменили на менее ограничительный клапан, который позволяет 20 галлонов на минуту (гпм) расхода от текущего 12 гпм расхода. С дополнительным потоком стажер давления остается относительно высоким выше 4 psi, поэтому переключатель давления тока закрывается, чтобы завершить цепь и подзарядить стартер для оборота двигателя.
...
| клапан, Check (включает кольцо) | 4371676 | Да | Да | [[5416292]] |
| - уплотнение, О Кольцо | [[3089240]] | Нет | Нет | Не применяется |
...
| 37280938 (только CM2350) | 16 декабря 2019 | Сеймурский моторный завод |
