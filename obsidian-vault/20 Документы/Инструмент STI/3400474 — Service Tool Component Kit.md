---
aliases:
  - "Комплект компонентов сервисного инструмента"
type: "Инструкция по инструменту"
doc: "3400474"
title_en: "Service Tool Component Kit"
title_ru: "Комплект компонентов сервисного инструмента"
released: "2014-07-23"
modified: "2014-07-24"
engines:
  - "33224404"
  - "33239746"
  - "33239899"
  - "41340468"
families:
  - "K38/K50 · QSK38, QSK50, QSK60"
  - "QSK50"
parts:
  - "3013222"
figures: 1
lang: "ru+en"
translation: "машинный черновик"
source: "https://quickserve.cummins.com/qs3/pubsys2/xml/en/sti/3400474.html"
pdf: "https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/main/bulletins/sti/3400474.pdf"
tags:
  - "документ/инструмент"
  - "двигатель/K38/K50"
  - "двигатель/QSK50"
  - "перевод/машинный"
---

# Service Tool Component Kit
**Комплект компонентов сервисного инструмента**

> [!abstract] Инструкция по инструменту · `3400474`
> **Двигатели:** [[33224404 — QSK50 CM2150 MCRS CPL 3391|33224404]], [[33239746 — QSK60 CM2150 MCRS CPL 3451|33239746]], [[33239899 — QSK50 CM2150 MCRS CPL 3379|33239899]], [[41340468 — QSK50 CM2150 MCRS CPL 3728|41340468]]
> **Семейство:** K38/K50 · QSK38, QSK50, QSK60, QSK50
> **Даты:** выпущен 2014-07-23 · изменён 2014-07-24
> **Источник:** [QuickServe](https://quickserve.cummins.com/qs3/pubsys2/xml/en/sti/3400474.html) · [PDF-оригинал](https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/main/bulletins/sti/3400474.pdf)

> [!info]- Перевод на русский — машинный черновик
> Русский текст получен автоматическим переводом с английского
> с подстановкой отраслевой терминологии Cummins; он не
> проходил редакторскую вычитку.
> **Юридически значим только английский оригинал** — он
> приведён в свёрнутом блоке в конце заметки и в PDF.


### Наименование

Комплект компонентов сервисного инструмента

### Назначение

В этом документе представлена информация для набора компонентов, номер детали 5299014. Набор используется для модернизации существующего комплекта подъемных приспособлений топливного насоса, Части № 4918603, ныне устаревшей и замененной Части № 5299015, чтобы иметь ту же функциональность, что и текущий комплект подъемных приспособлений топливного насоса, Части № 5299015, используемый для удаления и установки топливного насоса на двигателях QSK50 MCRS и QSK60 MCRS.

![[22300007.png]]

| Компонентный комплект, номер детали 5299014 |  |  |  |  |
|---|---|---|---|---|
| Справочный номер | Номер детали | Наименование | Количество | Доступно для покупки |
| 1 | [[3013222]] | Винт Hexagon Head Cap | 4 | Да |
| 2 | 5299011 | Проставка | 2 | Да |
| 3 | 5299012 | Проставка | 2 | Да |
| 4 | 5299013 | Винт Hex Flange Head Cap | 4 | Да |

### История изменений документа

## Детали, упомянутые в документе

| Артикул | Наименование | Русское название |
|---|---|---|
| [[3013222]] | HEXAGON HEAD CAP SCREW | Болт с шестигранной головкой |

> [!quote]- Original (English) · английский оригинал
> ### Description
>
> Service Tool Component Kit
>
> ### Purpose
>
> This document provides information for Service Tool Component Kit, Part Number 5299014. The kit is used to upgrade existing fuel pump lifting fixture kit, Part Number 4918603, now obsolete and superseded by, Part Number 5299015, to have the same functionality as current fuel pump lifting fixture kit, Part Number 5299015, used to remove and install the fuel pump on the QSK50 MCRS and QSK60 MCRS engines.
>
> | Service Tool Component Kit, Part Number 5299014 |  |  |  |  |
> |---|---|---|---|---|
> | Reference Number | Part Number | Description | Quantity | Available to Purchase |
> | 1 | [[3013222]] | Screw, Hexagon Head Cap | 4 | Yes |
> | 2 | 5299011 | Spacer | 2 | Yes |
> | 3 | 5299012 | Spacer | 2 | Yes |
> | 4 | 5299013 | Screw, Hex Flange Head Cap | 4 | Yes |
>
> ### Document History
