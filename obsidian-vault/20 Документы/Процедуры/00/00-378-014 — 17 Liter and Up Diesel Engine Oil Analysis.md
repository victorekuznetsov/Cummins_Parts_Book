...
Второе ограничение анализа масла – невозможность обнаружить быстро развивающиеся сбои. Для выявления потенциальной проблемы в момент взятия последнего образца масла должен был произойти сбой. Некоторые режимы отказа развиваются так быстро, что они не будут определены в анализе масла.

Руководящие принципы, изложенные в этой процедуре, не должны использоваться для установления интервалов слива масла. Использование этих руководящих принципов для увеличения интервалов отвода масла может привести к продолжению использования масла, которое больше не обеспечивает предполагаемую защиту. Тенденции анализа масла могут быть отображены в течение определенного периода времени и могут предоставить важную информацию о интервалах изменения масла. Для получения дополнительной информации по этой теме, обратитесь к руководству по обслуживанию жидкостей для Cummins®, Бюллетень [[5411406 — Fluids for Cummins® Products Service Manual\|5411406]]. См. раздел 4, или Руководство по эксплуатации и техническому обслуживанию для конкретной модели двигателя для другой информации, связанной с маслом.

### Параметры испытаний

В таблице 1 перечислены те, кто носит металлы, масляные загрязнители и свойства масла, которые обеспечивают наиболее надежные показатели для выявления потенциальной проблемы. Этот список содержит как минимальный набор элементов, которые должны быть проверены во время анализа масла, так и список рекомендуемых элементов для мониторинга для расширенного тестирования. Продление испытаний рекомендуется при подозрении на наличие проблемы с состоянием масла либо по результатам минимального испытания, либо по какому-либо другому показателю при работе двигателя.

| **Таблица 1: Элементы для мониторинга анализа масла двигателя высокой мощности**|  |  |  |
|---|---|---|---|
| **Масляная собственность** | **Аббревиатура** | **Минимальные испытания** | **Расширенные испытания** |
...
| 1 Метод D2896 ASTM не используется. Число TBN может быть ложно преувеличено для используемого моторного масла. 2 Лаборатории могут сообщать о значении от E2412 (FTIR), калиброванном до E1131. 3 Данные инфракрасной спектроскопии преобразования Фурье (FTIR) могут использовать либо метод ASTM, но должны сообщать о тренде или вычитании. |  |  |  |
...
Для установления исходного уровня важно провести анализ масла на новой (неиспользованной) масла. Новые (неиспользованные) образцы для анализа масла следует брать каждый раз, когда тип масла или поставщик масла изменяется или, как минимум, два раза в год. Образцы должны быть взяты из резервуаров для сыпучих материалов для определения состава масла, а также для подтверждения того, что система хранения не вводит никаких загрязняющих веществ.
...
- Соберите минимум 118 мл [4 фло-унции] масла.
...
- Очистите внешнюю часть клапана, вытирая чистой сухой тряпкой.
- Запускайте двигатель на холостом ходу и доводите до теплой температуры.
- Очистите образец, позволив небольшому количеству масла течь через клапан.
- Соберите образец в чистую, сухую бутылку из потока масла, перекачиваемого холостым двигателем.
...
3. Разрежьте трубку так, чтобы она достигала 25-50 мм \[1-2 в \] ниже уровня масла в кастрюле.
...
- Общие мощности масляной системы

Если все двигатели сгруппированы вместе, независимо от масляных систем, двигатели с фильтрацией типа CentinelTM, EliminatorTM/Centrifuge и более высокими мощностями масляной системы будут испытывать более высокую степень износа двигателя до достижения предела флага.

В таблице 2 перечислены пределы осуждения масла. Если эти ограничения достигнуты, то масло должна быть изменена.

| **Таблица 2: Осуждение за анализ масла двигателя высокой мощности** |  |  |  |
|---|---|---|---|
| **Масляная собственность** | **Аббревиатура** | **Единицы** | **Ограничение на осуждение** |
...
| Общее количество кислот | Тан | миллиграмм гидроксида калия на грамм образца (мг KOH/г) | 2.5 увеличение по сравнению с новой маслом или равное общему базовому числу (TBN) |
| сажа | сажа | % (%) | 5% 2 |
| Нитратация | Нит | единицы поглощения на сантиметр (абс/см) | 25 |
| окисление | окс | единицы поглощения на сантиметр (абс/см) | 15 1 |
| 1 Увеличение по сравнению с новым (неиспользованным) образцом масла. 2 Масла, соответствующие стандарту API CH-4 (CES 20071), представляют собой минимальный стандарт качества для двигателей Cummins® с высокой мощностью. Масла, отвечающие более старым требованиям API (C F-4, CG-4), не могут переносить более 1,5 процента сажи. |  |  |  |
...
|---|---|---|---|---|
| **Масляная собственность** | **Аббревиатура** | **Единицы** | **Предельный флаг** | **Критический флаг** |
...
Устранение неполадок деревьев, перечисленных ниже, предназначено для того, чтобы служить руководством для расследования случаев износа металлов или масляных загрязнителей. Некоторые из деревьев предназначены для прямого исследования, основанного на уровне тяжести флага.
...
Флаги анализа масла для распространенных режимов отказа:
...
| **Проблемы, возникающие в результате анализа масляных флагов** |  |
|---|---|
...
> [!note] Примечание
> Чтобы быть предупрежденным о проблеме, в момент взятия последнего образца масла должен был произойти сбой. Некоторые режимы отказа развиваются так быстро, что они не будут определены в анализе масла.

> [!note] Примечание
> Когда масло изменяется на двигателе с удаленным масляным баком, в котором масло непрерывно циркулирует (**не**, в состав бака CentinelTM входит масло), масло в удаленном баке должно быть изменено, в дополнение к маслу в отстойнике двигателя. Если масло в удаленном резервуаре не будет изменено, оно будет действовать как источник загрязнения для следующего образца масла.

Процедуры устранения неполадок и ремонта можно найти в соответствующем наборе руководств по QuickserveTM Online для исследуемого двигателя.

Cummins Inc. не оплачивает работу по анализу масла и не возмещает расходы на тестирование, необходимое для программы анализа масла.

| **Уровень содержания алюминия на миллион (PPM) превышает уровень осторожности или критический предел алюминиевого флага, основанный на результатах масляных лабораторий или Cummins Inc. пределов** |  |  |
|---|---|---|
| **Причина** | **Пояснение** | **Коррекция** |
| Расширенный интервал отбора проб масла | Если время работы масла в пробе больше, чем время работы масла в пробах прошлого образца, износ металлов может быть повышен. | Проверьте, не превышает ли время работы масла на момент пробы более чем на 25 процентов по сравнению с предыдущими часами пробы. Если да, то повторите выборку через регулярный интервал. |
| Дистанционное циркуляция масляных резервуаров, макияжное масло или системы CentinelTM не работают должным образом | Масло не пополняется или мощность масла снижается, что приводит к повышенному уровню износа металлов. | Ремонт удаленной циркуляции масла, макияжа или системы CentinelTM. Резервный образец через регулярный интервал. |
...
| Недавно был отремонтирован двигатель, что привело к загрязнению масла. | Предыдущий ремонт вызвал загрязнение маслом, и масляная система была **не** очищена. | Меняйте масло в отстойнике двигателя и удаленном масляном баке. Проверьте внешнюю масляную схему (т.е. Удаленный фильтр головки/удаленный масляный бак) для загрязнения. Резервный образец через регулярный интервал. |
...
| **Уровень содержания медных деталей на миллион превышает предельный уровень, установленный на основании результатов масляных лабораторий или Cummins Inc. пределов** |  |  |
|---|---|---|
| **Причина** | **Пояснение** | **Коррекция** |
| Расширенный интервал отбора проб масла | Если время работы масла в пробе больше, чем время работы масла в пробах прошлого образца, износ металлов может быть повышен. | Проверьте, не превышает ли время работы масла на момент пробы более чем на 25 процентов по сравнению с предыдущими часами пробы. Если да, то повторите выборку через регулярный интервал. |
| Дистанционное циркуляция масляных резервуаров, макияжное масло или системы CentinelTM не работают должным образом | Масло не пополняется или мощность масла снижается, что приводит к повышенному уровню износа металлов. | Ремонт удаленной циркуляции масла, макияжа или системы CentinelTM. Резервный образец через регулярный интервал. |
...
| **Уровень содержания медных деталей на миллион превышает критический предел медного флага, основанный на результатах масляных лабораторий или Cummins Inc. пределов** |  |  |
|---|---|---|
| **Причина** | **Пояснение** | **Коррекция** |
...
| Ношение втулок **не**, содержащих свинец | Подключение стержневых втулок, зубчатых втулок, крановых втулок. | Проверьте температуру масла или перепады давления. Проверьте, низкая ли вязкость. Если да, то обратитесь к дереву с низкой вязкостью. Вырезать фильтры, открывать или открывать центрифугу и проверять наличие мусора. Если в фильтрах/центрифуге не обнаружено обломков, запросите феррографический анализ образца масла, который произвел повышенный уровень меди. Если мусор в фильтре/центрифуге или феррографическом анализе указывает на втулку мусора, проверьте втулки. |

| **Разбавление топлива превышает лимит флага на основе результатов масляной лаборатории или Cummins Inc. пределов** |  |  |
|---|---|---|
| **Причина** | **Пояснение** | **Коррекция** |
| Частые старты | Топливо не горит полностью при низких температурах камеры сгорания и может просачиваться мимо поршневых колец и в картер. | Меняйте масло в отстойнике двигателя и удаленном масляном баке. |
| Чрезмерное идлинг | Топливо не горит полностью при низких температурах камеры сгорания и может просачиваться мимо поршневых колец и в картер. | Меняйте масло в отстойнике двигателя и удаленном масляном баке. Если возможно, выключите двигатель, а не на холостом ходу или поднимите скорость холостого хода. |
| Холодная погода Операция | Топливо не горит полностью при низких температурах камеры сгорания и может просачиваться мимо поршневых колец и в картер. | Меняйте масло в отстойнике двигателя и удаленном масляном баке. |
| Частые запуски, чрезмерное холостое время или холодная погода | Топливо не горит полностью при низких температурах камеры сгорания и может просачиваться мимо поршневых колец и в картер. | Меняйте масло в отстойнике двигателя и удаленном масляном баке. Если возможно, выключите двигатель, а не на холостом ходу или поднимите скорость холостого хода. |
| Недавно был отремонтирован двигатель, что привело к загрязнению масла. | Предыдущий ремонт вызвал загрязнение маслом, и масляная система была **не** очищена. | Меняйте масло в отстойнике двигателя и удаленном масляном баке. Проверьте внешнюю масляную схему (т.е. Удаленный фильтр головки/удаленный масляный бак) для загрязнения. Резервный образец через регулярный интервал. |
...
| **Уровень содержания железных частей на миллион превышает предельный уровень предупреждения о железе, основанный на результатах масляных лабораторий или Cummins Inc. пределов** |  |  |
|---|---|---|
| **Причина** | **Пояснение** | **Коррекция** |
| Расширенный интервал отбора проб масла | Если время работы масла в пробе больше, чем время работы масла в пробах прошлого образца, износ металлов может быть повышен. | Проверьте, не превышает ли время работы масла на момент пробы более чем на 25 процентов по сравнению с предыдущими часами пробы. Если да, то повторите выборку через регулярный интервал. |
| Дистанционное циркуляция масляных резервуаров, макияжное масло или системы CentinelTM не работают должным образом | Масло не пополняется или мощность масла снижается, что приводит к повышенному уровню износа металлов. | Ремонт удаленной циркуляции масла, макияжа или системы CentinelTM. Резервный образец через регулярный интервал. |
...
основанный на результатах масляных лабораторий или Cummins Inc. пределов** |  |  |
|---|---|---|
| **Причина** | **Пояснение** | **Коррекция** |
...
| **Уровень содержания калия на миллион (PPM) превышает предупредительный предел калия, основанный на результатах масляной лаборатории или Cummins Inc. пределов** |  |  |
|---|---|---|
| **Причина** | **Пояснение** | **Коррекция** |
| Недавно был отремонтирован двигатель, что привело к загрязнению масла. | Предыдущий ремонт вызвал загрязнение маслом, и масляная система была **не** очищена. | Меняйте масло в отстойнике двигателя и удаленном масляном баке. Проверьте внешнюю масляную схему (т.е. Удаленный фильтр головки/удаленный масляный бак) для загрязнения. Резервный образец через регулярный интервал. |
...
| **Уровень содержания калия на миллион (PPM) превышает критический предел калия, установленный на основании результатов масляных лабораторий или Cummins Inc. пределов** |  |  |
|---|---|---|
| **Причина** | **Пояснение** | **Коррекция** |
...
| **Уровень кремниевых деталей на миллион превышает предел осторожности или критического значения кремниевого флага, основанный на результатах масляных лабораторий или Cummins Inc. пределов** |  |  |
|---|---|---|
| **Причина** | **Пояснение** | **Коррекция** |
| Расширенный интервал отбора проб масла | Если время работы масла в пробе больше, чем время работы масла в пробах прошлого образца, износ металлов может быть повышен. | Проверьте, не превышает ли время работы масла на момент пробы более чем на 25 процентов по сравнению с предыдущими часами пробы. Если да, то повторите выборку через регулярный интервал. |
| Дистанционное циркуляция масляных резервуаров, макияжное масло или системы CentinelTM не работают должным образом | Масло не пополняется или мощность масла снижается, что приводит к повышенному уровню износа металлов. | Ремонт удаленной циркуляции масла, макияжа или системы CentinelTM. Резервный образец через регулярный интервал. |
...
| **Уровень содержания натрия на миллион (PPM) превышает предельный уровень натриевой осторожности, основанный на результатах масляных лабораторий или Cummins Inc. пределов** |  |  |
|---|---|---|
| **Причина** | **Пояснение** | **Коррекция** |
| Недавно был отремонтирован двигатель, что привело к загрязнению масла. | Предыдущий ремонт вызвал загрязнение маслом, и масляная система была **не** очищена. | Меняйте масло в отстойнике двигателя и удаленном масляном баке. Проверьте внешнюю масляную схему (т.е. Удаленный фильтр головки/удаленный масляный бак) для загрязнения. Резервный образец через регулярный интервал. |
...
| **Уровень содержания натрия на миллион (PPM) превышает критический предел натриевого флага, основанный на результатах масляных лабораторий или Cummins Inc. пределов** |  |  |
|---|---|---|
| **Причина** | **Пояснение** | **Коррекция** |
...
| **Вязкость выше предела на основе результатов масляной лаборатории или Cummins Inc. пределов** |  |  |
|---|---|---|
| **Причина** | **Пояснение** | **Коррекция** |
| Расширенный интервал отбора проб масла | Если время работы масла в пробе больше, чем время работы масла в пробах прошлого образца, вязкость может быть повышена. | Проверьте, не превышает ли время работы масла на момент пробы более чем на 25 процентов по сравнению с предыдущими часами пробы. Если да, то повторите выборку через регулярный интервал. |
| Дистанционное циркуляция масляных резервуаров, макияжное масло или системы CentinelTM не работают должным образом | Масло не пополняется или мощность масла снижается, вызывая повышенную вязкость. | Ремонт удаленной циркуляции масла, макияжа или системы CentinelTM. Резервный образец через регулярный интервал. |
| Недавно был отремонтирован двигатель, что привело к загрязнению масла. | Предыдущий ремонт вызвал загрязнение маслом, и масляная система была **не** очищена. | Меняйте масло в отстойнике двигателя и удаленном масляном баке. Проверьте внешнюю масляную схему (т.е. Удаленный фильтр головки/удаленный масляный бак) для загрязнения. Резервный образец через регулярный интервал. |
...
| **Вязкость ниже предела на основе результатов масляной лаборатории или Cummins Inc. пределов** |  |  |
|---|---|---|
| **Причина** | **Пояснение** | **Коррекция** |
| Недавно был отремонтирован двигатель, что привело к загрязнению масла. | Предыдущий ремонт вызвал загрязнение маслом, и масляная система была **не** очищена. | Меняйте масло в отстойнике двигателя и удаленном масляном баке. Проверьте внешнюю масляную схему (т.е. Удаленный фильтр головки/удаленный масляный бак) для загрязнения. Резервный образец через регулярный интервал. |
