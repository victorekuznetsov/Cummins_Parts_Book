...
pdf: "https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/main/bulletins/procedures/00-378-013.pdf"
tags:
  - "документ/процедура"
  - "двигатель/C8.3"
  - "двигатель/K38/K50"
...
> **Источник:** [QuickServe](https://quickserve.cummins.com/qs3/pubsys2/xml/en/procedures/00/00-378-013.html) · [PDF-оригинал](https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/main/bulletins/procedures/00-378-013.pdf)
...
Понимание масляной системы имеет решающее значение для определения того, когда образец масла указывает на проблему. Большая емкость масляной системы, использование систем пополнения масла CentinelTM и использование систем фильтрации типа центрифуги приведет к общему снижению уровня загрязнения по сравнению с двигателем, который имеет стандартную емкость отстойника, **no** систему пополнения и стандартные масляные фильтры.

Емкость системы может быть определена путем знания объема масла, необходимого для прикосновения к знаку высокого уровня на измерителе уровня масла, и объема любых удаленных масляных резервуаров на машине, в которой масло непрерывно циркулирует. Мощности отстойника масла перечислены в Руководстве по эксплуатации и техническому обслуживанию для всех двигателей Cummins®. Если машина оснащена системой запаса масла с резервуаром, удаленным от отстойника моторного масла, объем резервуара **должен быть добавлен к объему отстойника двигателя для определения общей емкости системы. Это верно только для удаленных резервуаров, в которых масло постоянно циркулирует. Объем макияжа CentinelTM** не** увеличивает емкость системы, поскольку моторное масло** не непрерывно циркулирует через этот бак.

Добавление новой масла также уменьшит количество загрязнений в определенном объеме масла. Во всех случаях потребление масла должно быть документально подтверждено количеством масла, добавленного между каждым интервалом обслуживания. Трендовое потребление масла важно, так как любой растущий тренд или изменение уровня потребления масла может указывать на возможное износ цилиндров, утечки системы или другие неисправности системы.

### Ограничения флага

Цель ограничения флага – выявить неблагоприятные тенденции в анализе масла, которые требуют дальнейшего изучения. Руководящие принципы, предусмотренные в этой процедуре, не должны использоваться для увеличения интервалов слива масла. Если лимит флага достигнут, обратитесь к местному поставщику услуг Cummins®.
...
Cummins Inc. Рекомендуется работать с лабораторией анализа масла для установления предупредительных и критических пределов флага с использованием метода ASTM D7720-11. Если образцы периодически берутся в течение всего интервала отвода масла, значение параметра масла может быть преобразовано в скорость увеличения по сравнению с новым маслом. Например, части на миллион (ppm) на милю (или час).

В качестве ориентира для статистического сравнения пределов следует сгруппировать следующие механизмы.

- Модельный сервис
- Применение
- Похожие циклы дежурств
- С CentinelTM и без CentinelTM
- С фильтрацией типа центрифуги или EliminatorTM и со стандартными конфигурациями фильтров
- Мощности общей масляной системы.

Если все двигатели сгруппированы вместе, независимо от масляных систем, двигатели с фильтрацией типа CentinelTM, EliminatorTM/центрифуга и более высокими мощностями масляной системы будут испытывать более высокую степень износа двигателя до достижения предела флага.

| Таблица 1: Пределы анализа моторного масла |  |
|---|---|
| Параметр анализа масла | предел |
| **Дизельное топливо и природный газ** |  |
| Изменение вязкости при 100°C \[212°F\] (Американское общество испытаний и материалов (ASTM) D445) | Для xW-40 - MIN: 12.5, Макс: 16.3 Для xW-30 - Мин: 9.3, Макс: 12.5 |
| Разбавление топлива | 5% MAX |
| **Газолин** |  |
| Изменение вязкости при 100°C \[212°F\] (Американское общество испытаний и материалов (ASTM) D445) | Для xW-30 - MIN: 7.5, Макс: 12.5 |
| Разбавление топлива | 12% MAX |

| Таблица 2: Руководящие принципы по загрязнению сажи |  |  |
|---|---|---|
| Cummins® Engineering Standard (CES) | Средний диапазон B и C предел сажи | Все остальные двигатели сажают лимит |
| CES 20076, 20077, 20078, 20081, 20086, 20087 и 20088 | 3,0% MAX | 5,0% MAX |
| CES 20075 | 1,5 % MAX | 1,5 % MAX |

| Таблица 3: Пределы загрязнения маслом для общего базового числа (TBN) |  |  |
|---|---|---|
|  | Двигатели ≤ 19 л Смещение | Двигатели ≥ 19 л Смещение |
| Общее базовое число (ASTM) D4739 | 2,5 мин. | 2.5 мин. или равный TAN |

> [!note] Примечание
> Таблица 4 должна использоваться только в качестве справочной информации, если ограничения по флагу не могут быть установлены посредством статистического анализа.

| Таблица 4: Руководство по анализу моторного масла |  |
|---|---|
| Параметр анализа масла | предел |
| Содержание воды (ASTM D6304) | 3000 частей на миллион (ppm) или 0,3% MAX |
| Кремний (SI) | 15 ppm больше, чем новая масло |
| Натрий (Na) | 20 ppm прироста по сравнению с новой маслом |
| Бор (Б) | 25 ppm больше, чем новая масло |
| Калий (K) | 20 ppm прироста по сравнению с новой маслом |
| Медь (CU) | 30 ppm больше, чем новая масло |
| Железо (FE) | 80 ppm выросли по сравнению с новой маслом |
| Алюминий (AL) | 20 ppm прироста по сравнению с новой маслом |

[[00-378-014 — 17 Liter and Up Diesel Engine Oil Analysis|См. процедуру 378-014]]в разделе 4 для конкретных рекомендаций по анализу масла дизельного двигателя 17 литров и выше.

### Толкование результатов

Приведенные ниже таблицы предназначены для использования в качестве руководства для изучения результатов анализа масла. В таблице 5 приводится информация, касающаяся случаев высокоизносных металлов или масляных загрязнителей, полученных в результате анализа, и потенциальных источников этих загрязнителей. В таблице 6 представлена информация, касающаяся сочетаний износа металлов и/или загрязняющих веществ и связанных с этим проблем с двигателем, которые могут привести к этим результатам.

| Таблица 5: Источники носовых металлов или загрязняющих веществ |  |
|---|---|
| Алюминий (AL) | Пистоны (некоторые применения), коленчатые валы и подшипники тяги кулачкового вала, втулки кулачкового вала, грязь, теплообменник, охлаждающий поток пайки, шатуны и основные подшипники, втулки кулачкового вала и зубчатого состава |
| Медь (Cu) | Подключающие стержни и основные подшипники, подшипники турбокомпрессора, соединительные втулки стержней, втулки распределительных валов, втулки бездельников и вспомогательные приводные втулки и тяговые мойщики, штифты кранов, втулки рычагов клапанных клапанов, втулки смазочных насосов и тяговые мойщики, охладители медного масла |
...
| Железо (Fe) | Линейка цилиндров, поршни (некоторые приложения), шестерни, коленчатый вал, распределительный вал, распределительные ролики и валы |
| Свинец (Pb) | Некоторые приложения: Род и основные подшипники, подшипники турбокомпрессора, втулки распределительного вала, втулки для бездельников и вспомогательные приводные втулки и моющие устройства с тягой, втулки смазочных насосов и мойщики с тягой |
| Калий (K) | Элемент, содержащийся в некоторых пакетах с добавками охлаждающей жидкости, что указывает на загрязнение масла охлаждающей жидкостью, поток пайки теплообменника охладитель |
| Кремний (Si) | Песок, грязь, кремниевые герметики и прокладочные материалы, масляные добавки, ингибитор системы охлаждения |
| Натрий (Na) | Элемент, содержащийся в некоторых пакетах с добавками охлаждающей жидкости, который указывает на загрязнение масла охлаждающей жидкостью, соли из моющего средства или дороги, масляной добавки |
| вязкость | Свойство жидкости, измеренное при определенной температуре, которая сопротивляется силе, чтобы вызвать поток жидкости. Вязкость может увеличиваться после термической деградации и может уменьшаться при механическом сдвиге. |
| Общее количество кислот (TAN) | Мера кислотности масла. Новое масло имеет определенный уровень, или стартовый TAN, за счет вида и количества добавок. |
| Общее базовое число (TBN) | Измерение способности масла нейтрализовать кислоту. Новое масло имеет определенный уровень в масляной формуле, который со временем истощается из-за контакта с кислотой, образующейся в результате продува газов. |
| сажа | Побочные продукты горения в масле. Чрезмерная сажа может указывать на проблемы с топливной системой или сгоранием. |
...
| Таблица 6: Проблемы с двигателем, которые приводят к флагам анализа масла |  |
|---|---|
| Разбавление топлива | Лучшими показателями разбавления топлива являются повышенный процент топлива в масле и снижение вязкости. В случаях высокого разбавления топлива масло может пахнуть топливом, и уровень моторного масла может увеличиться. |
| Загрязнение охлаждающими веществами | Лучший способ обнаружить загрязнение масла хладагентом — это искать следы элементов, таких как натрий и калий, которые содержатся в упаковке с добавками хладагента. Эти элементы могут быть обнаружены на очень низких уровнях, поэтому они могут служить в качестве предварительного уведомления. Натрий иногда содержится в пакетах смазочных добавок; поэтому важно сравнить уровень натрия с уровнем, измеренным в исходном (неиспользованном) образце масла, и знать состав упаковки с добавками охлаждающей жидкости. Тест на содержание воды является **не** надежным показателем утечки охлаждающей жидкости, поскольку температура масла обычно выше точки кипения воды, что позволяет воде выкипеть во время нормальной работы двигателя, оставляя за собой только элементы, которые составляют пакет присадок охлаждающей жидкости. Тесты на гликол также могут быть неточными. Небольшое количество гликоля может быть измеримым, а высокие температуры масла могут расщеплять гликоль до состояния, когда он не будет обнаружен. Помимо выявления источника загрязнения охлаждающей жидкостью, важно определить, произошло ли прогрессирующее повреждение. Охлаждающее загрязнение моторного масла может повредить подшипники и втулки. |
| Выброшенная пыль | Грязь состоит в основном из кремния и глинозема, поэтому повышенные уровни кремния и алюминия в соотношении от 2 до 1 до 10 до 1 указывают на проникновение грязи. Помимо определения источника попадания грязи, важно определить, произошло ли прогрессирующее повреждение. Выпадение пыли может вызвать износ поршневого кольца / цилиндрового гильзы, что может привести к загрязнению моторного масла или нарушению свойств моторного масла и отказам подшипников / подвесок. |
| Кольцо Пистона / Носить цилиндрический гильза | Повышенный уровень железа указывает на износ цилиндров. Кроме того, износ колец и/или вкладыша может позволить продувающимся газам проходить мимо колец и загрязнять масло. Можно наблюдать повышенный уровень сажи, окисления и нитратации, а также увеличение вязкости. Загрязнение масла также увеличит общее количество кислоты и уменьшит общее количество основания. Помимо выявления источника загрязнения охлаждающей жидкостью, важно определить, произошло ли прогрессирующее повреждение. Загрязненное моторное масло может повредить подшипники и втулки. |
| Несоблюдение/буширование неисправности | Свинцовые подшипники состоят в основном из свинца и меди. Повышенный уровень обоих этих металлов одновременно может указывать на неисправность подшипника и/или втулки. Увеличение свинца без увеличения меди обычно соответствует деградации состояния масла, что приводит к атаке верхнего слоя подшипника. Некоторые другие признаки деградированного масла могут включать увеличение окисления, нитратирования, TAN и/или снижение TBN. Свинцовые свободные подшипники будут показывать увеличение таких элементов, как медь, висмут, алюминий и олово. |
