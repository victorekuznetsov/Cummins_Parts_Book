...
Удалите стяжка, используемый для защиты шлангов. Пометьте и пометьте шланги, когда они удаляются. Укажите их местоположение.
...
Удалите уплотнение лица № 4 на 90-градусном локте (1) у масляной винты.
...
Удалите шланг подачи масляной винты (5) из 9/16-18 прямоточного кольцевого крепления (6) с наружной резьбой в масляной винте блока двигателя.
...
Удалите шланг подачи масляной винты (2) из масляной винты, установленной на клапан управления (4).

Удалите шланг пополнения масла (8) из клапана (4) управления в крышку отверстия (6).

![[07800047.png]]

Удалите масляную винт (1) в блок двигателя масляной винты (9/16-18 SAE o-ring), расположенную чуть выше и справа, обращенную к управляющему клапану.
...
Удалите фитинг из масляной винты.
...
Удалите шланг у масляной винты (1) и макияж соленоида.
...
Отсоедините шланг подачи масляной винты (1) от управляющего клапана к разъёму клапана проверки масляной винты.
...
> [!note] Примечание
> Вероятно, потребуется удалить ECM, чтобы получить доступ к рекомендуемым портам масляных винтов. Однако на обычных грузовиках с коротким носом сделать это очень сложно и занимает много времени из-за помех кабине и раме рельса. Предполагается, что первым альтернативным местом поставки масла является выбор одной из двух вилок подачи масла, расположенных на вспомогательном диске. Если выбрано это место, необходимо будет заказать уплотнение лица 4 кольцом х 1/4 NPT или уплотнение лица 4 кольцом х 9/16-18 кольцом прямой резьб для размещения уплотнения лица 4 кольцом локтем 90 градусов. В качестве последнего выбора, подачу масла на головку масляного фильтра, которая обычно используется для подачи масла на тормоза сжатия, является вариантом. Однако, если вы выберете это место, вы должны использовать проводную линию подачи масла, потому что это место находится на горячей стороне двигателя (турбокомпрессор и выхлопной коллектор).

Удалите масляную винт на стартовой стороне блока двигателя.

![[07800032.png]]

Установите 1/4 NPT (**не**) или 9/16-18 прямоточный адаптер с кольцом в масляный порт винты.
...
Установите шланг на номер 4, герметизирующий кольцо 90-градусного локтя (1) у масляной винты.
...
Установите 9/16-18 прямоточного окольца с внешней резьбой (6) в двигательный блок масляной винты порт, расположенный чуть выше и справа, обращенный к управляющему клапану.
...
Подключите шланг подачи масляной винты (5) к прямоточному кольцу (6) с наружным резьбовым креплением (6) в масляной винте блока двигателя. Затяните шланговый разъём.
...
Установите масляную винт (1) в блок двигателя масляной винты порт (9/16-18 SAE o-ring), расположенный чуть выше и справа, обращенный к управляющему клапану.
...
Подключите шланг подачи масляной винты (5) от масляной винты к клапану управления.
...
> [!note] Примечание
> Масляная винт содержит три 1/8-дюймовых трубных заглушки и две 9/16-дюймовые кольцевые заглушки.

Выберите наиболее удобное место для установки контрольного клапана. Удалите 1/8-дюймовый трубчатый заглушек из масляной винты.
...
Установите контрольный клапан в масляную винт. Контрольный клапан представляет собой 1/8-дюймовый NPTF до 6-го торцевого уплотнительного кольцевого фитинга.
...
> [!note] Примечание
> Блок цилиндра, вероятно, будет расположен позади стартера. Если это так, то необходимо будет использовать другую блок-плагину, которая обычно используется для слива масла STC. В этом случае потребуется «Т» для объединения линии нефтеотвода STC и линии пополнения CentinelTM. Следующие шаги описывают процедуру.
...
Удалите масляную винт 9/16-18 или 1/8-27 NPT (**не**) из середины топливного насоса на стороне блока двигателя.

> [!note] Примечание
> Если этот порт недоступен, найдите доступный порт на главной масляной винте.

![[05100021.png]]

Установите адаптер с прямым кольцевым приводом 9/16-18 или адаптер с 1/8 NPT (**не**) в порт масляной винты.
...
Установите локтевой уплотнитель с кольцевым кольцом № 4 на 9/16-18 прямоточный адаптер с кольцевым кольцом, который был только что установлен в масляном порту винты.
...
Установите шланг на торцевую уплотнение № 4 90-градусного локтя на масляной винте (1).
...
Подключите шланг подачи масляной винты (1) от управляющего клапана к разъёму клапана проверки масляной винты. Поместите разъём и шланг контрольного клапана, чтобы убедиться, что между шлангом и впускным коллектором есть зазор.
...
Установите уплотнительный шайбый станок туда, где только что было удалено кольцо.

![[05400041.png]]

Там, где пробка была удалена из масляной поддона, установите адаптер 1/2-20 с уплотнительной шайбой машиной.
...
Установите клапан (1) проверки масляной винты в переднем чехле передней передачи.
