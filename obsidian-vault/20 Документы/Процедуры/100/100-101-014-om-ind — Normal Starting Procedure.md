...
source: "https://quickserve.cummins.com/qs3/pubsys2/xml/en/procedures/100/100-101-014-om-ind.html"
pdf: "https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/main/bulletins/procedures/100-101-014-om-ind.pdf"
tags:
  - "документ/процедура"
  - "двигатель/C8.3"
  - "группа/100"
...
> [!abstract] Процедура · `100-101-014-om-ind`
...
> **Источник:** [QuickServe](https://quickserve.cummins.com/qs3/pubsys2/xml/en/procedures/100/100-101-014-om-ind.html) · [PDF-оригинал](https://github.com/victorekuznetsov/Cummins_Parts_Book/raw/main/bulletins/procedures/100-101-014-om-ind.pdf)
...
Если температура окружающей среды ниже 16 ° C \[60 ° F \], обратитесь к следующей процедуре.[[100-101-004-om-ind — Cold Weather Starting|См. процедуру 101-004 в разделе 1.]]
...
> [!note] Примечание
> Двигатели, оснащенные двигателями с воздушным пуском, требуют минимум 480 кПа[70 psi].

- Отключите приводной блок или, если он оборудован, поместите трансмиссию в нейтральное положение.
- С педалью ускорителя или рычагом в положении холостого хода поверните переключатель зажигания в положение Включения и дождитесь выхода лампы WAIT-TO-START; затем поверните ключ к положению START.
- Полный дроссел применяется после включения стартера, после 5 секунд выпуска до холостого дросселя.
...
Переключатель зажигания переключателя в положение Включено или Бег. Когда ключ находится в этом положении, лампа «Подождите до начала» будет освещаться максимум 20 секунд. Двигатель должен быть включен до тех пор, пока лампа WAIT TO START не выключится.
...
Когда лампа «Подождите до начала» выходит из строя, цикл предварительного нагрева завершается.

![[15900018.png]]

Запуск двигателя с помощью дроссельной заслонки в положении IDLE.

Эта черная кнопка (5) используется для запуска стартера.
...
> [!warning] ОСТОРОЖНО
> Не работайте с двигателем на низком холостом ходу в течение длительного времени при температуре охлаждающей жидкости двигателя ниже минимальной спецификации в технических характеристиках технического обслуживания в этом руководстве. Это может привести к следующему:

- Разбавление топлива моторного масла
...
> [!warning] ОСТОРОЖНО
> При использовании прыгунных кабелей для запуска двигателя убедитесь, что кабели соединены параллельно: Положительный (+) к положительному (+) и отрицательный (-) к отрицательному (-). При использовании внешнего электрического источника для запуска двигателя, поверните выключатель в положение выключения. Удалите ключ перед креплением прыгунов.

> [!warning] ОСТОРОЖНО
> Чтобы избежать повреждения деталей двигателя, не подключайте пусковой или зарядный кабель к любой топливной системе или электронному компоненту.
...
>
> If ambient temperature is below 16°C \[60°F\], reference the following procedure. [[100-101-004-om-ind — Cold Weather Starting|Refer to Procedure 101-004 in Section 1.]]
>
> **CAUTION · Осторожно**
...
> - Full throttle is applied after engaging the starter, after 5 seconds release to idle throttle.
...
>
> **Note · Примечание**
> There is a separate keyswitch wired to the primary panel. It will be installed in the helm at the boat manufacturer's or installer's desired location. The keyswitch **must** be in the ON or RUN position to crank-start the engine.
>
> Disengage the drive unit.
>
> Turn the key switch to the ON or RUN position. When the key is in this position, the WAIT TO START lamp will be illuminated for a maximum of 20 seconds. The engine should **not** be cranked until the WAIT TO START lamp shuts off.
>
> **Note · Примечание**
> The controller is reset each time the ignition is turned off and the cycle will start over.
>
> When the WAIT TO START lamp goes out, the preheat cycle is complete.
>
> Start the engine with the throttle in the IDLE position.
>
> This black push button (5) is used to engage the starter motor.
>
> **CAUTION · Осторожно**
> The engine must have adequate oil pressure within 15 seconds after starting. If the WARNING lamp indicating low oil pressure has not gone out or there is no oil pressure indicated on a gauge within 15 seconds, shut off the engine immediately to avoid engine damage. The low oil pressure troubleshooting procedure is located in Troubleshooting Symptoms (Section TS).
>
...
>
> **CAUTION · Осторожно**
> Do not operate engine at low idle for long periods with engine coolant temperature below the minimum specification in Maintenance Specifications in this manual. This can result in the following:
>
