...
- Блок SDU410 будет **не** реагировать на команду сброса/подтверждения.
...
| Проверьте блок DCU410 или удаленную панель для мигающего светодиода. Нажмите кнопку «Признать», чтобы увидеть, изменяется ли светодиод на устойчивую подсветку. | Светодиодная вспышка? *** Заменить модуль SDU410. Обратитесь в авторизованный сервисный центр Cummins®. | Ремонт завершён |
| Светодиодная вспышка? ** НЕТ** | 1В |  |
...
| **Условия: ** Откройте окно интерфейса клиента. Отключите сигнал перекрытия выключения двигателя и возвратные провода на блоке SDU410 и разъеме C4. |  |  |
|---|---|---|
| **Действие** | ** Спецификация/ремонт** | ** Следующий шаг** |
| Проверьте сигнал переключения выключения двигателя и возвратные провода для открытого. Поместите один испытательный щуп на провод сигнала отключения двигателя на блоке SDU410. Поместите другой испытательный щуп на контакт сигнала отключения двигателя на разъеме C4. Поместите один испытательный щуп на провод обратного отключения двигателя на блоке SDU410. Поместите другой испытательный щуп на выключатель двигателя, переопределите обратный контакт на разъеме C4. См. соответствующую схему или схему проводов для идентификации штифта и провода. | Менее 10 Ом? *Да** | 2А |
| Менее 10 Ом? **NORepair:** Заменить провод.[[116-015-023 — Customer Interface Box\|См. процедуру 015-023 (Customer Interface Box) в разделе 15.]] | Ремонт завершён |  |
